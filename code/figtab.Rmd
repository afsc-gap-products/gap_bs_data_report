...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_print <- table_print %>% 
    ...
    flextable::valign(valign = "top") %>%
    theme_flextable_nmfstm(font0 = font0, x = .) 
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_print <- table_raw %>% 
    ...
    NMFSReports::theme_flextable_nmfstm(font0 = font0, x = ., row_lines = TRUE)  %>%
    flextable::padding(x = .,
                       j = 1, i = ~ is.na(oto_samp_type),
                       padding.left = 5)
...
table_print <- table_raw %>%
  ...
  NMFSReports::theme_flextable_nmfstm(font0 = font0, row_lines = TRUE) %>% 
  flextable::padding(x = .,
                     j = 1, i = ~ is.na(SRVY),
                     padding.left = 5)
...
for (i in 1:2) { # two cases, above and below
  ...
  figure <- plot_temps_facet(
    ...
    title0 = stringr::str_to_sentence(case))
...
if (SRVY == "NEBS") {
  ...
  figure <- plot_temps_facet(
    ...
    colorbar_breaks = c(-Inf, seq(from = 0, to = 14, by = 2), Inf),
    row = 2, #ifelse(length(nbsyr)>=2, 2, 1), 
    viridis_palette_option = "H") 
...
for (i in 1:2) {
  ...
  if (i == 1){ # CPUE (WT/HA) and BIOMASS
    ...
    table_print <- table_print %>%
      ...
      flextable::valign(valign = "top") %>%
      NMFSReports::theme_flextable_nmfstm(x = ., font0 = font0, row_lines = TRUE, 
                                          pgwidth = full_page_landscape_width)
    ...
    table_print <- table_print %>%
      ...
      flextable::merge_v(x = ., 
                         ...
                         part = "body") %>% 
      flextable::valign(valign = "top") %>%
      NMFSReports::theme_flextable_nmfstm(font0 = font0, 
                             row_lines = TRUE, 
                             pgwidth = full_page_landscape_width)
  }
