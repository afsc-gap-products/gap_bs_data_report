...
spp_sci <- ifelse(spp_file == "tunicates", "Tunicata", spp_file) # TOLEDO
spp_sci <- ifelse(spp_file == "neptune-whelks", "*Neptune* spp.", spp_file) # TOLEDO
...
spp_plot_agecomp <- (nrow(agecomp |>
                            dplyr::filter(species_code %in% spp_code) |>
                            dplyr::filter(year %in% analysisyrs) ) >0)
age_cutoff <- ifelse(spp_plot_agecomp, a$age_cutoff[1], NA)
...
dist_plot <- function(yrs, height, row0, nickname, report_title0, spp_print, spp_code) {
  ...
  header <- paste0("CPUE (kg/km^2^) distribution of ", 
                   ...
                   " from the ",
                   range_text(yrs), " ", 
                   text_list(paste0(unique(table_raw$srvy_long))), # SURVEY, 
                   " Bering Sea survey", 
...
srvy_len <- unique(sizecomp$srvy[sizecomp$year %in% yrs])
...
age_plot <- function(srvy, yrs, agecomp0, height, width, nickname, legend_font_size, report_title0, age_cutoff) {
  ...
  header <- paste0("Total abundance-at-age estimates of ",
                   ...
                   "Age distributions are scaled up to the total estimated population size. ")
  ridgeline <- TRUE
  
  if (ridgeline) {
    
    header <- paste0(header, "For figure legibility, specimens over ", age_cutoff, " have been removed. ")
    
    table_raw0 <- table_raw |> 
      dplyr::group_by(srvy, year, age) |> 
      dplyr::summarise(population_count = sum(population_count, na.rm = TRUE)) |> 
      dplyr::ungroup() |> 
    ...
    figure_print <- ggplot2::ggplot(
      ...
      ggridges::geom_ridgeline(
        ...
        # rel_min_height = 0.01, 
        linewidth = 0.1) +
      #   ggplot2::facet_grid(cols = vars(srvy)) +
      #      ggridges::geom_ridgeline(
      #        fill = "#3B528B",     # Solid fill color
      # alpha = 0.5,
      # color = "#3B528B", # NA      # Single clean outline around each year
      # linewidth = 0.1) + 
      ggplot2::theme(legend.position = 'none') +
      ggplot2::scale_y_reverse( # scale_y_continuous
        breaks = breaks_width(2),
        labels = scales::label_number(accuracy = 1, big.mark = ""), 
        name = paste0("Population", unit_word)) +
      ggplot2::scale_x_continuous(
        name = "Age bin", 
    ...
    header <- paste0(header, "Note that unexed otoliths were removed for figure legibility and that otoliths are not collected for each species each year and require years to process after collection. ")
    ...
    figure_print <- ggplot2::ggplot(
      ...
      ggplot2::facet_grid(rows = vars(year), cols = vars(srvy)) +
      ...
      #                           na.value = "transparent", 
      #                           drop = FALSE) +
      ggplot2::scale_x_continuous(name = "Age bin"#, 
                                  # label = abs,
                                  # breaks = function(age) unique(floor(pretty(seq(-(max(age) + 1) * 1.1, 
                                  #                                                (max(age) + 1) * 1.1))))
      ) +
      ggplot2::scale_y_continuous(name = paste0(#spp_print, " ",
        "Population", unit_word),
        # labels = abs,
        expand = c(0, 0)) +
  ...
  if (!ridgeline) {
    ...
    figure_print <- figure_print + 
      ...
      ggplot2::geom_text(data = table(year = table_raw$year, 
                                      # srvy_long = table_raw$srvy_long, 
                                      srvy = table_raw$srvy) |> 
                           data.frame() |> 
                           dplyr::mutate(sex  = "Males", # had to pick something and this is the most generalizable across species
                                         sign = ifelse(Freq == 0, "*No Collection/\nNot Processed", NA), 
                                         # srvy_long = srvy_long, 
                                         srvy = srvy), # stringr::str_to_title(srvy_long)), 
...
yrs <- sort(unique(agecomp0$year)) 

agecomp0 <- agecomp0 |> 
  dplyr::filter(year >= max(yrs)-10)

yrs <- yrs_age <- sort(unique(agecomp0$year)) 

srvy_age <- unique(agecomp0$srvy[agecomp0$year %in% yrs_age])
...
temp <- age_plot(srvy, yrs, agecomp0 = agecomp0, height, width, nickname, legend_font_size, report_title0, age_cutoff)
for (jjj in 1:length(temp)) { assign(names(temp)[jjj], temp[[jjj]]) }
pres_age <- temp$figure_print
res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```
...
temp_bio <- dplyr::right_join(
  ...
  by = c("year", "srvy")) |> 
  dplyr::right_join(data.frame(srvy = c(names(compareyr), names(compareyr)), 
                               year = c(compareyr, rep_len(x = maxyr, length.out = length(compareyr)) ))) |>     # dplyr::filter(year %in% yrs) |> 
  ...
  temp_bio1  <- dplyr::bind_rows(
    ...
    c(
      ...
      "Biomass" = dplyr::case_when(
        ...
        temp_bio$biomass_mt[i] <= 1 ~ ">1 t", 
        TRUE ~ paste0(xunits(temp_bio$biomass_mt[i],
...
temp_bio1$`Population; Biomass` <- paste0(temp_bio1$Population, "; ", temp_bio1$Biomass) 
...
temp_catch <- catch_haul_cruises |> 
  dplyr::filter(species_code %in% spp_code) |> 
  dplyr::right_join(data.frame(srvy = c(names(compareyr), names(compareyr)), 
                               year = c(compareyr, rep_len(x = maxyr, length.out = length(compareyr)) ))) |>     # dplyr::filter(year %in% y  
...
temp_catch$str <- paste0(
  ifelse(temp_catch[,paste0("yr_1111")] < temp_catch[,paste0("yr_", maxyr)],
         "Increased", "Decreased"), 
  " from ", 
  xunits(unlist(temp_catch[,"yr_1111"])), 
  ifelse(spp_print == "tunicates", " kg", ""), " in ", 
  temp_catch$compareyr,
  " to ", 
  xunits(unlist(temp_catch[,paste0("yr_", maxyr)])), # ),
  " ",
  ifelse(spp_print == "tunicates", "kg", ""), 
  ifelse(temp_catch[,paste0("yr_", maxyr)]>1 & spp_print != "tunicates", "individuals","individual"),
  " in ",
  maxyr)
...
header <- paste0("Summary of ",maxyr," catch presence, temperature ranges, and extrapolated biomass and population estimates for ", 
                 ...
                 ifelse(length(table_raw$`Bering Sea\nSurvey`) == 1, 
                        paste0(tolower(table_raw$`Bering Sea\nSurvey`), " Bering Sea"), SURVEY), 
...
# if there are more than 99 individuals for one survey but not the other, 
# replace entries for biomass % rows with "-"
table_print0[table_print0$var == "% of Total Biomass", -1][which(as.numeric(table_print0[table_print0$var == "catch_min", -1])<=100)] <- "-"
table_print0[table_print0$var == "% Change in Biomass", -1][which(as.numeric(table_print0[table_print0$var == "catch_min", -1])<=100)] <- "-"

# remove rows if there are less than 99 individuals 
if (sum(table_raw$catch_min >= 100) == nrow(table_raw)) {
  # if there are more than 99 individuals for both surveys, remove "Observed Catch Totals" row
  table_print0 <- table_print0[table_print0$var != "Observed Catch Totals",]
} else if (sum(table_raw$catch_min <= 100) == nrow(table_raw)) {
  # if there are less than 99 individuals for both surveys, remove % biomass rows
...
if (sum(table_raw$catch_min >= 100) != nrow(table_raw)) {
  header <- paste0(header, 
                   "Too few organisms were observed on the ", text_list(tolower(table_raw$`Bering Sea\nSurvey`[table_raw$catch_min <= 100]))," Bering Sea survey to share meaningful percent change of total biomass values",ifelse(sum(table_print0 == "-")>0, " (note cells with '-')", ""),", so observed values are shared instead. ")      
}
...
if (sum(temp$biomass_mt[temp$stratum == "Total"] != 0, na.rm = TRUE) > 0) {
  ...
  for (i in 1:2) {
    ...
    header <- paste0(header, 
                     ifelse(length(setdiff(srvy1, unique(table_raw$srvy))) > 0,
                            # If species is missing from one survey:
                            paste0(" during the ", maxyr, " ", 
                                   haul_cruises_maxyr$srvy_long[(haul_cruises_maxyr$srvy %in% unique(table_raw$srvy))], " survey. ",
                                   "This taxon was not encountered in the ",
                                   maxyr, " ",
                                   haul_cruises_maxyr$srvy_long[!(haul_cruises_maxyr$srvy %in% unique(table_raw$srvy))],
                                   " survey. "),
                            # If species is present in both surveys:
                            paste0(" during the ", maxyr, " ", SURVEY, 
                                   " survey", plural_surveys, ". ")))
...
height <- 5 # full_page_portrait_height-2.5
...
if (spp_plot_sizecomp) {
  ...
  # comm_size_legend <- cowplot::get_legend(comm_size)
  ...
  if (grepl(x = comm_size_txt, pattern = "eastern and northern") & 
      grepl(x = comm_size_txt, pattern = range_text(c(maxyr, compareyr))) ) {
    comm_size1 <- comm_size1 + 
      ggplot2::theme(
...
if (spp_plot_agecomp & spp_plot_sizecomp) {
  
  if (length(srvy_age) == 1 & length(srvy_len) != 1) {
    spacing0 <- c(.25, .15)
  } else if (length(srvy_age) != 1 & length(srvy_len) == 1) {
    spacing0 <- c(.15, .25)
  } else {
    spacing0 <- c(.2, .2)
  }
  ...
  figure_len_age <- cowplot::plot_grid(
    comm_size1, 
    pres_age1,
    ncol = 2, 
    rel_widths = spacing0,
  ...
  #   cowplot::draw_label("D", x = ifelse((length(srvy_age) == 1), 0.85, .8), y = 0.94, hjust = 0, vjust = 0, size = 16)
  ...
  if (FALSE){  
    figure_print <- cowplot::ggdraw(figure_print) +
      cowplot::draw_label(
        label = "D", 
        x = dplyr::case_when(
          length(srvy_age) == 1 & length(srvy_len) == 1 ~ 0.8,
          length(srvy_age) == 1 & length(srvy_len) == 2 ~ 0.85,
          length(srvy_age) == 2 ~ 0.8
        ), 
        y = 0.94, 
        hjust = 0, 
        vjust = 0, 
...
} else if (!spp_plot_agecomp & spp_plot_sizecomp) {
  figure_print <- cowplot::plot_grid(
    temp1, 
    NULL,
    comm_size1, 
...
if (FALSE) { # TOLEDO
  ...
  figure_print <- cowplot::ggdraw(figure_print) +
    cowplot::draw_label("A", 
                        x = ifelse(exists("comm_size"), 0.04, 0.01), 
...
header <- paste0(
  ...
  "survey",fig_srvy_plural," (points and solid lines, with estimated 95% confidence interval error bars) and time series mean (dashed lines; ",
...
if (spp_plot_agecomp) {
  ...
  srvy_age <- gsub(pattern = "EBS", replacement = "eastern", x = srvy_age)
  srvy_age <- gsub(pattern = "NBS", replacement = "northern", x = srvy_age)
  
  header <- paste0(header, "(D) Additionally, total abundance-at-age (cm) estimates were calculated and scaled to the total estimated population from the last three years of available data (",text_list(yrs_age),") in the ",text_list(srvy_age),"  Bering Sea surveys. ")
  
}

header0 <- header
figure_print0 <- figure_print


res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-combined-[spp]-pres}
nickname <- paste0("fig-combined-", spp_file, "-pres")
height <- 5.25 # full_page_portrait_height-2.5
width <- full_page_landscape_width

temp <- dplyr::left_join(
  pres_timeseries_mean |> 
    dplyr::filter(y_long == "Biomass"), 
  pres_stats |> 
    dplyr::select(-srvy_long))
...
figure_print <- ggdraw(add_sub(
  plot = figure_print0, 
  size = 10, 
  label = paste0(spp_print, ifelse(spp_plot_agecomp, paste0("; age cutoff: ", age_cutoff), ""), "\n", 
