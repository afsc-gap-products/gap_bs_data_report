...
Bottom (Fig. \@ref(fig:fig-temperature-bt-data)) and surface (Fig. \@ref(fig:fig-temperature-st-data)) water temperatures varied spatially and among years due to variation in atmospheric and oceanic conditions that influence temperature patterns in the eastern Bering Sea [@RN931]. The warmest bottom temperatures were typically observed in the `inner domain (< 5`0 m; Fig. \@ref(fig:fig-survey-sample-stations)) along the Alaskan mainland. The coldest bottom temperatures typically occurred in the `middle domain (50–100 m)`. Surface temperatures generally increased as the survey progressed westward from the interior of Bristol Bay to the northwestern outer shelf and northward into Norton Sound. These surface temperature patterns reflect seasonal warming that occurs over the course of the survey [@Cokelet2016; @RohanColdPool].
...
maxyr_area0 <- sum(temp$area_km2[temp$year == maxyr]
maxyr_area <- formatC(x = maxyr_area0, digits = 0, big.mark = ",", format = "f")

# compareyr_perc <- formatC(x = sum(temp$perc[temp$year == compareyr]), digits = 1, big.mark = ",", format = "f")
compareyr_area0 <- sum(temp$area_km2[temp$year == compareyr])
compareyr_area <- formatC(x = compareyr_area0, digits = 0, big.mark = ",", format = "f")
compareyr_area_perc_diff <- pchange(start = compareyr_area0, end = maxyr_area0)

mean_perc <- formatC(x = sum(temp$perc[temp$year == compareyr]), digits = 1, big.mark = ",", format = "f")
mean_area <- formatC(x = sum(temp$area_km2[temp$year == compareyr]), digits = 0, big.mark = ",", format = "f")

max_perc <- formatC(x = sum(temp$perc[temp$year == maxyr]), digits = 1, big.mark = ",", format = "f")
max_area <- formatC(x = sum(temp$area_km2[temp$year == maxyr]), digits = 0, big.mark = ",", format = "f")

min_perc <- formatC(x = sum(temp$perc[temp$perc == maxyr]), digits = 1, big.mark = ",", format = "f")
min_area <- formatC(x = sum(temp$area_km2[temp$year == maxyr]), digits = 0, big.mark = ",", format = "f")
  
```


The cold pool is primarily found in the `middle domain (50–100 m)` and covered `r maxyr_perc`% (`maxyr_area` km^2^) of the eastern Bering Sea survey area in `r maxyr` (Figs. \@ref(fig:fig-temperature-bt-data) and \@ref(fig:fig-cold-pool-area)). The cold pool was `27.7`% (`43,425` km^2^) `smaller` than in `r maxyr-1` and was `36.6`% `smaller` than the time series mean (`179,000` km^2^). The extent of isotherms for bottom temperatures in `r maxyr` of ≤ 0°C (`30,275` km^2^) and ≤ -1°C (`8,350` km^2^) were `6.0`% and `19.8`% larger than in `r maxyr-1` During the `43`-year time series, the cold pool area has ranged from a minimum of `6,150` km^2^ in `2018` to a maximum of `385,975` km^2^ in `1999`, comprising `1.2`% to `78.2`% of the total eastern Bering Sea survey area.
