...
if (report_title == "data") {
  ...
  nickname <- paste0("fig-", jj, "-", spp_file, "-3a-distribution-pres")
  yrs <- sort(nbsyr)
  height <- 4
  width <- 8
  row0 <- 2
...
legend.justification = c(0, 1)) + 
      ggtitle(gsub(x = key.title, pattern = "\n", replacement = ": "))
...
```{r fig-dist-[taxon]-[spp]-legend, eval = (spp_plot_idw | spp_plot_pa)}
# if (report_title == "pres") {
#   nickname <- paste0("fig-", jj, "-", spp_file, "-3b-distribution-pres")
#   yrs <- sort(nbsyr)
#   height <- 1
#   width <- 4
#   header <- ""
  ...
  if ("EBS" %in% table_raw$SRVY) {
    figure1 <- figure <- plot_sizecomp(
      sizecomp0 = table_raw %>% 
        ...
        dplyr::mutate(SRVY0 = SRVY, 
                      SRVY = stringr::str_to_sentence(SRVY_long)),
      lengths0 = lengths0,
      spp_code = spp_code,
