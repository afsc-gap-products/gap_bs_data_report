...
if (sum(temp$biomass_mt[temp$stratum == "Total"] != 0, na.rm = TRUE) > 0) {
  ...
  for (i in 1:2) {
    ...
    table_print <- table_print %>% 
      ...
      theme_flextable_nmfstm(row_lines = FALSE, 
                             pgwidth = full_page_portrait_width + 1.5)  %>%
      flextable::width(width = ((full_page_portrait_width + 1.5)/ncol(table_print0)), unit = "in") %>% # need to not account for the SRVY_long column
