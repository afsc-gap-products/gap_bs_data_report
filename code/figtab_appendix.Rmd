...
for (i in 1:nrow(temp)) {
  ...
  dplyr::filter(!is.na(print_name)) %>% 
    ...
    tidyr::pivot_wider(names_from = print_name, 
                       values_from = weight, 
                       values_fill = "-") %>%
    dplyr::mutate(start_time = format(x = as.POSIXlt(x = start_time), format = "%Y-%m-%d %H:%M:%S"))  %>%
  dplyr::mutate(across(where(is.numeric), 
                       formatC, big.mark=",", digits = 1, format = "f")) %>% 
    dplyr::rename("Start date and time" = start_time,
                  "Haul number" = haul,
...
table_print <- flextable::flextable(table_print) %>%
  ...
  flextable::set_header_labels(.,
                                 row = "") %>%
  theme_flextable_nmfstm(row_lines = FALSE, body_size = 9, spacing = .6)
...
for (i in 1:nrow(temp)) {
  ...
  table_raw <- catch_haul_cruises_maxyr %>% 
    ...
    dplyr::left_join(
      x = ., 
      y = spp_info %>% 
        dplyr::select(ifelse(temp$taxon[i] == "fish", all_of("family_taxon"), all_of("phylum_taxon")), species_name, common_name,  taxon, species_code), 
      by = "species_code") %>% 
    dplyr::filter(taxon == temp$taxon0[i]) %>% 
    dplyr::mutate(taxon = ifelse(temp$taxon[i] == "fish", family_taxon, phylum_taxon)) %>% 
...
table_print <- table_print %>%
  ...
  theme_flextable_nmfstm(row_lines = FALSE, body_size = 9, header_size = 9, spacing = .8)
...
temp <- tidyr::crossing(haul_cruises_maxyr %>% 
                        dplyr::select(SRVY_long, SRVY), 
                        report_spp1 %>% 
                          dplyr::filter(taxon == "fish" & table_cpue == TRUE) %>% 
