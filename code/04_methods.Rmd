...
The standardized `r SURVEY` bottom trawl survey`r ifelse(SRVY == "NEBS", "s are", " is")` based on a systematic design with `r ifelse(SRVY == "NEBS", NMFSReports::text_list(paste0(haul_cruises_maxyr$stations_avail, " fixed sampling stations in the ", haul_cruises_maxyr$SRVY)), paste0( haul_cruises_maxyr$stations_avail, " fixed sampling stations"))` centered within 37.04 × 37.04 km (20 × 20 nautical mile) grid squares (Figure `r crossref(list_obj = list_figures, nickname = "fig_sample_grid", sublist = "number") `). Additional stations, called "corner stations", were added to the survey design to better sample regions of historically high blue king crab abundances. There are 26 corner stations located at the intersection of the grid lines in the waters surrounding St. Matthew and the Pribilof Islands (Figure `r crossref(list_obj = list_figures, nickname = "fig_sample_grid", sublist = "number") `). These corner stations are sampled in addition to the center of the grid cells. `r ifelse(SRVY == "NEBS", paste0("In ", maxyr, " the NBS survey was conducted in addition to the EBS survey, using the same systematic sampling design for stations bounded by the U.S.-Russian Maritime Boundary, the Bering Strait, and Norton Sound.") , "")` 

## Survey Vessels and Sampling Gear

The `r maxyr` `r NMFSReports::text_list(SRVY1)` survey`r ifelse(length(SRVY1)>1, "s were", " was")` conducted aboard the chartered commercial stern-trawlers `r NMFSReports::text_list(vessel_info$vessel_ital)`  (Figure `r crossref(list_obj = list_figures, nickname = "fig_vessels", sublist = "number") `). `r ifelse(nrow(vessel_info) == 1, "The vessel", "Both vessels") ` are house-forward trawlers with stern ramps. The length overall of `r text_list(paste0("the ", vessel_info$vessel_ital, " is ", vessel_info$length_m, " m (", vessel_info$length_ft, " ft)") ) `. All fishing operations were conducted in compliance with national and regional protocols detailed in @RN933. Trawl sampling was conducted using 83-112 eastern otter trawls, each with a 25.3 m (83 ft) headrope and 34.1 m (112 ft) footrope (Figure `r crossref(list_obj = list_figures, nickname = "fig_trawl_gear", sublist = "number") `). The net was attached to tail chains with 54.9 m (30 fm) paired dandylines. Each lower dandyline had a 0.61 m chain extension connected to the lower wing edge to improve bottom tending. Steel "V" doors measuring 1.8 × 2.7 m (6 × 9 ft) and weighing 816 kg (1,800 lbs) each were used for spreading the net opening while the trawl was fishing on the seafloor.
...
if (sum(!is.na(temp$vessel)) == nrow(temp)) {
  ...
  str0 <- paste0(str0, "To estimate missing net width values, the mgcv package in R [@RN997] was used to relate mean net width with the inverse scope (m) and mean net height (m) from valid tows following the relationship investigated by @RN921, where $w$ is the net width (m), $h$ is the net height (m), and $s$ is the scope.
...
At the beginning of the survey, scientists boarded the chartered vessels (`r text_list(vessel_info$vessel_ital)`) in Dutch Harbor, Alaska, then transited to eastern Bristol Bay to begin sampling.  From Bristol Bay, the survey proceeded westward completing north-south columns of grid cells to the shelf edge (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `). The east to west survey progression is intended to ensure the survey moves in the opposite direction of the seasonal on-shelf (eastward) migration of yellowfin sole and other species, reducing the likelihood of encountering a portion of these populations multiple times [@RN928; @RN913]. In the EBS, `r str0$str[str0$SRVY == "EBS"]` (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `).
...
Standard catch sampling procedures used in RACE Bering Sea assessment surveys are described in detail by @RN939 and @RN933. A brief summary of these procedures is described below.
...
Catches estimated to be less than approximately 1,200 kg (2,650 lbs) were entirely sorted and enumerated, while larger catches were weighed in aggregate or volumetrically measured and subsampled before sorting. The goal of subsampling is to obtain a representative sample, which requires some variation in catch processing methods among hauls, depending on the overall size and species composition of the catches. After sorting subsampled catches, individual species were weighed and counted in aggregate, and these weights and numbers were then expanded proportionally to the total catch. Fishes and invertebrates were sorted and identified to the lowest reliable taxonomic level. 

All Pacific halibut (*Hippoglossus stenolepis*) were enumerated and all commercial crab species were weighed and enumerated from each catch. Other selected species including Greenland turbot, large skates, rockfish, Atka mackerel, prowfish, Bering wolffish, giant wrymouth, Pacific cod, some sculpins, sharks, and any other large, rare species that are not represented in the subsample were completely sorted from the catch in most cases.

Length measurements were obtained from a random subsample of selected fish species from every haul (Table`r ifelse(SRVY == "NEBS", "s", "")` `r crossref(list_obj = list_tables, nickname = "tab_specimen_samples_", sublist = "number", exact = FALSE)`). The number of fish in a random length subsample for a species was dependent on the size range of that species in the haul, up to a maximum target of `r ifelse(maxyr>2021, "100", "300")` specimens. For each fish in a length subsample, sex was determined and then the fork length or total length was measured to the nearest 1.0 cm. Unless retained for biological sampling by the International Pacific Halibut Commission (IPHC), Pacific halibut were measured to fork length upon capture `r ifelse(maxyr > 2020, "", "and 50% were randomly selected to receive a preopercle tag,")`, then immediately returned to the sea in an effort to reduce mortality. The weights of all Pacific halibut were estimated using an IPHC length-weight regression [@RN895].
...
`r NMFSReports::crossref(list_obj = list_tables, nickname = c("tab_specimen_samples_", "tab_oto_collection_scheme_"), sublist = "number", exact = FALSE)`) and shipped to the AFSC for age determination. Otolith samples were collected following `r str2 ` (Table `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_oto_collection_scheme_", sublist = "number", exact = FALSE)`). Otoliths were preserved in a glycerol-thymol solution. Individual fish weights and lengths were collected for each fish from which age structures were taken. For walleye pollock, the survey area was also divided into low-density and high-density age structure sampling strata based on historical population densities and an isobath of approximately 70 m.  

```{r stomach_collection}
temp <- list_tables$tab_stomach_collection_scheme$raw
```

Stomachs were collected in the field from `r xunits(nrow(temp))` fish species (Table `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_stomach_collection_scheme", sublist = "number") `) and were preserved in the field in 10% formalin. Arrowtooth flounder and Kamchatka flounder (*Atheresthes* spp.) stomachs were collected together as one genus because they occupy a similar trophic niche in the Bering Sea. 
...
Design-based estimates of biomass, population, and size structure of fishes and invertebrate species were calculated from EBS and NBS survey data. A brief description of the procedures used in the analysis of RACE Bering Sea survey data follows [@RN939]. Some species were grouped by family for catch data analysis because of their limited commercial value or an inability to identify to lower taxonomic level while in the field. 
