...
The `r text_list(vessel_info$vessel_ital)` conducted the standard EBS shelf survey charters. At the beginning of the survey, scientists boarded the vessels in Dutch Harbor, Alaska, then headed to eastern Bristol Bay to begin surveying.  From Bristol Bay, the survey then proceeded westward to the shelf edge (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `). The progression from east to west was established in response to movements of yellowfin sole and other species, which may be migrating eastward during the course of the survey [@RN928]. In the EBS, `r str$str[str$SRVY == "EBS"]` (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `).
...
> It has been suggested that this table is not as well formatted as it can be because there is so much empty space. Thoughts? 
