...
temp00 <- function(table_raw, yrs, maxyr, compareyr, unit, unt, y_long) {
...
b <- a$table_raw %>% 
  ...
  # dplyr::mutate(dplyr::across(dplyr::everything(), gsub, pattern = ",", replace = "")) %>% 
  # dplyr::mutate(dplyr::across(where(is.numeric),  # dplyr::starts_with("change_"), 
  #                               .fns = formatC, 
  #                               digits = ifelse(temp$divby>1, 0, 1), 
  #                               big.mark = ",", 
  #                               format = "f", 
  #                               drop0trailing = TRUE)) %>% 
  # dplyr::mutate(dplyr::across(dplyr::starts_with("change_"),
  #                               .fns = formatC,
...
for (i in which(colnames(b) %in% yrs)) {
  for (ii in 1:nrow(b)) {
    temp000 <- as.numeric(gsub(x = b[ii,i], pattern = ",", replacement = ""))
    temp00 <- find_units(dat = temp000)
    b[ii,i] <- paste0(formatC(x = temp000, #/temp$divby, 
                              digits = ifelse(temp00$divby>1, 0, 1), 
                              big.mark = ",", format = "f", 
                              drop0trailing = TRUE)#, 
                      # temp$unit_wrd
                      )
...
descr <- ggdraw() + draw_text(text = text0, 
                              fontface = 
                                # c('bold', "plain", "plain", "plain"), 
                                rep_len(x = c('bold', "plain", "plain", "plain"), length.out = length(text0)), 
...
a <- temp00(table_raw, yrs, maxyr, compareyr, unit, unt, y_long)
header <- a$header
figure <- a$figure
...
if (spp_plot_idw | spp_plot_pa) {
  ...
  height <- 3 #ifelse(SRVY == "NEBS", 5.5, 4.25) 
