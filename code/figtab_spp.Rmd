...
haul_cruises_maxyr$SRVY_long[haul_cruises_maxyr$SRVY_long == "eastern Bering Sea"] <- "eastern Bering Sea"
...
if (spp_plot_idw_coldwarm){
  ...
  for (i in 1:2) { # two cases, above and below
    ...
    figure <- plot_idw_xbyx(
      ...
      grid.cell = c(20000, 20000), #c(ifelse(workfaster, 20000, 0.02), # c(20000, 20000) # 20x20km grid
...
if (spp_plot_idw | spp_plot_pa){
  ...
  figure <- plot_idw_xbyx(
    ...
    dat = table_raw, # %>% 
        # dplyr::filter(!is.na(cpue_kgha)), # TOLEDO
    lat = "latitude",
    lon = "longitude",
    var = "cpue_kgha",
    year = "year",
    ...
    key.title = paste0(stringr::str_to_sentence(spp_print), "\nrelative biomass (kg/ha)"),
    grid.cell = c(20000, 20000), #c(ifelse(workfaster, 20000, 0.02), # c(20000, 20000) # 20x20km grid
              # ifelse(workfaster, 20000, 0.02)),
    set.breaks = set.breaks, 
    row0 = 1)

  } else if (spp_plot_pa) {
    header <- paste0("Maps of ",spp_print, ifelse(is.na(spp_sci), "", paste0(" (",spp_sci,")")), 
                     " presence from the ",
                     compareyr," (left) and ",maxyr,
                     " (right) EBS (eastern Bering Sea) and NBS (northern Bering Sea) bottom trawl surveys. Historically, not enough of this species is caught to calculate a distribution. ")
