...
if (sum(haul_cruises_maxyr$SRVY %in% "EBS")>0) {
  insert2 <- paste0(insert2, 
                    ...
                    ifelse(maxyr>2020, '(except 2020 due to the COVID-19 pandemic) ', ''),
                    'since 1982 and is the longest running timeseries for this kind of data in the region [@RN976]. ')
}
...
if (sum(haul_cruises_maxyr$SRVY %in% "NBS")>0) {
  temp2 <- cruises %>% 
    ...
    unlist()
  insert2 <- paste0(insert2,
                         'The NBS survey has only been conducted in ',NMFSReports::text_list(temp2),' [@RN909]. ')
}
