...
`r ifelse(refcontent, ifelse(SRVY %in% "NEBS", "## **NBS**", ""), "")`
...
`r ifelse(refcontent, ifelse(SRVY %in% "NEBS", "## **NEBS**", ""), "")`
...
```{r}
insert <- readtext(file = paste0(dir_out_rawdata, "/",spp_file,".docx"))
insert <- gsub(pattern = "\n", replacement = "\n\n\n", x = insert$text)
```
