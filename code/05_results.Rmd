...
Specimens collected during the `r maxyr` `r SURVEY` shelf trawl survey are shown in Table`r plural_surveys` \@ref(fig:tab-specimen-samples-ebs) and \@ref(fig:tab-specimen-samples-nbs). Other special collections are listed in Table \@ref(tab:tab-special-projects).
...
```{r results-spatial-distribution-inverts}
...
for (i in 1:nrow(temp0)) {
  a <- list.files(path = dir_out_figtab, 
                  pattern = paste0("fig-dist-",taxon0,"-",temp0$file_name[i], "-data"), 
                  ignore.case = TRUE)
  a <- a[!grepl(pattern = "-comm", x = a)]
  a <- a[grepl(pattern = ".rdata", x = a, ignore.case = TRUE)]
  
  temp <- c(temp, 
            paste0(
              temp0$print_name[i], 
              " (Fig. \\@ref(fig:fig-dist-",taxon0,"-",temp0$file_name[i],"-data)",
...
Plots of spatial distribution and tables of CPUE (kg/hectare and no./hectare) for the `r length(unique(report_spp1$file_name[report_spp1$taxon == "invert"]))` major invertebrate species caught during the `r SURVEY` continental shelf surveys, `r str0`, are presented below. Differences in estimates and totals may be due to rounding. 
