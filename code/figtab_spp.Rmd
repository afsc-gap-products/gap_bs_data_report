...
table_raw <- table_raw %>% 
  dplyr::bind_rows(yr_missing %>% dplyr::mutate(var = "biomass_mt")) %>% 
...
# }
  ...
  if (length(table_raw_mean$y_long) > 1) {
      # facet grid
    figure_print <- figure_print + 
...
temp2 <- ggdraw() +
  draw_plot(temp2) +
  draw_image(
    paste0(dir_img, spp_file, ".png"), width = 2, x = 1,
    hjust = 1, halign = 1, valign = 0
  )

# then combine with the top row for final plot
figure_print <- cowplot::plot_grid(temp1, temp2, nrow = 2, rel_heights = c(1,1))
figure_print <- ggdraw(add_sub(plot = figure_print, 
                               label = paste0(pres_stats$SRVY_long, " Biomass Mean: ", 
