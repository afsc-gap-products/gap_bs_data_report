...
header <- paste0("Contour map of surface temperatures from the ",maxyr," ",
                 NMFSReports::text_list(haul_cruises_maxyr$SRVY_long)," shelf bottom trawl surveys.")
  ...
  table_raw <- table_raw %>% 
    dplyr::select("group", "taxon", "999", "a", "CI95", "prop", 
      ...
      footnote0 <- paste0("Proportion of total estimated biomass, fish and invertebrates combined, for the total survey area = ", 
                      NMFSReports::xunits(total, words = FALSE), " t.")
  
  # xx <- 1:nrow(table_raw)
  # xx[!grepl(pattern = "Total ", x = table_raw$taxon)] <- NA
  # xx_group <- as.numeric(factor(x = table_raw$group, 
  #                               labels = 1:length(unique(table_raw$group)),
  #                               levels = unique(table_raw$group),
  #                               ordered = FALSE))
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0("Biomass estimates (t) for major invertebrate species and groups taken during the ", 
                   maxyr, " ", 
                   haul_cruises_maxyr$SRVY_long[i],
                   " shelf bottom trawl survey.")
