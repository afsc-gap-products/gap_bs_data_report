...
The purpose of the bottom trawl `r ifelse(SRVY=="NEBS", "surveys", "survey")`survey is to collect information about the fish and invertebrate populations and environmental conditions. In `r maxyr`, the RACE Division of NMFS' AFSC conducted `r text_list(paste0('the ', haul_cruises_maxyr$yrofsurvey, haul_cruises_maxyr$stndth, " " , haul_cruises_maxyr$SRVY," shelf bottom trawl survey"))`. `r insert2` `r insert`
...
In this document, we will compare the `r maxyr` `r NMFSReports::text_list(SRVY1)` survey results with those from the `r compareyr` `r NMFSReports::text_list(SRVY1)` survey [`r ref_compareyr`], which is the most recent and similar past, comparable survey. For survey results referenced from previous surveys herein, the reader should refer to the AFSC Technical Memoranda cited above.
