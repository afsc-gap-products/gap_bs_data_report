...
a <- specimen_maxyr %>% 
  ...
  table_raw0 <- 
    ...
    dplyr::left_join(x = ., 
                     y = report_spp1 %>% 
                       dplyr::select(species_code,  print_name, order) %>% 
                       dplyr::rename(common_name = print_name), 
                     by = "species_code") %>% 
    dplyr::select(-species_code) %>%
    dplyr::group_by(SRVY, common_name, order) %>% 
    dplyr::summarise_if(is.numeric, sum, na.rm = TRUE) %>% 
    dplyr::arrange(SRVY, order) 
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_print <- table_raw <- table_raw0
...
> are blank special project titles not to be included?
...
places <- data.frame(matrix(data = c("AFSC", 
                            "Alaska Fisheries Science Cener", 
                            "ADF&G", "Alaska Department of Fish & Game", 
                            "FMA", 
                            "Fisheries Monitoring & Assessment Division", 
                            "IPHC", 
                            "International Pacific Halibut Commission", 
                            "NWFSC", 
                            "Northwest Fiseries Science Center", 
                            "PMEL", 
                            ...
                            ncol = 2, 
                            byrow = TRUE)) 
names(places) <- c("agency", "agency_long")
...
```{r fig_bt_all_nebs_surveys_}
# nickname0 <- "fig_idw_bottemp_longterm_mean_"
nickname0 <- "fig_bt_all_nebs_surveys_"
...
> fix decimals for change and biomass of each year
...
header <- paste0("Total estimated biomass in metric tons (t) and the percent change from the ",compareyr," and ",maxyr," NBS (northern Bering Sea) bottom trawl surveys for invertebrate and fish taxa captured in greater than 50 trawl samples from both years combined.")
...
temp2 <- table_raw %>% 
  ...
  table_print <- dplyr::bind_cols(temp1, temp2) %>%
    # dplyr::select(-taxon) %>% 
    dplyr::mutate(across(starts_with("change"), formatC, big.mark=",", digits = 1, format = "f")) %>%
    # dplyr::select(-taxon) %>% 
    dplyr::mutate(across(starts_with("change"), paste0, "%")) %>%
    dplyr::mutate(across(where(is.numeric), formatC, big.mark=",", digits = 0, format = "f")) %>%
  ...
  table_print <- table_print %>%
    flextable::flextable(data = ., 
                         col_keys = c(ifelse(length(unique(table_print$SRVY)) == 1, "", "Survey"), 
                                                "Fish taxon", #"dummy", 
                                                as.character(yrs), "change", 
                                      "Invertebrate taxon", #"dummy_", 
                                                paste0(yrs, " "), "change ")) %>%
    # compose(j = "dummy", 
