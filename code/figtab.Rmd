...
```{r tab-pres-special-projects}
nickname <- "tab-pres-special-projects" 
...
```{r tab-pres-length-samples}

nickname <- paste0("tab-pres-length-samples") 
...
table_raw <- lengths_maxyr %>% # length data - from oracle
...
nickname <- paste0("tab-pres-age-samples") 
...
```{r fig-temperature-[st/bt], eval = FALSE}
...
## fig-pres-temperature-[bt/st]

```{r fig-pres-temperature-[bt/st]}
nickname <- "fig-pres-temperature-"
height <- 5
width <- full_page_portrait_width
yrs <- nbsyr
row0 <- 2
...
for (case in c("bottom", "surface")) {
  ...
  if (case == "surface") {
    raster_nebs <- terra::rast(coldpool::nbs_ebs_surface_temperature)
    raster_ebs <- terra::rast(coldpool::ebs_surface_temperature)
    colorbar_breaks <- c(-Inf, seq(0, 10, 2), Inf)
  } else if (case == "bottom") {
    raster_nebs <- terra::rast(coldpool::nbs_ebs_bottom_temperature)
    raster_ebs <- terra::rast(coldpool::ebs_bottom_temperature)
    # colorbar_breaks <- c(-Inf, -1, 0, 1, seq(2, 8, 2), Inf)
    colorbar_breaks <- c(-Inf, seq(-2, 8, 2), Inf)
  }
  ...
  nickname <- paste0("fig-pres-temperature-", ifelse(case=="bottom", "bt", "st"))
  ...
  key.title = ifelse(case == "bottom", 
                     ...
                     expression(bold("Surface temperature (°C)")))
  reg_dat = reg_dat0
  title0 = paste0(toupper(substr(x = SURVEY, start = 1, stop = 1)), 
                  substr(x = SURVEY, start = 2, stop = nchar(SURVEY)), " ", case, " temperature") 
...
# header <- paste0("Major taxa sampled in the northern Bering Sea bottom trawl survey for ", 
#                  text_list(nbsyr),
...
table_raw <- obj$raw
table_print <- obj$print # %>% 
# flextable::void(x = ., part = "all", ~ change) #  + dummy
...
```{r tab-biomass-pchange-pres, eval = FALSE}
...
table_raw <- 
  ...
  dplyr::select(-order) %>%
  dplyr::ungroup() %>% 
  dplyr::relocate(common_name, year, biomass_EBS, population_EBS, biomass_NBS, population_NBS)
...
table_print <- table_raw %>% 
  ...
  flextable::valign(x = ., valign = "top", part = "body", j = 3:6) %>%
