...
```{r res_spp_data_wrangle_{{jj}}, eval = FALSE }
...
biomass_cpue <- biomass %>%
  dplyr::filter(year %in% c(maxyr, compareyr) &
                  ...
                  stratum %in% 999) %>%
  dplyr::select(SRVY, year, biomass_mt, population, cpue_kgkm2_mean, cpue_nokm2_mean) %>%
  dplyr::mutate(print_name = spp_print) %>%
  dplyr::rename(cpue_kgha = cpue_kgkm2_mean,
...
if (spp_table_cpue) {
  tab_wt <- paste0("\\@ref(tab:tab-est-",spp_taxon,"-", spp_file, "-wt)")
  tab_no <- paste0("\\@ref(tab:tab-est-",spp_taxon,"-", spp_file, "-num)")
}
...
```{r res_spp_ebs_{{jj}}, results='asis', eval = FALSE}
tab0 <- str0 <- ""
...
```{r tab-env-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```
...
```{r tab-bio-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```
:::
...
```{r fig-sizecomp-{{jj}}}
nickname <- paste0("fig-sizecomp-",spp_taxon,"-", spp_file, ifelse(report_title == "data", "", "-comm"))
```
...
```{r fig-dist-{{jj}}}
nickname <- paste0("fig-dist-",spp_taxon,"-", spp_file, ifelse(report_title == "data", "", "-comm"))
```
...
```{r tab-est-{{jj}}-wt}
nickname <- paste0("tab-est-",spp_taxon,"-", spp_file, "-wt")
```
...
```{r tab-est-{{jj}}-num}
nickname <- paste0("tab-est-",spp_taxon,"-", spp_file, "-num")
```
