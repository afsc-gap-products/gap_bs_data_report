...
The `r maxyr` EBS shelf and NBS bottom trawl surveys were conducted aboard the chartered commercial stern-trawlers `r text_list(cruises_maxyr0$vessel_ital)` (Fig. \@ref(fig:fig-vessels)). `r nbs_insert ` After the NBS survey was completed, both vessels returned to Dutch Harbor to offload survey equipment and biological samples. The NBS shelf was divided into three strata: one including the area north of St. Lawrence Island and Norton Sound and two others south of St. Lawrence Island separated by the 50-m (`r round(50/divmforft, digits = 1)`-ft) isobath. 
...
```{r team_desc_insert_text, child=paste0(dir_out_rawdata, "/0_team_desc.Rmd")}
```
