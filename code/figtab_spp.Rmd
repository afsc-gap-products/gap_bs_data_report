...
agecomp0 <- agecomp |> 
  dplyr::filter(species_code %in% spp_code)
# take the last 3 years worth of data, whatever years they are
yrs <- yrs_age <- sort(unique(agecomp0$year)) #head(sort((unique(agecomp0$year)), decreasing = TRUE), 3)
# yrs <- yrs_age <- yrs[yrs > maxyr-10]
...
temp <- age_plot(srvy, yrs, agecomp = agecomp0, height, width, nickname, legend_font_size, report_title0)
for (jjj in 1:length(temp)) { assign(names(temp)[jjj], temp[[jjj]]) }
