...
```{r end-data-sources, child=paste0(dir_out_rawdata, "0-data-sources.qmd")}
```
...
```{r end-acknowledgments, child=paste0(dir_out_rawdata, "0-acknowledgements.qmd")}
```
...
```{r tab-glossary-child, child = paste0(dir_code, "_child_report_figtab.qmd"), eval = (report_title == "community")}
```
