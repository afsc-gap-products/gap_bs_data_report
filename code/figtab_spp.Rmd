  ...
  for (case_yr in case_yr0) {
    ...
    if (spp_plot_idw) {
      ...
      figure <- ggplot() +
        ...
        ggplot2::geom_stars(data = stars0,
                            na.rm = FALSE,
                            show.legend = TRUE) +
...
std_b <- officer::fp_border(width = 0, color = "grey10")
...
table_print <- table_print1 <- table_raw %>%
  ...
  flextable::colformat_image(j = "img" , width = 2, height = 2) %>% 
  flextable::merge_v(j = "img") %>% # part = "all", combine = TRUE, 
  flextable::padding(part = "all", j = 1, padding = 0) %>% 
  flextable::hline_top(x = ., border = std_b, j = 1, part = "all") %>% 
  flextable::hline_bottom(x = ., border = std_b, j = 1, part = "all") %>% 
