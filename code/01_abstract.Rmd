...
For the fifth consecutive year, average surface (7.6°C) and bottom (4.2 °C) water temperatures for the EBS shelf were well above the long-term means from 1982 to 2018 for the surface (6.6°C) and for the bottom (2.5°C). 

> below from NBS report, to replace above

Bottom temperature is a major environmental driver that can affect the distribution of fishes, crabs, and other invertebrates on the shelf (Figures `r crossref(list_obj = list_figures, nickname = 'fig_mean_temperature', sublist = 'number')` and `r crossref(list_obj = list_figures, nickname = 'fig_cold_pool_area', sublist = 'number')`). Using the long-term time series of bottom temperatures from the EBS shelf survey as reference, the years 2006-2013 were colder than average ("cold stanza") and the years 2002-2005 and 2014-2019 were warmer than average ("warm stanza"). During the `r haul_cruises_maxyr$yrofsurvey[haul_cruises_maxyr$SRVY == "EBS"] `-year time series (1982–`r maxyr`) of the annual EBS shelf bottom trawl survey, mean summer bottom temperatures were variable, ranging from `r round(min(temps_avg_yr$bt), digits = 1)`°C to `r round(max(temps_avg_yr$bt), digits = 1)`°C, with a grand mean for all years of `r round(temps_avg_yr$bt_mean[1], digits = 1)`°C (Figure `r crossref(list_obj = list_figures, nickname = 'fig_mean_temperature', sublist = 'number')`).
