...
#TOLEDO--check if this worked LINES 65-74
```{r ocean_temps}
...
if (temp$rank[temp$year == maxyr] < 6) {
  temp4a <- NMFSReports::numbers2words_th(temp$rank[temp$year == maxyr])
  temp4 <- paste0(ifelse(temp4a != "first", paste0(temp4a, " "), ""),
                  "lowest")
} 
if (abs(temp$rank[temp$year == maxyr]-length(temp$rank)) < 6) {
  temp4a <- NMFSReports::numbers2words_th(abs(temp$rank[temp$year == maxyr]-length(temp$rank)))
  temp4 <- paste0(ifelse(temp4a != "first", paste0(temp4a, " "), ""),
                  "highest")
}
...
A total of `r xunits(temp1$species_code) ` different fish species representing `r xunits(temp1$family_taxon)` families and `r xunits(temp1$genus_taxon)` genera were identified during the `r maxyr` `r NMFSReports::text_list(SRVY1)` survey`r ifelse(length(SRVY1)>1, "s", "")` (`r ifelse(SRVY == "NEBS", "appendices", "appendix")` `r NMFSReports::crossref(list_tables, "tab_app_taxa_encountered_*_fish", "number", exact = FALSE)`). In `r maxyr`, `r NMFSReports::text_list(paste0("the ", temp0$SRVY, " had a total of ", xunits(temp0$species_code), " taxa of which ", xunits(temp0$species_taxon), " were identified to the species level"))`. The remaining fish taxa in each survey area were identified to the genus level or higher. `r insert `
...
A total of `r xunits(temp1$species_code) ` different invertebrate taxa representing `r xunits(temp1$phylum_taxon)` phyla were identified during the `r maxyr` `r NMFSReports::text_list(SRVY1)` survey (`r ifelse(SRVY == "NEBS", "appendices", "appendix")` `r NMFSReports::crossref(list_tables, "tab_app_taxa_encountered_*_invertebrate", "number", exact = FALSE)`). In `r maxyr`, `r NMFSReports::text_list(paste0("the ", temp0$SRVY, " had a total of ", xunits(temp0$species_code), " taxa of which ", xunits(temp0$species_taxon), " were identified to the species level"))`. The remaining invertebrate taxa in each survey area were identified to the genus level or higher. The lack of species level identifications among invertebrates was due to a variety of factors that are outlined in @RN934. Additionally, trawl catchability of small invertebrates is not known.
...
An interactive map of CPUE by species for the `r maxyr` `r NMFSReports::text_list(SRVY1)` surveys, as well as for all other AFSC bottom trawls surveys, can be found at <https://apps-st.fisheries.noaa.gov/dismap/>. The CPUE data with associated station information including position, surface and bottom temperatures, and bottom depth can be downloaded from the Fisheries One Stop Shop <https://www.fisheries.noaa.gov/foss/f?p=215:200:1099772399154:Mail:NO:::>. There, users can interactively select, view, and download data for this and other surveys conducted by our team. 

### Spatial Distribution, Abundance, Size Compositions, Biomass, and Population Estimates of Selected Fish
...
Plots of the spatial distribution and tables of CPUE (kg/hectare and no./hectare) for `r length(NMFSReports::crossref(list_obj = list_figures, nickname = "fig_idw_cpue_fish_", sublist = "number", exact = FALSE, text = FALSE)) ` major fish species (Figures `r NMFSReports::crossref(list_obj = list_figures, nickname = "fig_idw_cpue_fish_", sublist = "number", exact = FALSE) ` and Tables `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_estimates_maxyr_fish_", sublist = "number", exact = FALSE)`) caught during the `r NMFSReports::text_list(SRVY1)` continental shelf are presented in the subsections below. Total abundance-at-length and mean length estimates for major fish species are also presented in the subsections below(Figures `r NMFSReports::crossref(list_obj = list_figures, nickname = "fig_sizecomp_fish_", sublist = "number", exact = FALSE) `).

Survey results for major fish taxa are presented in maps of geographic distribution and abundance, plots of total abundance-at-size, and tables with estimates of biomass and population number (Table `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_estimates_maxyr_spp_", sublist = "number", exact = FALSE)`). Species include `r str0`. Mean CPUE by weight and number, as well as maps of geographic distribution, for many other species are also included (Tables `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_majortaxa_pchange_", sublist = "number", exact = FALSE)` and Figures `r NMFSReports::crossref(list_obj = list_figures, nickname = "fig_idw_cpue_fish_", sublist = "number", exact = FALSE)`). Appendices `r NMFSReports::crossref(list_tables, "tab_app_principal_fish_", "number", exact = FALSE)` contain population estimates by sex and size class for all `r nrow(temp0)` of the fish species listed earlier in this paragraph. 
...
if (length(unique(temp$print_name))==1) {
  ...
  str0 <- paste0("The ",temp$print_name[1]," (*",temp$species_name1[1],"*) is common in the Bering ",
                 ...
                 " and it was the invertebrate taxon with the highest catch rate by weight in the " , 
