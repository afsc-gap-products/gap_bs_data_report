...
res <- knitr::knit_child(
  text = knitr::knit_expand(
    file = paste0(dir_code, "_child_save_fig.Rmd")), 
  quiet = TRUE
)
```
...
res <- knitr::knit_child(
  text = knitr::knit_expand(
    file = paste0(dir_code, "_child_save_tab.Rmd")), 
  quiet = TRUE
)
...
## fig-bt-temperature-ebs-[above or below]

```{r fig-bt-temperature-ebs-}
# nickname0 <- "fig_idw_bottemp_longterm_mean_"
nickname0 <- "fig-bt-temperature-ebs-"
...
for (i in 1:2) { # two cases, above and below
  ...
  header <- paste0("Eastern Bering Sea shelf bottom trawl survey near-bottom temperatures in years ",
                   case,
...
## fig-temperature-nebs-surveys-[st or bt]

```{r fig_[st or bt]-temperature-nebs-surveys}

if (SRVY == "NEBS") {
  
  for (i in 1:2) { # two cases, above and below
    case <- ifelse(i==1, "bottom", "surface")
    nickname <- paste0("fig-temperature-nebs-surveys-",
                       ifelse(i==1, "bt", "st"))
    
    subobj <- TRUE
    newobj <- ifelse(i==1, TRUE, FALSE)
    
    header <- paste0(stringr::str_to_title(case)," temperatures (°C) in the ",
                     text_list(haul_cruises_maxyr$SRVY_long),
                     " during the ",
                     text_list(nbsyr),
                     " surveys, which included the full northern Bering Sea shelf bottom trawl survey. ")
...
### tab-biomass-est-fish_ (EBS and NBS)

```{r tab-biomass-est-fish_}
...
### tab-biomass-est-invert\_ (EBS and NBS)

```{r tab-biomass-est-invert_}
