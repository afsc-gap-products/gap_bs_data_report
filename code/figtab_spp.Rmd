...
header <- spp_file
nickname <- paste0("fig-", spp_file, "-0-pres")
height <- width <- 5
...
if (report_title == "data") {
  ...
  nickname <- paste0("fig-", spp_file, "-3dist-pres")
  yrs <- sort(nbsyr)
  height <- 8
  width <- 8
  row0 <- 1
...
if (report_title == "data") {
  ...
  height <- full_page_portrait_height
  SRVY11 <- SRVY1 # "NBS"
  nickname <- paste0("fig-", spp_file, "-4sizecomp-pres")
}
...
temp00 <- function(table_raw0, 
  ...
  for (i in 1:nrow(b)){
    ...
    str00 <- c(paste0(b0$SRVY, ifelse(i == 1, paste0(" ", y_long), "")), 
                ...
                ifelse(b0$pchange0 %in% c("NA", "Inf"), 
                       "", 
                       paste0("\n(", b0$pchange0, "%)\n")), 
                "")
...
height <- 4
nickname <- paste0("fig-", spp_file, "-1biomass-pres")
table_raw <- table_raw0
...
unt = ""
height <- 4
nickname <- paste0("fig-", spp_file, "-2population-pres")
table_raw <- table_raw0
figure <- temp00(table_raw0 = table_raw0, 
