...
for (i in 1:nrow(temp)) {
  # if (nrow(temp)>1) {
  #   subobj <- ifelse(nrow(temp)>1, TRUE, FALSE)
  ...
  nickname <- paste0(nickname0, 
                     ...
                     gsub(pattern = " ", replacement = "", perl = TRUE, 
                          x = gsub(pattern = "F/V ", replacement = "", 
                                   x = temp$vessel_name[i], fixed = TRUE)))

  header <- paste0("Haul and catch data for successfully completed tows by ", temp$vessel_ital[i]," during the ", maxyr, " ", temp$SRVY_long[i], " bottom trawl survey.")
  ...
  dplyr::filter(!is.na(print_name)) %>% 
    ...
    tidyr::pivot_wider(names_from = print_name, 
                       values_from = weight, 
                       values_fill = "-") %>%
    dplyr::mutate(start_time = format(x = as.POSIXlt(x = start_time), format = "%Y-%m-%d"))  %>% # format = "%Y-%m-%d %H:%M:%S"
  dplyr::mutate(across(where(is.numeric), 
                       formatC, big.mark=",", digits = 1, format = "f")) %>% 
    dplyr::rename("Start date"# and time" 
                  = start_time,
                  "Haul number" = haul,
  ...
  table_print$row <- rownames(table_raw) 
  # table_raw <- table_raw[,1:15]
  ...
  table_print0 <- list()
  c_start <- 1
  ncol_per_page <- 14 # how many columns should go on each page
  # for (ii in 1:ceiling(ncol(table_print)/ncol_per_page)) {
  #   c_start <- c_start + 1
  #   if (ii == ceiling(ncol(table_print)/ncol_per_page)) {
  #     c_end <- ncol(table_print)
  #   } else {
  #     c_end <- c_start + ncol_per_page-1
  #   }
  #   table_print0$temp <- table_print[,c(1, c_start:c_end)] %>% 
  ...
  #   
  #   names(table_print0)[names(table_print0) == "temp"] <- paste0("sub_", ii)
  #   c_start <- c_end
  # }
  # 
  # table_print <-table_print0
  
  table_print <- #data.frame(t(
    table_print[,c(1:15)] %>% 
        flextable::flextable(data = .) %>%
        # flextable::rotate(x = ., rotation = "btlr", align = "center", part = "all") %>% 
        flextable::bold(x = ., j = "row") %>% 
        flextable::set_header_labels(., row = "") %>%
        theme_flextable_nmfstm(font = "Arial Narrow", row_lines = FALSE, 
                               body_size = 8, pad = 0, spacing = 0.9)
...
```{r tab_taxa_encountered_}
nickname0 <- "tab_taxa_encountered_" 
cnt_tables <- 0

temp <- tidyr::crossing(haul_cruises_maxyr %>% 
                        dplyr::select(SRVY_long, SRVY), 
                        data.frame(taxon = c("invertebrate", "fish"), 
...
for (i in 1:nrow(temp)) {
  ...
  header <- paste0(stringr::str_to_sentence(temp$taxon[i]), 
                   ...
                   " bottom trawl survey listed alphabetically by family.")
...
```{r tab_principal_fish_}
nickname0 <- "tab_principal_fish_" 
cnt_tables <- 0
...
table_print <- table_raw %>%
  ...
  theme_flextable_nmfstm(font = "Arial Narrow", row_lines = FALSE, body_size = 8, header_size = 9)
