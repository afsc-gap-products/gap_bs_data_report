...
list_tables1 <- data.frame(nickname0 = names(list_tables), 
                           nickname = 
                              gsub(pattern = "_", replacement = " ", 
                                   gsub(pattern = "fig_", replacement = "", x = names(list_tables), 
                                        fixed = TRUE), fixed = TRUE), 
                            header = as.character(lapply(list_tables, `[[`, 'header')), 
                            filename = paste0(dir_out_figures, lapply(list_tables, `[[`, 'filename'), '.png'))

list_tables1$title <- stringr::str_to_title(gsub(pattern = "idw ", replacement = "", x = 
                                                    gsub(pattern = "size dist ", replacement = "", 
                                                         x = list_tables1$nickname)))



# insert <- c()
# for (i in 1:nrow(list_tables1)){
#   insert <- paste0(insert, 
#                      paste(paste0("## ", list_tables1$title[i], 
#                      "\n\n",
#                      lapply(list_tables, `[[`, "print"),
#                      "\n\n", collpase = ""),
#               sep = "", collapse = ""))
# }
