...
table_print <- table_raw %>%
  ...
  flextable::as_flextable(., hide_grouplabel = TRUE)  %>% 
  flextable::bold(j = 1, i = ~ !is.na(SRVY_long) ) %>%
  flextable::width(j = "Agency", width = 2, unit = "in") %>%
  flextable::width(j = "Principal investigator", width = 3, unit = "in") %>%
  flextable::width(j = "Project title", width = 4, unit = "in") %>% 
...
temp1 <- function(table_raw, strat, total) {
  ...
  table_print <- table_print %>% 
    ...
    flextable::align(j = c("999", "prop"), # , "CI95", strat
                     align = "right", part = "all") %>%
