...
In `r maxyr`, the Resource Assessment and Conservation Engineering (RACE) division of the National Marine Fisheries Service’s (NMFS) Alaska Fisheries Science Center (AFSC) conducted the `r NMFSReports::text_list(paste0(temp$yrofsurvey, temp$stndth, " ", temp$survey_name, " (", temp$SRVY, ") from ", temp$start_mo_long, " to ", temp$end_mo_long), " ", maxyr)`. `r nbs_str0` `r stringr::str_to_sentence(NMFSReports::numbers2words(nrow(vessel_info)))` stern trawler`r ifelse(nrow(vessel_info)>1, "s", "")`, the `r NMFSReports::text_list(paste0(vessel_info$length_m, "-m ",  vessel_info$vessel_ital)) `, were chartered for `r ifelse(SRVY=="NEBS", "these surveys", "the survey")`. Demersal populations of fishes and invertebrates were sampled by trawling for 30 minutes at stations arranged on a systematic grid consisting of a total of `r NMFSReports::text_list(paste0(haul_cruises_maxyr$stations_avail, " stations in the ", haul_cruises_maxyr$SRVY))`. At each station, species composition, length distribution, and age structure samples were collected from ecologically and commercially important species. `r str0 `. 
                 ...
                 ifelse(temp$warmest_rank<(nrow(temps_avg_yr)-6), 
                        ...
                        paste0(ifelse((nrow(temps_avg_yr)-temp$warmest_rank) == 1, "", 
                               paste0(NMFSReports::numbers2words_th(nrow(temps_avg_yr)-
                                                               temp$warmest_rank), " ")), 
                        "coldest")), 
                             " observed since the beginning the EBS shelf bottom trawl survey timeseries in 1982. ")
}
