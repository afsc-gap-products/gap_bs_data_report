...
Temperature and depth profiles were recorded using a Sea-Bird SBE-39 temperature-depth recorder (Sea-Bird Electronics Inc., Bellevue, WA) attached to the headrope of the trawl. Observations were made at 3-second intervals at each station. Average bottom depth was calculated by adding the average net height to the average depth of the headrope while the net was in contact with the sea floor. 
...
for (i in 1:length(unique(haul_cruises_maxyr$SRVY))){
  ...
  if (length(unique(cruises_maxyr0$start_date_haul)) != 1 &
    ...
    temp <- paste0(temp, "the survey started for ", 
...
load(file = paste0(dir_out_figtab, "tab-oto-collection-scheme-EBS.Rdata")) # creates object called 'obj'
```
...
load(file = paste0(dir_out_figtab, "tab-oto-collection-scheme-EBS.Rdata"))
obj_ebs<-obj
load(file = paste0(dir_out_figtab, "tab-oto-collection-scheme-NBS.Rdata"))
obj_nbs<-obj
...
Stomachs were collected in the field from `r xunits(nrow(obj$raw))` fish species and were preserved in the field in 10% formalin (Table \@ref(tab:tab-stomach-collection-scheme)). Arrowtooth flounder and Kamchatka flounder (*Atheresthes* spp.) stomachs were collected as one genus because they occupy a similar trophic niche in the Bering Sea. 
