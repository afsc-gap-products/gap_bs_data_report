...
for (case in c("bottom", "surface")) {
  for (case_yr in c(1,2)) {
    ...
    if (case == "surface") {
      raster_nebs <- terra::unwrap(coldpool::nbs_ebs_surface_temperature)
      raster_ebs <- terra::unwrap(coldpool::ebs_surface_temperature)
    } else if (case == "bottom") {
      raster_nebs <- terra::unwrap(coldpool::nbs_ebs_bottom_temperature)
      raster_ebs <- terra::unwrap(coldpool::ebs_bottom_temperature)
    }
    
    raster_nebs <- terra::subset(raster_nebs, (names(raster_nebs) %in% yrs))
    raster_ebs <- terra::subset(raster_ebs, (names(raster_ebs) %in% yrs))
    raster_ebs <- terra::subset(raster_ebs, !(names(raster_ebs) %in% names(raster_nebs)))
    yr_names <- c(names(raster_nebs), names(raster_ebs))
    
    terra::ext(raster_ebs) <- terra::ext(raster_nebs)
    crs(raster_ebs) <- crs(raster_nebs)
    
    # dim(raster_ebs) <- dim(raster_nebs)
    # res(raster_ebs) <- res(raster_nebs)
    # raster_ <- merge(raster_nebs, raster_ebs)
    raster_ <- c(raster_nebs, raster_ebs)
    names(raster_) <- yr_names
    raster_ <- sort(raster_)

    library(tidyterra)
    
    ggplot2::ggplot() +
      tidyterra::geom_spatraster(data = raster_) +
      ggplot2::facet_wrap(~lyr, ncol = 3)
    ...
    # rasterbrick <- c(sebs_temp, nbs_no_2018)
    
    temp_2020 <- rasterbrick[[1]]
    names(temp_2020) <- "2020"
    terra::values(temp_2020[[1]]) <- NA
    rasterbrick <- c(rasterbrick, temp_2020)
...
table_raw0 <- 
  dplyr::left_join(
    ...
    x = report_spp1 %>% 
      dplyr::select(print_name, species_name = species_name1, species_code, taxon, table_bio_portion), 
    relationship = "many-to-many",
    by = c("species_code")) %>%
  dplyr::filter(table_bio_portion & 
                !is.na(print_name) & 
                  !is.na(year)) %>%
