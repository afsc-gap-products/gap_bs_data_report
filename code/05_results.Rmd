...
temp_insert <- paste0(paste0(
  "The mean sea surface temperature for the ", 
  temps_wt_avg_yr_longterm$SRVY, 
  " was ",
  round(x = temp$st_wt, digits = 1),
  "° C (Fig. 4), which was ",
  round(x = temp$st_wt_1_diff, digits = 1),
  "° C lower than ",maxyr-1," and ",
  round(x = temp$st_wt_mean_diff, digits = 1),
  "° C above the time-series mean (",
  round(x = max(temp$st_wt_mean), digits = 1),
  "° C)."))
  
  # " In the EBS south of 60° N, surface temperatures increased from east to west across the shelf, and in the NBS they were generally higher and above 10° C in Norton Sound (Fig. 5).")

temp <- temps_wt_avg_strat %>% 
  dplyr::filter(year == maxyr)
```

> so different!!! Sea surface temperatures recorded during the `r maxyr ` `r SRVY` survey ranged from `r round(x = min(temp$st_wt_stratum), digits = 1)`° to `r round(x = max(temp$st_wt_stratum), digits = 1)`° C

> should it be `r `round(x = min(haul$surface_temperature, na.rm = TRUE), digits = 1)` ???

Sea surface temperatures recorded during the `r maxyr ` `r SRVY` survey ranged from `r round(x = min(haul$surface_temperature, na.rm = TRUE), digits = 1)`° to `r round(x = max(haul$surface_temperature, na.rm = TRUE), digits = 1)`°C and bottom temperatures ranged from `r round(x = min(haul$gear_temperature, na.rm = TRUE), digits = 1)`° to `r round(x = max(haul$gear_temperature, na.rm = TRUE), digits = 1)`° C. The mean sea surface temperature for the EBS alone was 7.8° C (Fig. 4), which was 1.6° C lower than 2016 and 1.2° C above the time-series mean (6.6° C). In the EBS south of 60° N, surface temperatures increased from east to west across the shelf, and in the NBS they were generally higher and above 10° C in Norton Sound (Fig. 5).
