...
## Biomass of Major Taxa and Taxonomic Groups

> #TOLEDO finish this section for EBS-only reports
...
for (i in 1:length(SRVY1)) {
  ...
  temp <- dplyr::bind_rows(temp, temp1)
}
...
if (SRVY == "NEBS") { # TOLEDO
  str0 <- paste0("Total demersal animal biomass for the EBS was estimated at 16.5 million metric tons (t) and for the NBS at 4.6 million t. ")
}

str0 <- paste0(str0, 
               "In the EBS, the proportion of fishes (75%; Table ", 
               crossref(list_obj = list_tables, nickname = "tab_biomass_est_fish_EBS", sublist = "number"), 
               ") was three times higher than invertebrates (25%; Table ", 
               crossref(list_obj = list_tables, nickname = "tab_biomass_est_invert_EBS", sublist = "number"),
               "). ")
               ...
               # ifelse(SRVY == "NEBS",
                      str0 <- paste0(str0, 
               paste0(ifelse(temp$prop[temp$SRVY=="EBS"]>temp$prop[temp$SRVY=="NBS"], 
...
for (i in 1:length(SRVY1)) {
  ...
  temp2 <- list_tables[names(list_tables) ==
                         paste0("tab_biomass_est_fish_", SRVY1[i])][[1]]$raw %>%
      dplyr::mutate(SRVY = SRVY1[i]) %>%
    dplyr::filter(group %in% temp$group[1:2] & 
                    !grepl(pattern = "Total ", x = taxon, ignore.case = TRUE )) %>% 
...
## Relative Biomass and Abundance Catch per Unit Effort
