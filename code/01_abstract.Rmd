...
# TOLEDO
# define stanzas by defining years of consequative cold and warm years

# Using the long-term time series of bottom temperatures from the EBS shelf survey as reference, the years `r NMFSReports::text_list(sort(temps_avg_yr_abovebelow$below)) ` were colder than average ("cold stanza") and the years `r NMFSReports::text_list(sort(temps_avg_yr_abovebelow$above)) ` were warmer than average ("warm stanza"). 


# temp <- temps_avg_yr %>% 
#       dplyr::filter(SRVY == "EBS" #& bt_above_mean == TRUE
#                     ) %>% 
#   dplyr::select(year, bt_above_mean)
# 
...
> below from NBS report, to replace above
...
nbs_insert <- paste0(
  "Between ", compareyr, " and ", maxyr, "the total CPUE values for many of the animals in the NBS shifted from being low in ", compareyr, " to being much higher in ", maxyr, ". The total estimated animal biomass in the NBS increased from ", xunits(sum(biomass_compareyr$biomass[biomass_compareyr$SRVY == "NBS"])) ," t in ", compareyr, " to ", xunits(sum(biomass_maxyr$biomass[biomass_maxyr$SRVY == "NBS"]))," t in ", maxyr)
