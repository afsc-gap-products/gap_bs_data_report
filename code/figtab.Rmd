...
# header <- paste0("Map of the Bering Sea survey stations sampled in ",maxyr," during the EBS and NBS survey. The area enclosed within the light gray line contains the EBS shelf stations that have been sampled annually since 1982, whereas the area outlined in the dark gray line are the NBS stations that were sampled in ",maxyr,". The dots within each area indicate each station location.") # TOLEDO
  ...
  height <- full_page_portrait_height # height <- 6
...
header <- paste0("Stratum areas and sampling densities for the ",maxyr,
                 " ", SRVY ," bottom trawl survey. These stratum areas were calculated in ", 
                 stratum_info$year[1] , ". ")
...
table_print <- flextable::flextable(
  ...
  flextable::width(x = ., j = "stratum", width = 2.5) %>% 
  ...
  flextable::bold(x = ., j = 1, i = ~ grepl(pattern = "Total", x = table_raw$Region) ) %>% #!is.na(Region) ) %>%
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_print <- table_print %>% 
    ...
    flextable::width(x = ., j = "common_name", width = 4, unit = "in") %>% 
...
table_print <- table_raw %>%
  ...
  flextable::footnote(x = ., value = as_paragraph(footnote0), 
                      part = "header", j = "Agency")  %>%
  NMFSReports::theme_flextable_nmfstm(x = ., 
                                      font0 = font0, 
                                      row_lines = TRUE, 
                                      pgwidth = full_page_portrait_width) %>% 
...
for (i in 1:2) { # two cases, above and below
  ...
  rasterbrick <- raster::subset(x = coldpool:::ebs_bottom_temperature,
                                subset = paste0("sebs_ste_", year, "_gear_temperature"))
  ...
  figure <- plot_temps_facet(
    ...
    colorbar_breaks = c(-Inf, seq(from = 0, to = 14, by = 2), Inf),
    viridis_palette_option = "H", 
    row = row, # ifelse(length(year)<10, 3, 4), 
...
if (SRVY == "NEBS") {
  ...
  header <- paste0("Map showing ",case," temperatures (°C) in the NBS and EBS during the ",
...
temp1 <- function(table_raw, strat, total) {
  ...
  footnote0 <- paste0("Proportion of total estimated biomass, fish and invertebrates combined, for the total survey area = ", 
                      formatC(x = total, digits = 0, format = "f", big.mark = ","), " mt.")
  
  # table_print <- table_raw
  
  # table_print[is.na(table_print) | table_print %in% 0] <- "-"
  ...
  table_print <- table_print %>%
    ...
    # flextable::width(x = ., # j = c(999, strat), 
    #                  width = 2.5, unit = "in") %>%
    flextable::width(x = ., #j = c("999"), 
                     width = 1.5, unit = "in") %>%
    flextable::width(x = ., j = c("999"), 
                     width = 1.5, unit = "in") %>%
...
#     NMFSReports::theme_flextable_nmfstm(font0 = font0, row_lines = FALSE)  %>% 
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0(
    ...
    compareyr," and ",maxyr," ", 
    haul_cruises_maxyr$SRVY[i], 
    " Shelf Bottom Trawl Surveys for predominant invertebrate and fish taxa.")
  ...
  table_print <- table_print %>% 
    ...
    # dplyr::select(`Fish taxon`, dplyr::all_of(c(maxyr, compareyr, paste0(maxyr, " "), paste0(compareyr, " "))), change, 
    #               `Invertebrate taxon`, `change `) %>% 
    dplyr::select(dplyr::all_of(c("Fish taxon", paste0(maxyr, " "), paste0(compareyr, " "), "change ",
                                  "Invertebrate taxon", maxyr, compareyr, "change")))
  
      # dplyr::select(`Fish taxon`, dplyr::all_of(c(paste0(maxyr, " "), paste0(compareyr, " "))), `change `, 
      #             `Invertebrate taxon`, dplyr::all_of(c(maxyr, compareyr)), change)
  ...
  table_print <- table_print %>%
    ...
    NMFSReports::theme_flextable_nmfstm(x = ., 
                                        ...
                                        row_lines = FALSE, 
                                        pgwidth = full_page_landscape_width) %>% 
    flextable::vline(x = ., part = "all", j = 5) %>%
  ...
  if (i == 1){ # CPUE (WT/HA) and BIOMASS
    ...
    table_print <- table_print %>%
      ...
      NMFSReports::theme_flextable_nmfstm(x = ., 
                                          ...
                                          row_lines = TRUE, 
                                          pgwidth = full_page_landscape_width) %>%
      flextable::align(x = ., align = "right", part = "body") %>%
      flextable::align(x = ., j = c("print_name"), 
    ...
    varpop0 <- find_units(unit = "", unt = "", dat = table_raw$varpop)
    ...
    table_print <- table_print %>%
      ...
      flextable::set_header_labels(
        ...
        population = paste0("Estimated\nPopulation", population0$unit_word), 
        varpop = paste0("SD Population", varpop0$unit_word), 
        lowerp = paste0("95% LCL", population0$unit_word), 
        upperp = paste0("95% UCL", population0$unit_word), 
        # haulcount = "Total hauls", 
        catcount = "Hauls with weights", 
        numcount = "Hauls with counts", 
        lencount = "Hauls with lengths") %>% 
      ...
      flextable::merge_v(x = ., 
                         ...
                         part = "body") %>% 
      flextable::valign(valign = "top") %>%
      NMFSReports::theme_flextable_nmfstm(x = ., font0 = font0, 
                             row_lines = TRUE, 
                             pgwidth = full_page_landscape_width) %>%
      flextable::align(x = ., align = "right", part = "body") %>%
      flextable::align(x = ., j = c("print_name"), 
