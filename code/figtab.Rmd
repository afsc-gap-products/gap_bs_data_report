...
figure <- ggplot() + 
  ...
  scale_y_continuous(name = "Proportion of EBS Shelf Survey Area",
                     ...
                     breaks = seq(0,1,0.1)) +
  scale_x_continuous(name = "Year", 
                     limits = c(1982, maxyr + 0.5),
                     expand = c(0, 0),
                     breaks = seq(1982, maxyr, 4)) +
  theme_bw() +
...
z <- catch_haul_cruises_maxyr %>%
  tidyr::complete(species_code, nesting(#cruise, 
                                 haul, #vessel, 
                                 year, hauljoin, stratum, stationid, distance_fished, net_width)) %>%
  dplyr::select(cruise, hauljoin, haul, #vessel, 
...
catch_ebs_with_zeros <- 
  dplyr::full_join(x = catch_haul_cruises_maxyr, 
                   y = z, 
                   by = c("cruise", "hauljoin", "haul", #"vessel", 
...
biomass_ebs_by_stratum <- cpue_ebs_by_stratum %>%
  mutate(biomass_mt = cpue_avg_by_stratum * (area * 0.1))  %>%
...
temp <- function(dat){
  ...
  ## find estiated total biomass by stratum for group complexes (e.g., groups with more than one line)
  biomass_group_totals <- dat %>%
    dplyr::filter(grepl(pattern = "_", x = group0)) %>% 
    dplyr:: group_by(strata, group) %>%
    dplyr::summarise(biomass_mt = sum(biomass_mt)) %>% 
  ...
  biomass_group_strata <- dat %>%
    dplyr::group_by(strata, group, taxon) %>%
    dplyr::summarise(biomass_mt = sum(biomass_mt))
  ...
  out <- dplyr::bind_rows(biomass_group_strata, 
    ...
    dplyr::arrange(group, taxon)  %>% 
    dplyr::ungroup() %>%
    dplyr::mutate(CI95 = 0, 
                  a = "±", 
                  prop = `999`/sum(biomass_totals$`999`, na.rm=TRUE)) %>% 
    dplyr::relocate(group, taxon, `999`, a, CI95, prop)
  
  return(out)
  
}

  temp1 <- function(table_raw, strat) {
  ...
  table_print <- table_raw %>% 
    dplyr::mutate(prop = ifelse(prop < 0.0001, "<0.0001", 
                                as.character(round(x = prop, digits = 4))), 
  ...
  table_print <- table_print %>%
    ...
    # flextable::bold(x = ., i = ~ (grep(pattern = "Total ", x = taxon) )) %>%
    flextable::set_header_labels(.,
                                 "group" = "", 
                                 "taxon" = "", 
                                 "999" = "", 
...
fish_groups <- biomass_ebs_by_stratum %>%
  ...
  dplyr::mutate(group0 = case_when(
    ...
    species_code >= 30000 & species_code <= 31550 & 
      species_code != 30060 ~ "other rockfish",
    TRUE ~ "other fish"))

table_raw <- temp(dat = fish_groups)
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  footnote0 <- paste0("Proportion of total estimated biomass, fish and invertebrates combined, for the total survey area = ", NMFSReports::xunits(sum(biomass_ebs_by_stratum$biomass_mt[biomass_ebs_by_stratum$stratum %in% strat], na.rm = TRUE))," t.")
  
  a <- temp1(table_raw, strat)
...
invert_groups <- biomass_ebs_by_stratum %>%
...
table_raw <- temp(dat = invert_groups)
