...
table_print <- table_raw %>%
  ...
  flextable::set_header_labels(area_name = "",
                               stratum = "Stratum",
  ...
  flextable::width(width = c(.5, .5, 1, 1, 1, 1.5)) %>% 
...
table_raw <- readxl::read_excel(path = paste0(dir_out_rawdata, "/special-projects.xlsx"), 
  ...
  dplyr::mutate(
    ...
    SRVY_long = factor(x = SRVY_long, 
                       levels = c("Eastern and northern Bering Sea Combined", "Eastern Bering Sea", "Northern Bering Sea"), 
                       labels = c("Eastern and northern Bering Sea Combined", "Eastern Bering Sea", "Northern Bering Sea"), 
                       ordered = TRUE)) %>%
...
table_raw0 <- biomass  %>% 
  dplyr::filter(year == maxyr &
                  ...
                  group %in% unlist(report_spp %>% 
                                      dplyr::filter(table_est) %>% #  & group == taxon0
...
for (i in 1:nrow(comb)) {
  ...
  strat <- names(table_raw)[7:ncol(table_raw)]
  
  # Make table_print -------------------------------------------------------------
  
  
  footnote0 <- paste0("The ", total$SRVY_long, " bottom trawl survey total estimated ", text_list(total$taxon_long)," biomass is ", 
  ...
  table_print <- table_raw %>%
    ...
    flextable::add_header_row(top = TRUE,
                              values = c("Taxon",
                                         ...
                                         paste0("Proportion of total ",
                                                ifelse(comb$taxon[i] =="invert", "invertebrate", "fish"),
                                                " biomass"),
                                         "Estimated biomass by stratum (t)"),
                              colwidths = c(2,3,1,(length(strat)))) %>%
    flextable::width(width = ifelse((length(strat))>3, 0.7, 1.25),
                     unit = "in") %>%
    flextable::width(j = "prop", 
                     width = 0.8, unit = "in") %>%
    flextable::width(j = c("a"), 
                     width = 0.25, unit = "in") %>%   
    flextable::width(j = c("group"), 
                     width = 1, unit = "in") %>%    
    flextable::width(j = c("taxon"), 
    ...
    flextable::merge_at(j = c("999", "a", "CI95"), part = "header") %>% 
    flextable::merge_at(j = c("prop"), part = "header") 
