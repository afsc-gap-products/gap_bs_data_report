...
# places$agency
#   
#   tidyr::separate_wider_delim(Agency, delim = " & ", names = c("a", "b"))
  ...
  for (case_yr in c(1,2)) {
    ...
    if (case == "surface") {
      ...
      sebs_temp <- raster::subset(coldpool::ebs_surface_temperature,
                                  subset = sebs_only) %>%
        # raster::projectRaster(crs = crs(coldpool::nbs_ebs_surface_temperature))
              raster::projectRaster(raster::projectExtent(nbs_no_2018, crs = "EPSG:3338"))
...
if (SRVY == "NEBS"){
  ...
  table_raw <- catch_haul_cruises_maxyr %>% 
    ...
    dplyr::mutate(where = dplyr::case_when(
      !is.na(EBS) & !is.na(NBS) ~ "Encountered in EBS and NBS", 
      is.na(NBS) & !is.na(EBS)~ "Encountered in EBS but not in NBS", 
      is.na(EBS) & !is.na(NBS) ~ "Encountered in NBS but not in EBS")) %>% 
    dplyr::left_join(x = ., 
                     y = spp_info, #%>% 
  ...
  temp1 <- temp %>% 
    dplyr::filter(where == "Encountered in EBS but not in NBS" & 
                    !is.na(common_name)) %>% 
    dplyr::select(-where, -NBS) %>% 
    dplyr::rename(above_60 = EBS)
  
  temp2 <- temp %>% 
    dplyr::filter(where == "Encountered in NBS but not in EBS" & 
                    !is.na(common_name)) %>% 
