...
`r ifelse(refcontent & spp_text, "*REFERENCE MATERIAL FOR WRITERS - START*", "")`
...
# biomass %>% 
#   dplyr::filter(year %in% c(2021, 2022) & grepl(pattern = "snailfish", x = common_name) & SRVY == "NBS") %>% 
#   dplyr::select(year, common_name, catcount) %>% 
#   dplyr::group_by(year, common_name) %>% 
#   dplyr::summarise(freq = sum(catcount)) %>% 
#   dplyr::arrange(desc(freq)) %>% 
#   tidyr::pivot_wider(data = ., id_cols = c("common_name"), names_from = "year", values_from = freq)
...
if (show) {
    ...
    biomass_cpue_tab_name = c("tab_majortaxa_pchange_NBS")
  }
temp<-species_content(
...
# if (spp_text) {
  ...
  # tab_majortaxa_pchange_EBS <- "\@ref(tab:tab-majortaxa-pchange-EBS)"
...
land_start <- ifelse(report_title == "data", "<!---BLOCK_LANDSCAPE_START--->", "")
land_end <- ifelse(report_title == "data", "<!---BLOCK_LANDSCAPE_STOP--->", "")
...
# 
# # Main text
# fig_bot_temp_map <- "\@ref(fig:fig-bt-all-nebs-surveys)"
# 
# fig_surf_temp_map <- "\@ref(fig:fig-st-all-nebs-surveys)"
# 
# fig_both_temp_maps <- "\@ref(fig:fig-bt-all-nebs-surveys) and \@ref(fig:fig-st-all-nebs-surveys)"
# 
# tab_majortaxa_pchange <- "\@ref(tab:tab-majortaxa-pchange)"
# 
# # Species specific
# if (spp_plot_sizecomp) {
...
# 
# if (spp_plot_idw) {
#   fig_idw <- paste0("\@ref(fig:fig-idw-cpue-",spp_taxon,"-", spp_file, ")")
# }
# 
# land_start <- ""
# land_end <- ""
...
```{r spp_text-{{jj}}, child=paste0(dir_out_rawdata, spp_file,".Rmd"), eval=spp_text}
```

```{r fig-idw-cpue-{{jj}}}
nickname <- paste0("fig-idw-cpue-",spp_taxon,"-", spp_file)
```

```{r fig-idw-cpue-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), eval=ifelse(spp_plot_idw | spp_plot_idw_coldwarm, TRUE, FALSE)}
