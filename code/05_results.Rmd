...
str0 <- paste0(
  ...
  " than the time series mean (", round(x = temp$st_mean, digits = 1),"°C; Figs. \\\\@ref(fig:fig-mean-temperature), \\\\@ref(fig:fig-cold-pool-area), \\\\@ref(fig:fig-bt-temperature-ebs-below), and \\\\@ref(fig:fig-bt-temperature-ebs-above)", 
  ifelse(SRVY == "NEBS", 
         paste0(
           ifelse(a$coefficients[2]<0 & a$coefficients[8] <= 0.05, 
                  paste0("In the EBS south of 60°N, surface temperatures increased from east to west across the shelf (Fig. \\\\@ref(fig:fig-temperature-nebs-surveys-bt)). "), ""),
           "An average of ", 
           round(x = mean(temp2$surface_temperature), digits = 1),
           "°C was observed in Norton Sound (Fig. \\\\@ref(fig:fig-temperature-nebs-surveys-bt))."), 
         ""))
...
```{r fig-bt-temperature-ebs-}
nickname <- "fig-bt-temperature-ebs-"
```

```{r fig-bt-temperature-ebs--child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```

```{r temperature-nebs-surveys}
nickname <- "fig-temperature-nebs-surveys-"
```

```{r temperature-nebs-surveys-child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```
...
temp0 <- function(cols, SRVY, report_spp1) {
  ...
  load(file = paste0(dir_out_tables, "tab-specimen-samples-EBS.Rdata"))
  obj_ebs <- obj
  
  if (SRVY == "NEBS") {
    
    load(file = paste0(dir_out_tables, "tab-specimen-samples-NBS.Rdata"))
    obj_nbs <- obj
...
Specimens collected during the `r text_list(SRVY1)` shelf trawl survey are shown in Table`r ifelse(SRVY == "NEBS", "s", "")` \@ref(fig:tab-specimen-samples-ebs) and \@ref(fig:tab-specimen-samples-nbs). `r str0` Other special collections are listed in Table \@ref(tab:tab-special-projects).
...
if (SRVY == "NEBS") {
  ...
  load(file = paste0(dir_out_tables, "tab-species-composition.rdata"))
  temp3 <- obj$raw %>%
    dplyr::left_join(x = .,
                     y = spp_info %>%
  ...
  insert <- paste0( 
    ...
    " species were present in the NBS but absent in EBS (Table \\\\@ref(tab:tab-species-composition)). ")
...
A total of `r xunits(temp1$species_code)` different fish species representing `r xunits(temp1$family_taxon)` families and `r xunits(temp1$genus_taxon)` genera were identified during the `r maxyr` `r text_list(SRVY1)` survey`r ifelse(length(SRVY1)>1, "s", "")` (Table`r ifelse(SRVY == "NEBS", "s", "")` \@ref(tab:tab-app-taxa-encountered-EBS-fish)`r ifelse(SRVY == "NEBS", "and \\\\@ref(tab:tab-app-taxa-encountered-NBS-fish)", "")`). In `r maxyr`, `r text_list(paste0("the ", temp0$SRVY, " survey recorded ", xunits(temp0$species_code), " total taxa, of which ", xunits(temp0$species_taxon), " were identified to the species-level"))`. The remaining fish taxa in each survey area were identified to the genus level or higher. `r insert`
...
`r stringr::str_to_sentence(numbers2words(temp1$species_code))` different invertebrate taxa representing `r xunits(temp1$phylum_taxon)` phyla were identified during the `r maxyr` `r text_list(SRVY1)` survey (Table`r ifelse(SRVY == "NEBS", "s", "")` \@ref(tab:tab-app-taxa-encountered-EBS-invertebrate)`r ifelse(SRVY == "NEBS", "and \\\\@ref(tab:tab-app-taxa-encountered-NBS-invertebrate)", "")`). In `r maxyr`, `r text_list(paste0("the ", temp0$SRVY, " survey recorded a total of ", xunits(temp0$species_code), " taxa of which ", xunits(temp0$species_taxon), " were identified to the species-level"))`. The remaining invertebrate taxa `r ifelse(SRVY == "NEBS", "in each survey area ", "")`were identified to the genus level or higher. The lack of species-level identifications among invertebrates was due to a variety of factors that are outlined in @RN934 and @Stevensonetal2016. Additionally, trawl catchability of small invertebrates is not known.
...
for (i in 1:nrow(temp0)) {
  load(file = paste0(dir_out_tables, "tab-biomass-est-", temp0$type[i],"-", temp0$SRVY[i], ".rdata"))
...
for (ii in 1:length(temp2$SRVY)) {
  ...
  str00 <- paste0(str00, 
                  ...
                  paste0(temp2$SRVY[ii], 
                         ...
                         temp2$perc_fish[ii], 
                         "; Table \\\\@ref(tab:tab-biomass-est-fish-", temp2$SRVY[ii], 
                         ")) was ", str000,  
                         ifelse(temp2$prop_fish[ii]>temp2$prop_invert[ii], "higher","lower"),
                         " than invertebrates (", 
                         temp2$perc_invert[ii], 
                         "; Table \\\\@ref(tab:tab-biomass-est-invert-", temp2$SRVY[ii], 
                         "))"))
}
...
if (SRVY == "NEBS") {
  str0 <- paste0(str0, 
                 paste0(ifelse(temp1$prop[temp1$SRVY=="EBS"]>temp1$prop[temp1$SRVY=="NBS"], 
                        ...
                        " with results of a broader analysis of all survey years in @RN936 showing decreasing fish biomass with increasing latitude on the Bering Sea continental shelf (Tables \\\\@ref(tab:tab-biomass-est-fish-EBS) and \\\\@ref(tab:tab-biomass-est-fish-NBS)). This trend may change over time with warming ocean temperatures.", 
                        "") ) 
}
...
if (SRVY == "NEBS") {
  ...
  for (i in 1:length(SRVY1)) {
    load(file = paste0(dir_out_tables, "tab-biomass-est-fish-", temp0$SRVY[i], ".rdata"))
    temp1 <- obj$raw %>%
  ...
  for (i in 1:length(SRVY1)){
    ...
    temp3 <- paste0(temp3, 
                    paste0("In the ", SRVY1[i], ", ", 
                           ...
                           "; Tables \\\\@ref(tab:tab-biomass-est-fish-EBS) and \\\\@ref(tab:tab-biomass-est-fish-NBS)). "))
  }
  ...
  str0 <- paste0(
    ...
    " of the total biomass, respectively; Tables \\\\@ref(tab:tab-biomass-est-fish-EBS) and \\\\@ref(tab:tab-biomass-est-fish-NBS)). ", 
    temp3)
}
...
for (i in 1:length(SRVY1)) {
  load(file = paste0(dir_out_tables, "tab-majortaxa-pchange-", SRVY1[i], ".rdata"))
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  load(file = paste0(dir_out_tables, "tab-estimates-maxyr-spp-wt.rdata"))
...
# temp[,1:4]<-mod_number(x = temp[,1:4], comma_seperator = T, digits = 0)
...
str0 <- text_list(paste0(
  ...
  " kg/ha per station; Table \\\\@ref(tab:tab-estimates-maxyr-spp-wt))."), sep = "", sep_last = "")
...
```{r tab-estimates-maxyr-spp-}
nickname <- "tab-estimates-maxyr-spp-"
```

```{r tab-estimates-maxyr-spp--child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```
...
for (i in 1:nrow(temp0)) {
  a <- list.files(path = dir_out_figures, 
                  pattern = paste0("fig-idw-cpue-fish-",temp0$file_name[i]), 
                  ignore.case = TRUE)
  ...
  temp <- c(temp, 
            paste0(
              temp0$print_name[i], 
              " (Fig", ifelse(length(a)>1, "s.", "."),
              ifelse(length(a)>1, 
                     paste0(" \\@ref(fig:fig-idw-cpue-fish-",temp0$file_name[i], "-above) and",
                            " \\@ref(fig:fig-idw-cpue-fish-",temp0$file_name[i],"-below)"), 
                     paste0(" \\@ref(fig:fig-idw-cpue-fish-",temp0$file_name[i],")") ),
              " and Tables \\@ref(tab:tab-estimates-maxyr-fish-",temp0$file_name[i],"-wt) and",
              " \\@ref(tab:tab-estimates-maxyr-fish-",temp0$file_name[i],"-num))"))
}
str0 <- text_list(temp)
```

Within this report plots of the spatial distribution and tables of CPUE (kg/hectare and no./hectare) for `r length(unique(report_spp1$file_name[report_spp1$taxon == "fish"]))` major species caught during the `r text_list(SRVY1)` continental shelf survey are presented below  (`r str0`). Differences in sums of estimates and totals are due to rounding. Total abundance-at-length and mean length estimates for major fish species are also presented in the subsections below. Population estimates by sex and size class for `r length(unique(report_spp1$file_name[report_spp1$plot_sizecomp == TRUE]))` fish species.
...
for (i in 1:nrow(temp0)) {
  a <- list.files(path = dir_out_figures, 
                  pattern = paste0("fig-idw-cpue-invert-",temp0$file_name[i]), 
                  ignore.case = TRUE)
  a <- a[grepl(pattern = ".rdata", x = a, ignore.case = TRUE)]
  ...
  temp <- c(temp, 
            paste0(
              ...
              " (Fig", ifelse(length(a)>1, "s.", "."),
              ifelse(length(a)>1, 
                     paste0(" \\@ref(fig:fig-idw-cpue-invet-",temp0$file_name[i], "-above) and",
                            " \\@ref(fig:fig-idw-cpue-invet-",temp0$file_name[i],"-below)"), 
                     paste0(" \\@ref(fig:fig-idw-cpue-invet-",temp0$file_name[i],")") ),
              " and Tables \\@ref(tab:tab-estimates-maxyr-invet-",temp0$file_name[i],"-wt) and",
              " \\@ref(tab:tab-estimates-maxyr-invet-",temp0$file_name[i],"-num))"))
}
str0 <- text_list(temp)
```

Plots of spatial distribution and tables of CPUE (kg/hectare and no./hectare) and tables for `r length(unique(report_spp1$file_name[report_spp1$taxon == "invert"]))` major invertebrate species caught during the `r text_list(SRVY1)` continental shelf are presented below (`r str0`). Differences in sums of estimates and totals are due to rounding. The crab species are discussed and analyzed in more detail in a report prepared by the AFSC Shellfish Assessment Program [`r paste0("@SAPcrab", maxyr)`].

```{r results_taxon_pchange}
  load(file = paste0(dir_out_tables, "tab-majortaxa-pchange-EBS.Rdata"))
  obj_ebs <- obj
  
if (SRVY == "NEBS") {
  
  load(file = paste0(dir_out_tables, "tab-majortaxa-pchange-NBS.Rdata"))
  obj_nbs <- obj
  temp <- dplyr::bind_rows(
      obj_ebs$raw, 
      obj_nbs$raw)
...
if (length(unique(temp$print_name))==1) {
  ...
  load(file = paste0(dir_out_tables, "tab-estimates-maxyr-invert-",report_spp1$file_name[report_spp1$print_name == temp$print_name[1]],"-wt.Rdata"))
  
  # temp0 <- paste0("tab-estimates-maxyr-invert-",report_spp1$file_name[report_spp1$print_name == temp$print_name[1]],"-wt")
  ...
  b <- list.files(path = dir_out_tables, 
                  pattern = report_spp1$file_name[report_spp1$print_name == temp$print_name[1]], 
                  ignore.case = TRUE)
  ...
  str0 <- paste0("The ",temp$print_name[1]," (*",temp$species_name1[1],"*) is common in the Bering ",
                 ...
                 ifelse(SRVY == "NEBS", 
                        " \\@ref(fig:tab_majortaxa_pchange_EBS) and \\@ref(fig:tab_majortaxa_pchange_NBS)) ", 
                        " \\@ref(fig:tab_majortaxa_pchange_EBS) "),
