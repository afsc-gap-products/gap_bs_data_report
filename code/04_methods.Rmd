...
> isnt "The results reported herein include data analyses for both the EBS and NBS." a bit obvious at this point and already stated in the intro?

## Survey Vessels and Sampling Gear

`r ifelse(nrow(vessel_info) == 1, "The vessel", "Both of the vessels") ` used for the `r maxyr` surveys are house-forward trawlers with stern ramps. The `r text_list(paste0(vessel_info$vessel_ital, " has a length overall (LOA) of ", vessel_info$length_m, " m (", vessel_info$length_ft, " ft)") ) `. All fishing operations were conducted in rigorous compliance with national and regional protocols detailed in @RN933. The AFSC equipped each of the vessels with standard 83-112 Eastern otter trawls, which have 25.3 m (83 ft) headropes and 34.1 m (112 ft) footropes (Figure `r crossref(list_obj = list_figures, nickname = "fig_trawl_gear", sublist = "number") `). These nets were attached to tail chains with 54.9 m (30 fm) paired dandylines. Each lower dandyline had a 0.61 m chain extension connected to the lower wing edge to improve bottom tending. Steel "V" doors measuring 1.8 × 2.7 m (6 × 9 ft) and weighing 816 kg (1,800 lbs) each were used for spreading the net opening while the trawl was fishing on the seafloor.
...
> is it still "3-second intervals" for temperature? has that changed with the new CTDs?
...
> "and perhaps other species' is a bit vauge - are there other species or... "perhaps" seems odd and this might be fixed by removing that word from the sentance 
...
`r NMFSReports::crossref(list_obj = list_tables, nickname = c("tab_specimen_samples_", "tab_oto_collection_scheme_"), sublist = "number", exact = FALSE)`) and brought to the AFSC to be processed for age determination. Depending on fish species, a length-stratified or a random sampling method was used for collecting otolith pairs in `r ifelse(SRVY == "NEBS", "the NBS and ", "")`two different EBS regions (southeast and northwest; Table `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_oto_collection_scheme_", sublist = "number", exact = FALSE)`). Otoliths were preserved in 50% glycerol-thymol solution. Individual fish weights and lengths were collected for each fish from which age structures were taken. For walleye pollock, the survey area was also divided into low- and high-density strata based on historical density and an isobath of approximately 70 m.  
...
> Liz, can you find the report number for [@RN891]? Add the AFSC-TM-### (or whatever it is) as "number = {...}" in the .bib file
