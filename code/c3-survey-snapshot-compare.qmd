...
`r str0`Prominent organisms are listed in Table (#tab-majortaxa-pchange-NBS) in order of largest increase to largest decrease in biomass. 
...
NBS survey catches were generally smaller than those from the EBS. Distributions of many predominant species extended throughout much of both survey regions. Several key species were found in the NBS in greater numbers than the EBS, including `r text_list(temp5$print_name)`.
...
```{r tab-majortaxa-pchange-NBS-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```

# Summary Results for Select Major Taxa

Survey results for select taxa are presented with a photograph of the taxon, maps of geographic distribution of CPUE (kg/km^2^), plots showing total population estimates by size, and a short summary of results. Maps of species distributions include both the eastern and northern Bering Sea survey regions to better illustrate patterns and trends in fish distribution and movement. For comparison, distribution maps and abundance-at-size plots show survey data for the `r text_list(nbsyr) ` surveys. Detailed summary profiles outlining several of the species showing ecologically significant trends are included below. **You can help us with this document by providing names in local language(s) and cultural or traditional uses for each fish and invertebrate species reviewed in this report.**
