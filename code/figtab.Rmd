...
header <- paste0("Fishing vessels ",
                 text_list(paste0(vessel_info$vessel_ital, 
                                               " (", c("left", "right"), ")")),
                 " contracted to assist with the ",maxyr," ",
                 text_list(SRVY1),
                 " bottom trawl survey.")
...
header <- paste0("Schematic diagram of the 83-112 eastern otter trawl gear used during the ", 
                 maxyr ," ", text_list(SRVY1) ," bottom trawl survey", 
...
header <- paste0("Stratum areas and sampling densities used during the ",maxyr,
                 " ", text_list(SRVY1) ," bottom trawl survey", 
...
## tab-oto-collection-scheme-[SRVY]

```{r tab-oto-collection-scheme-[SRVY]}

nickname0 <- paste0("tab-oto-collection-scheme-") 
...
for (case1 in case1_) {
  for (case in c("bottom", "surface")) {
    ...
    header <- paste0(stringr::str_to_title(case)," temperatures (°C) during the ", range_text(yrs),
    ...
    if (case1 == "EBS") {
      yrs <- (maxyr-19):maxyr # sort(sort(c(unique(cruises$year), 2020), decreasing = TRUE)[1:20])
      row0 <- 4
      ...
      if (case == "surface") {
        ...
        nbs_no_2018 <- raster::subset(coldpool::nbs_ebs_surface_temperature, 
                                      subset = names(coldpool::nbs_ebs_surface_temperature)[!grepl(x = names(coldpool::nbs_ebs_surface_temperature),
                                                                                                            pattern = 2018)])
        
        sebs_only <- names(coldpool::ebs_surface_temperature)[which(!(names(coldpool::ebs_surface_temperature) %in% 
                                                                       paste0("s", names(nbs_no_2018))))]
        
        sebs_temp <- raster::subset(coldpool::ebs_surface_temperature,
                                    subset = sebs_only) %>%
          raster::projectRaster(coldpool::nbs_ebs_surface_temperature)
        
        rasterbrick <- raster::addLayer(sebs_temp, 
                                        nbs_no_2018)
        
        # a <- raster::subset(x = rasterbrick,
        #        subset = names(rasterbrick)[length(names(rasterbrick))-1])
      } else {
        ...
        nbs_no_2018 <- raster::subset(coldpool::nbs_ebs_bottom_temperature, 
                                      subset = names(coldpool::nbs_ebs_bottom_temperature)[!grepl(x = names(coldpool::nbs_ebs_bottom_temperature),
                                                                                                            pattern = 2018)])
        
        sebs_only <- names(coldpool::ebs_bottom_temperature)[which(!(names(coldpool::ebs_bottom_temperature) %in% 
                                                                       paste0("s", names(nbs_no_2018))))]
        ...
        # a <- raster::subset(x = rasterbrick,
        #        subset = paste0("sebs_ste_2021_gear_temperature"))
      }
      ...
      # a@data@names <- gsub(pattern = "_2021_", replacement = "_2020_", x = a@data@names)
      rasterbrick$temp <- a
    ...
    nickname <- paste0("fig-temperature-",case1,"-", ifelse(case=="bottom", "bt", "st"))
    ...
    figure <- plot_temps_facet(
      ...
      row0 = row0, 
      title0 = paste0("Eastern", ifelse(case1 == "NEBS", " and northern", ""), 
                      " Bering Sea ", 
                      stringr::str_to_title(case), " Temperature")) 
...
if (SRVY == "NEBS"){
  ...
  header <- paste0("Fish taxa from survey catches in the ",
...
temp <- function(exp0, 
  ...
  # total_biomass0 <- total_biomass0[total_biomass0$year == maxyr, ]
...
### tab-biomass-est-fish_[SRVY]

```{r tab-biomass-est-fish_[SRVY]}

nickname0 <- "tab-biomass-est-fish-"
...
### tab-biomass-est-invert_[SRVY]

```{r tab-biomass-est-invert_[SRVY]}

nickname0 <- "tab-biomass-est-invert-"
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0(
    ...
    compareyr," and ",maxyr," ", 
    haul_cruises_maxyr$SRVY_long[i], 
    " shelf bottom trawl surveys for predominant fish and invertebrate taxa. Crab data is summarized under other crustaceans and discussed in detail in the annual crab technical memorandum produced by the shellfish assessment program. ")
...
## tab-estimates-maxyr-spp-[SRVY]

```{r tab-estimates-maxyr-spp-[SRVY]}

nickname0 <- paste0("tab-estimates-maxyr-spp-") 
