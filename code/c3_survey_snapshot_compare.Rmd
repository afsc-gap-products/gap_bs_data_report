...
nickname <- c(paste0("fig-temperature-bt-", 1:2), 
              paste0("fig-temperature-st-", 1:2))
```
...
str0 <- paste0("Between ", compareyr, " and ", maxyr , ", the largest increases in CPUE were seen in ",
...
From `r compareyr` to `r maxyr ` in the NBS, `r text_list(paste0(xunits(temp3$fish), " ", ifelse(temp3$fish==1, "fish", "fishes")," and ", xunits(temp3$invert), " ", ifelse(temp3$fish==1, "invertebrate", "invertebrates")," experienced ", temp3$x, " CPUE"))`. Prominent fish species that exhibited no change between `r compareyr` and `r maxyr`, included `r text_list(paste0(temp4$species_name[temp4$change_cpue_case == "no notable change in" & temp4$taxon == "fish"])) ` (within ± 25% change from the previous survey year) in CPUE. `r str0` 
...
for (i in 1:length(unique(temp1$case))) {
  str0 <- paste0(str0, 
                   paste0("Species groups that previously exhibited ", 
                          ...
                          "\\@ref(tab:tab-majortaxa-pchange-NBS)). "))
}
