...
header <- paste0("Map of ",maxyr," ",SURVEY," survey",plural_surveys," sampling stations. The area enclosed within the light gray line contains the eastern Bering Sea shelf stations that have been sampled annually since 1982, whereas the area outlined by the dark gray line contains the northern Bering Sea stations that were sampled in ",maxyr,". The dots within each area indicate station locations.") 
...
figure <- plot_survey_stations(
  reg_dat = reg_dat0, 
  # station = station %>% dplyr::filter(in_maxyr), 
  # haul_cruises_maxyr = haul_cruises_maxyr, 
  station_grid = FALSE, 
  stratum_grid = TRUE, 
  stratum_no = FALSE, place_labels = TRUE, 
  bathymetry = FALSE, 
...
```{r fig-survey-sample-stations-1530, eval = tow1530}
...
if (is.numeric(table_raw$stomach_target)){
  table_raw <- table_raw %>% 
...
table_raw <- lengths %>% # length data - from oracle
  dplyr::select(SRVY, year, species_code, frequency) %>% # common_name, 
  dplyr::filter(year %in% c(maxyr, compareyr) &
                  !(species_code %in% c(68560, 68580, 68590, 69322, 69323, 69400, 69310, 68577, 68781) )) %>% # NO CRABS!
  ...
  tidyr::pivot_wider(data = ., id_cols = c("common_name", "order"), 
                     names_from = c("SRVY", "year"), values_from = "length")  %>% 
  dplyr::ungroup()  %>%
  dplyr::arrange(order) %>%
  dplyr::select(-order)
...
for (case in c("bottom", "surface")) {
  for (case_yr in case_yr0) {
    ...
    a <- temp(yrs, row0, case, 
              title0 = paste0(stringr::str_to_title(SURVEY), " ",
                     case, " temperature (", range_text(yrs), ")"))
    figure <- a$figure
    header <- a$header
...
```{r fig-temperature-[st/bt]-ptpres}
height <- 8
width <- 8
...
for (case in c("bottom", "surface")) {
  ...
  a <- temp(yrs, row0, case, title0 = paste0(stringr::str_to_title(SURVEY), " ",  
                   case, " temperature (", range_text(yrs), ")"))
  figure <- a$figure
  header <- a$header
...
```{r fig-temperature-[st/bt]-nbspres}
height <- 8
width <- 8
...
for (case in c("bottom", "surface")) {
  nickname <- paste0("fig-temperature-", ifelse(case=="bottom", "bt", "st"), "-nbspres")
  a <- temp(yrs, row0, case)
  figure <- a$figure
  header <- a$header
...
temp1 <- function(table_raw, strat, total) {
  ...
  table_print <- table_raw0 %>%
    ...
    flextable::add_header_row(x = .,
                              ...
                              colwidths = c(2,1,1,length(strat)-1)) %>%
    flextable::width(x = .,  
    ...
    flextable::merge_at(x = ., j = c("999"), part = "header") %>% # , "CI95"
  ...
  table_print <- table_print %>% 
    ...
    flextable::align(x = ., j = c("999", #"CI95", 
                                  "prop",  strat), 
                     align = "right", part = "all") %>%
    flextable::align(x = ., 
                     j = c(strat), 
...
tab_app_fish_ <- function(temp, 
  ...
  for (i in 1:nrow(temp)) {
    ...
    table_raw1 <- sizecomp_maxyr %>% 
      dplyr::filter(species_code %in% spp_code &
                      SRVY == temp$SRVY[i]) %>% 
      dplyr::distinct() %>%
      dplyr::ungroup()
    
    if (nrow(table_raw1) != 0) { 
      ...
      table_raw1 <- table_raw1 %>% 
        ...
        tidyr::pivot_wider(data = ., 
                           ...
                           values_fill = 0) %>%
        dplyr::mutate(
          males = ifelse(("males" %in% table_raw1$sex), males, 0),
          females = ifelse(("females" %in% table_raw1$sex), females, 0),
          unsexed = ifelse(("unsexed" %in% table_raw1$sex), unsexed, 0),
      ...
      table_raw <- table_raw1 %>% 
        dplyr::ungroup() %>% 
        dplyr::mutate(
          prop = total/sum(table_raw1$total, na.rm = TRUE), 
          cumprop = cumsum(prop))
      # table_raw$cumprop <- cumsum(table_raw$prop)
...
temp <- tidyr::crossing(haul_cruises_maxyr %>% 
                        ...
                        report_spp1 %>% 
                          ...
                          dplyr::select(file_name, print_name, common_name1) %>% # , species_name1 , species_name
                          dplyr::distinct())
