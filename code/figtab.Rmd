...
a <- report_spp1 %>% 
  dplyr::select(print_name, species_code, plot_sizecomp, order) %>% 
  dplyr::distinct() %>% 
  dplyr::mutate(species_code1 = species_code, 
                include  = ifelse(is.na(plot_sizecomp ) | isFALSE(plot_sizecomp ), 
                                  FALSE, TRUE) ) %>% 
  dplyr::filter(include == TRUE &
                  !(print_name %in% c("snailfishes", "jellyfish", "sea urchins", "sea peach")))
