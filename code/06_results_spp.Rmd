...
temp <- obj$raw
cite_bio <- paste0(" (Tables ", tab_stat," and ", tab_wt, "; Figs. ", fig_dist, " and ", fig_biopop, 
...
cite_pop <- paste0(" (Tables ", tab_stat," and ", tab_no, "; Fig. ", fig_biopop,")")
  ...
  if ("Survey catch totals" %in% names(temp0)) {
  case <- paste0(case, 
                 ", which was a", 
                 ifelse(c("a", "e", "i", "o", "u") %in% tolower(substr(x = temp0$`Survey catch totals`, start = 1, stop = 1)), 
                        "n", ""), 
                 substr(x = temp0$`Survey catch totals`, start = 2, stop = nchar(temp0$`Survey catch totals`)) )
