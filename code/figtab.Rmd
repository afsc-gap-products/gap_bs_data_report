...
biomass_group_999 <- dat %>%  ## summarize by group to compare to prior years tech memo    
  ...
  biomass_total_bystrata <- dat %>% # find estimated total biomass for all stratum for all taxon
      dplyr::filter(grepl(pattern = "_", x = group0)) %>% 
      dplyr::group_by(SRVY, strata, num_hauls ) %>%
      dplyr::summarize(biomass_mt = sum(biomass_mt, na.rm = TRUE),
                       bio_var = sum(bio_var, na.rm = TRUE)) %>%
      dplyr::mutate(
        ci = confint(num_hauls, bio_var), 
  ...
  biomass_total_999 <- dat %>% # find estimated total biomass for all stratum for all taxon
    dplyr::filter(grepl(pattern = "_", x = group0)) %>% 
    dplyr::group_by(SRVY) %>%
    dplyr::summarize(biomass_mt = sum(biomass_mt, na.rm = TRUE),
                     bio_var = sum(bio_var, na.rm = TRUE)) %>%
