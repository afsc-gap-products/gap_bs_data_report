...
The `r maxyr` eastern Bering Sea shelf and northern Bering Sea bottom trawl surveys were conducted aboard the chartered commercial stern-trawlers `r text_list(cruises_maxyr0$vessel_ital)` (Fig. (#fig-vessels)). `r nbs_insert `  

```{r fig-vessels0}
nickname <- "fig-vessels"
```

```{r fig-vessels0-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```

```{r team-desc-insert-text, child=paste0(dir_out_rawdata, "/0-team-desc.qmd")}
```
...
Both vessels sampled using an 83-112 Eastern otter trawl that has been historically used for eastern Bering Sea shelf, Chukchi, and Beaufort Sea surveys (Fig. (#fig-trawl-gear-alt)). This trawl is significantly smaller and weighs less than trawls used for commercial fishing in Alaska. Each trawl tow is conducted for 30 minutes at a speed of 3 knots.

```{r fig-trawl-gear-alt0}
nickname <- "fig-trawl-gear-alt"
```

```{r fig-trawl-gear-alt0-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
