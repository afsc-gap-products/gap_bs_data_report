...
if (sum(temp$biomass_mt[temp$stratum == "Total"] != 0, na.rm = TRUE) > 0) {
  ...
  for (i in 1:2) {
    ...
    table_print <- table_print %>% 
      ...
      theme_flextable_nmfstm(row_lines = FALSE, 
                             pgwidth = full_page_portrait_width)  %>%
      flextable::width(width = ((full_page_portrait_width-.5)/(ncol(table_print0)-1)), unit = "in") %>%
...
table_raw <- table_raw %>%
  ...
  for (i in 1:length(unique(table_raw$SRVY_long))){
    ...
    tidyr::crossing(data.frame(var = unique(temp$var), 
                 ci_up_var = unique(temp$ci_up_var), 
                 ci_dw_var = unique(temp$ci_dw_var)))
  }

  a_bio <- find_units(unit = "metric tons", 
                  unt = "t", 
                  dat = table_raw$val[table_raw$var == "biomass_mt"]) 
  
  a_pop <- find_units(unit = "individuals", 
                  unt = "", 
                  dat = table_raw$val[table_raw$var == "population_count"]) 
  ...
  table_raw <- table_raw %>% 
    ...
    dplyr::mutate(
      ...
      var_ = dplyr::case_when(
        grepl(x = var, pattern = "biomass") ~ 
          paste0("Biomass", ifelse(a_bio$unit_word == "", "", "\n"), a_bio$unit_word), 
        grepl(x = var, pattern = "population") ~ 
  ...
  figure <-
    ...
    ggplot2::facet_grid(rows = vars(var_), 
                        ...
                        switch = "both") +
    # aesthetics
    ggplot2::guides(color = guide_legend(title = "", nrow = 1)) +
    ggplot2::theme( 
