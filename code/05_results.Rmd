...
Plots of the spatial distribution, abundance-at-length estimates, and tables of CPUE (kg/km^2^ and no/km^2^) for `r sum(!is.na(report_spp$order[report_spp$taxon == "fish"]))` major fish species caught during the `r SURVEY` continental shelf survey are presented in below. Differences in estimates and totals may be due to rounding.

### Selected Invertebrates Estimates

Plots of spatial distribution and tables of CPUE (kg/km^2^ and no/km^2^) for the `r sum(!is.na(report_spp$order[report_spp$taxon == "invert"]))` major invertebrate species caught during the `r SURVEY` continental shelf surveys presented below. Differences in estimates and totals may be due to rounding. 
