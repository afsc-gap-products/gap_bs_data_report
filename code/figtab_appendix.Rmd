...
tab_taxa_encountered_ <- function(temp, 
  ...
  cnt_tables <- 0
  nickname0 <- "tab-app-taxa-encountered-" 
...
tab_app_principal_fish_ <- function(temp, 
  ...
  nickname0 <- "tab-app-principal-fish-" 
  cnt_tables <- 0
  ...
  for (i in 1:nrow(temp)) {
    ...
    header <- paste0("Population estimates by sex and size for ",
                     temp$print_name[i], " (", spp_sci, ")", 
                     # ifelse(grepl(pattern = "*", x = spp_sci, fixed = TRUE), 
                     #        paste0(" (<em>",gsub(pattern = "*", 
                     #                                replacement = "", 
                     #                                x = temp$species_name[i], 
                     #                                fixed = TRUE),"</em>)"), 
                     #        spp_sci),
    " from the ", 
    maxyr, " ", temp$SRVY_long[i], 
...
list_tables0 <- tab_app_principal_fish_(
  temp = temp, 
  # cnt_pre = , # paste0("Appendix ", cnt_pre), 
  maxyr = maxyr, 
