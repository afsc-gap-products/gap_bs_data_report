...
```{r}
insert1 <- paste0(ifelse(length(haul_cruises_maxyr$SRVY_long)>1, xunits(length(haul_cruises_maxyr$SRVY_long)), "a"), 
       ' contiguous bottom trawl ', 
       ifelse(length(haul_cruises_maxyr$SRVY_long)>1, 'surveys', 'survey'),
       ' on the Bering Sea continental shelf: ', 
              text_list(paste0('the ',haul_cruises_maxyr$yrofsurvey ,haul_cruises_maxyr$stndth, ' "',haul_cruises_maxyr$SRVY_long,'" (',haul_cruises_maxyr$SRVY,') shelf survey')), '. ')
...
if (sum(haul_cruises_maxyr$SRVY %in% "EBS")>0) {
  insert2 <- paste0(insert2, 
                    ...
                    'since 1982 and is the longest running timeseries of its kind of data in the region [@RN976]. ')
}
...
In `r maxyr`, the National Marine Fisheries Service’s (NMFS) Resource Assessment and Conservation Engineering (RACE) Division of the Alaska Fisheries Science Center (AFSC) conducted `r insert1`
...
`r NMFSReports::text_list(x = paste0(maxyr, " ", haul_cruises_maxyr$SRVY, " survey results with those from the ", haul_cruises_maxyr$compareyr, " ", haul_cruises_maxyr$SRVY, " survey"))` `r paste0("[", haul_cruises_maxyr$compareyr_ref, "]")`. 
...
Good management of fisheries resources and a healthy ecosystem are especially important to the Alaska Native communities as a way of life and to the tens of thousands of people who are employed by the Alaska fishing industry, which generates billions of dollars for the U.S. economy.
...
In this document, we will compare the 2021 NEBS survey results with those from the 2019 NEBS survey [`r ref_compareyr`]. For survey results referenced from previous surveys herein, the reader should refer to the AFSC Technical Memoranda cited above.
