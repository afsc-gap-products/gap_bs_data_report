...
```{r}
temp <- list_tables$tab_stratum_areas$raw %>% 
  dplyr::filter(!grepl(pattern = "Total", x = Region) & 
                  grepl(pattern = "EBS", x = Region))


temp_tot <- list_tables$tab_stratum_areas$raw %>% 
...
`r list_table$tab_stratum_areas$res `
...
`r list_table$tab_specimen_samples_EBS$res `

`r ifelse(SRVY == "NEBS", tab_specimen_samples_NBS, "") `

> for next paragraph, use 0_oto_collections from general folder
...
`r ifelse(SRVY == "NEBS", ", "," and")` `r crossref(list_obj = list_tables, nickname = "tab_oto_collections_EBS", sublist = "number")`
`r ifelse(SRVY == "NEBS", paste0(", and ", crossref(list_obj = list_tables, nickname = "tab_oto_collections_NBS", sublist = "number")), "")`) and brought to the AFSC to be processed for age determination. Depending on fish species, a length-stratified or a random sampling method was used for collecting otolith pairs in `r ifelse(SRVY == "NEBS", "the NBS and ", "")`two different EBS regions (southeast and northwest; Table `r crossref(list_obj = list_tables, nickname = "tab_oto_collections_EBS", sublist = "number")`
`r ifelse(SRVY == "NEBS", paste0(" and ", crossref(list_obj = list_tables, nickname = "tab_oto_collections_NBS", sublist = "number")), "")`). Otoliths were preserved in 50% glycerol-thymol solution. Individual fish weights and lengths were collected for each fish from which age structures were taken. For walleye pollock, the survey area was also divided into low- and high-density strata based on historical density and an isobath of approximately 70 m.  

`r list_table$tab_oto_collection_scheme_EBS$res `

`r ifelse(SRVY == "NEBS", tab_oto_collection_scheme_NBS, "") `
...
`r list_table$tab_special_projects_EBS$res `
