...
fig_st <- "\\@ref(fig:fig-temperature-st-1) and \\@ref(fig:fig-temperature-st-2)"
  ...
  if (report_title == "community") {
    tab_majorspp_bio <- tab_majortaxa_pchange <- c("\\@ref(tab_majortaxa_pchange-comm)")
...
```{r res_spp_nbs_{{jj}}, results='asis', eval = FALSE}
tab0 <- str0 <- ""
...
```{r res_spp_nebs_{{jj}}, results='asis', eval = FALSE}
tab0 <- str0 <- ""
...
nickname <- paste0("tab-env-", spp_taxon,"-", spp_file) # , ifelse(report_title == "data", "-data", "-comm"))
# ```{r fig-spp-{{jj}} }
# nickname <- paste0("fig-spp-", spp_file) 
# ```
# 
# ```{r fig-spp-child-{{jj}}, child = paste0(dir_code, "_child_report_figtab.Rmd")}
# ```
```
...
```{r fig-dist-{{jj}}}
nickname <- paste0("fig-dist-",spp_taxon,"-", spp_file, ifelse(report_title == "data", "-data", "-comm"))
```
...
```{r fig-sizecomp-{{jj}}}
nickname <- paste0("fig-sizecomp-",spp_taxon,"-", spp_file, ifelse(report_title == "data", "-data", "-comm"))
```
