...
```{r fig-dist-[spp]-pres}
nickname <- paste0("fig-dist-", spp_file, "-comms")
...
temp <- size_plot(srvy, yrs, lengths, sizecomp |> dplyr::filter(srvy == "NBS"), print_n, height, width, nickname, legend_font_size, report_title0) 
comm_size <- temp$figure_print +
          ggplot2::theme(
...
if (exists("comm_size")) {
  temp1 <- cowplot::plot_grid(comm_dist, 
                              NULL, 
                              comm_size, 
                              ncol = 3, 
                              rel_widths = c(1.3,.1,.6)) # 1, .1., .9 worked for presentations
} else {
  pres_size_txt <- "There are no sizecomps for this species. "
  temp1 <- cowplot::plot_grid(pres_dist, 
                              NULL, 
...
temp2 <- cowplot::plot_grid(pres_timeseries + 
                            ...
                            pres_spp,
                            ncol = 3, 
                            rel_widths = c(1.3, .1, .6)) 

figure_print <- cowplot::plot_grid(temp1, temp2, nrow = 2, rel_heights = c(1.2,.8))

header <- paste0(
  "Top left: ", 
  "Total abundance-at-length estimates by sex encountered during the ", compareyr[names(compareyr) == "NBS"], " and ", maxyr, " northern Bering Sea surveys. Length distributions are scaled to the total estimated population size. ", # pres_size_txt, 
  "Top right: ", comm_dist_txt, 
  "Bottom left: Photo of ", spp_print, ". ", 
  "Bottom right: ", pres_timeseries_txt)
# header <- paste0(header0, "Bottom left: Photo of ", spp_print, ". ")
