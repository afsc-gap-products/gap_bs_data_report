...
header <- paste0(
  ...
  "(",ifelse((exists("comm_size")), "D", "C"),") Photo of ", ifelse(tolower(substr(spp_print, 1, 1)) %in% c("a", "e", "i", "o", "u") | substr(x = spp_print, nchar(spp_print), nchar(spp_print)) == "s", "an ", "a "), spp_print, ". ")
...
figure_print <- cowplot::plot_grid(
  ...
  comm_size1, 
  ncol = 3, 
  rel_widths = c(.6,.025,.375),
  align = "v",   # Align vertically
  axis = "l")     # Align by the left axis

# Then use ggdraw() to add a custom label on top of the combined plot
figure_print <- cowplot::ggdraw(figure_print) +
  cowplot::draw_label("A", x = 0.10, y = 0.94, hjust = 0, vjust = 0, size = 16) +
  cowplot::draw_label(ifelse((exists("comm_size")), "C", "B"), x = 0.10, y = 0.32, hjust = 0, vjust = 0, size = 16) #+
  # cowplot::draw_label(ifelse((exists("comm_size")), "D", "C"), x = 0.77, y = 0.32, hjust = 0, vjust = 0, size = 16)
