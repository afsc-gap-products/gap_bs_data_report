...
if (spp_plot_idw | spp_plot_pa) {
  ...
  for (case_yr in case_yr0) {
    ...
    if (spp_plot_idw) {
      ...
      stars0 <- stars::st_redimension(a$extrapolation.stack) 
      ...
      figure <- ggplot() +
        ...
        geom_stars(data = stars0,
                            na.rm = FALSE,
                            show.legend = TRUE) +
        ggplot2::facet_wrap( ~ new_dim, nrow = row0) + 
        coord_sf() + 
...
```{r fig-sizecomp-[taxon]-[spp], eval = FALSE}
...
```{r tab-env-[taxon]-[spp], eval = (report_title == "community")}
...
table_print <- table_print1 %>% 
  ...
  flextable::colformat_image(j = "img" , 
                             width = ifelse(length(stat$SRVY)==2, 2, 3), 
                             height = ifelse(length(stat$SRVY)==2, 2, 3)) %>% 
  flextable::merge_v(j = "img") %>% # part = "all", combine = TRUE, 
  ...
  flextable::width(j = 1:ncol(table_print1), 
                   width = c(ifelse(length(stat$SRVY)==2, 2, 3), 
                             1.25, rep_len(x = 1.5, length.out = (ncol(table_raw)-1)) ))
...
```{r tab-bio-[taxon]-[spp], eval = FALSE}
                                   ...
                                   sub(replacement = "", pattern = "an ", 
                                       x = gsub(pattern = "a ", replacement = "", 
                                                x = pchange(
                                                  ...
                                                  end = temp_bio$biomass_mt[temp_bio$year == (yrs[yrs < temp_bio$year[i]])])))))
  )
}

table_raw <- table_raw %>% 
  dplyr::filter(year != min(year)) %>% 
  dplyr::arrange(SRVY, desc(year))
...
```{r fig-pres-[spp], eval = FALSE}
nickname0 <- paste0("fig-pres-",spp_file)

table_raw0 <- biomass %>%
  dplyr::filter(stratum == 999 & species_code %in% spp_code) %>% 
  dplyr::select(SRVY, year, species_code, taxon, 
                # upperb, lowerb, 
                biomass_var, biomass_mt, 
                # upperp, lowerp, 
                population_sd, population_count) %>% 
  dplyr::left_join(x = . , 
                   y = haul_cruises_maxyr %>% 
                     dplyr::select(SRVY, SRVY_long),
                   by = "SRVY")  %>% 
...
temp00 <- function(table_raw, yrs, maxyr, compareyr, unit, unt, y_long, legend_combined = TRUE, mean_in_legend = FALSE) {
  ...
  header <- text_list(paste0("**", b$Survey, " ", y_long, "** ", 
...
height <- 4
nickname <- paste0("fig-pres-biomass-", spp_file)
...
## tab-pres-[spp]-biomass

```{r tab-pres-[spp]-biomass, eval = FALSE}

nickname0 <- paste0("tab-pres-", spp_file, "-biomass")
...
## fig-pres-[spp]-population

```{r, fig_pres_abundance_, eval = FALSE}
y_long <- "Abundance"
unit = "individuals"
unt = ""
...
height <- 4
width <- width0
nickname0 <- paste0("tab-pres-", spp_file, "-population")
...
# fig-pres-[spp]-biomass-population

```{r fig_pres_biomass_abundance_, eval = FALSE}
height <- height0
width <- width0
nickname0 <- paste0("tab-pres-", spp_file, "-biomass-population")
...
```{r fig-pres-[spp]-sizecomp, eval = FALSE}
# , eval = spp_plot_sizecomp
  height <- height0
  width <- width0
  header <- ""
  nickname <- paste0("fig-pres-",spp_file,"-sizecomp")
  ...
  header <- paste0("Total abundance-at-length estimates of ",
                   ...
                   ifelse(SRVY %in% c("NEBS", "NBS"), 
                          paste0(" by sex (",text_list(tolower(unique(sizecomp0$sex))), ")"), 
                          ""), 
                   " observed during the ",
                   text_list(unique(sizecomp0$SRVY)), 
                   " shelf bottom trawl survey", 
...
## fig_pres_dist_cpue_[spp]

```{r fig_pres_dist_[spp], eval = spp_plot_sizecomp}
# , eval = (spp_plot_idw | spp_plot_pa)
  nickname <- paste0("fig-pres-", spp_file, "_", 
  ...
  if (spp_plot_idw) {
    ...
    header <- paste0(stringr::str_to_sentence(spp_print), ifelse(is.na(spp_sci), "", paste0(" (",spp_sci,")")), 
                     ...
                     " (right) ", text_list(unique(table_raw$SRVY)),  " shelf bottom trawl survey", 
                     ifelse(length(unique(table_raw$SRVY))>1, "s", ""),".")
...
# fig-pres-sizecomp-dist-[spp]

```{r fig-pres-sizecomp-dist-[spp], eval = FALSE}
nickname <- paste0("fig-pres-sizecomp-dist", spp_file)
height <- 6
table_raw <- table_raw0
