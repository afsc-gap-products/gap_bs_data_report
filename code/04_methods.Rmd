...
> How do I calculate "net mensuration system failed to record data for six tows"? # TOLEDO 
...
> I changed the equation format... is that... ok? Easier to code and more official looking? # TOLEDO! 

During the EBS survey, `r str_ebs `, and for the NBS survey `r str_nbs `. To estimate these missing net width values, the mgcv package in R [@RN997] was used to relate mean net width with the inverse scope (m) and mean net height (m) from valid tows based on the relationship investigated by @RN921, where $w$ is the net width (m), $h$ is the net height, and $s$ is the scope, below inversed.
...
temp <- readxl::read_excel(path = paste0(dir_out_rawdata, "/0_vessel_equations.xlsx"), 
  ...
  dplyr::left_join(x = ., 
                y = haul_maxyr %>% 
                                # dplyr::filter(vessel == vess & SRVY %in% SRVY1) %>% 
                                dplyr::select(SRVY, haul, vessel) %>% 
                                dplyr::distinct() %>% 
                                dplyr::group_by(SRVY, vessel) %>% 
                                dplyr::summarize(n = n()), 
                by = c("vessel", "SRVY")) %>% 
...
", 
NMFSReports::text_list(paste0(temp0$SRVY, " (n = ", temp0$n, "):
  $$",temp0$eq, "$$
...
> How do I calculate "These equations were subsequently used to estimate the respective net spread values for the 19 tows with missing net width values."? # TOLEDO 
...
temp <- dplyr::bind_rows(
  ...
  dplyr::group_by(SRVY) %>% 
  dplyr::summarise(species_code_n = unique(species_name)) %>% 
  dplyr::summarise(n = length(all_of(SRVY))) 
