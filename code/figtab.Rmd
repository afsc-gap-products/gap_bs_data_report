...
temp <- readr::read_csv(paste0(dir_out_rawdata, "/0_other_field_collections.csv"), skip = 1) %>%
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_raw <- table_raw0 %>% 
    dplyr::ungroup() %>% 
      ...
      dplyr::select(-"SRVY") 
  table_raw[is.na(table_raw) | table_raw == "0"] <- "-"
...
nickname0 <- paste0("tab_oto_collection_scheme_") 
 ...
 table_raw0 <- readr::read_csv(paste0(dir_out_rawdata, "/0_collection_scheme.csv"), skip = 1) %>% 
...
## tab_estimates_maxyr_spp

> can I remove the total hauls column and add it to the header, instead? 

```{r tab_estimates_maxyr_spp}
nickname <- paste0("tab_estimates_maxyr_spp") 
