...
make_idw_stack <- function (x = NA, COMMON_NAME = NA, LATITUDE = NA, LONGITUDE = NA, 
                            ...
                            set.breaks = "jenks", grouping.vars, grid.cell = c(5000, 5000), 
                            in.crs = "+proj=longlat", out.crs = "EPSG:3338", 
...
make_idw_stack <- function(x = NA, 
  ...
  for(ii in 1:nrow(unique_groups)) {
    ...
    if(length(grouping.vars) > 1) {
      ...
      x_sub <- x[which(x[[grouping.vars]] == unlist(unique_groups)[ii]), ] ## changed this line
      #Not sure why this little change is making the difference in my code (only sometimes) 
...
for (case_yr in case_yr0) {
  ...
  header <- paste0("Distribution (Weight CPUE (kg/km^2^)) of ", header0)
  
  set.breaks <- set_breaks(dat = table_raw, var = "cpue_kgkm2")
  # if (spp_print == "jellyfish") {
  #   set.breaks <- c(0, #0.01,
  #                   # 0.1,
  #                   0.5,
  #                   1, #2,3,
  #                   7,
  #                   set.breaks[(length(set.breaks)-1)], 
...
a <- table_raw %>% 
  ...
  raster_test <- stars00 <- make_idw_stack(
    x = table_raw %>% 
...
#                          pgwidth = full_page_portrait_width+1) %>% 
...
#   flextable::width(width = c(.9, .9, .85, .85, .85, .65, .65, .65, 1.2)) %>% 
#   flextable::align(i = 1, j = 4, align = "center", part = "header")

# table_print_env <-
#   flextable::flextable(data = table_raw[,1:5]) %>%
#   flextable::bold(j = 1, part = "body") %>%
#   flextable::theme_zebra() %>%
#   flextable::set_header_labels("Bottom Temperature (°C)" = "Bottom",
#                                "Surface Temperature (°C)" = "Surface") %>%
#   flextable::add_header_row(top = TRUE,
#                             values = c(names(table_raw)[1:3], "Temperature (°C)"),
#                             colwidths = c(1,1,1,2)) %>%
#   theme_flextable_nmfstm(row_lines = FALSE,
...
# 
# table_print_bio <-
#   flextable::flextable(data = table_raw[,c(1, 6:9)]) %>%
#   flextable::bold(j = 1, part = "body") %>%
#   flextable::theme_zebra() %>%
#   flextable::bg(i = ~ grepl(x = `Biomass\n% Change`, pattern = "increase"),
#                 bg = positive,
...
if (sum(temp$biomass_mt[temp$stratum == "Total"] != 0, na.rm = TRUE) > 0) {
    ...
    if (i == 1) { # CPUE (WT/km²) and BIOMASS
      ...
      if (nrow(table_raw) != 0) { # if none were caught, don't do any of this
        ...
        header <- paste0("Mean weight CPUE",cpue_kgkm2_mean0$unit_word,
                         " with standard deviation",cpue_kgkm2_sd0$unit_wrd,
  ...
  flextable::as_flextable(hide_grouplabel = TRUE) %>%
    ...
    table_print <- table_print %>% 
      # flextable::merge_v(j = "SRVY_long", part = "body") %>% 
      ...
      theme_flextable_nmfstm(row_lines = FALSE, 
                             pgwidth = full_page_portrait_width)  %>%
      flextable::width(width = ((full_page_portrait_width-.5)/(nrow(table_raw)-1)), unit = "in") %>%
      flextable::width(width = .5, unit = "in", j = c("stratum")) %>%
      # flextable::width(width = .25, unit = "in", j = c("SRVY")) %>%
...
```{r, fig-[spp]-biomass-timeseries, eval = (report_title != "nbspres")}
if (report_title %in% c("community", "data")) {
  nickname <- paste0("fig-biomass-", spp_file, 
                     ifelse((report_title %in% c("community")), "-comm", "-data"))
  description <- FALSE
  height <- 3
...
figure <- figure_bio <- temp00(table_raw0 = table_raw, 
                               var = "biomass_mt", 
                               maxyr = maxyr, 
                               compareyr = compareyr, 
                               unit = unit, 
                               ...
                               y_long = y_long, 
                               mean_in_legend = TRUE, 
                               error_bar = TRUE, #ifelse(length(unique(table_raw0$species_code))>1, FALSE, TRUE), 
...
#     cowplot::draw_image(image = readPNG(paste0(dir_img, spp_file, ".png")),
#                         hjust = 0, vjust = 0,
#                         x = 0.70, y = .40, # x = 0.07, y = -.42,
#                         width = .25)
# }

header <- paste0("Estimates of ", spp_print, 
                 ifelse(is.na(spp_sci), "", 
                        paste0(" (",spp_sci,")")), 
                 " ", tolower(y_long), 
                 ifelse(unt == "", "", paste0(" (",unt, ")")), 
                 " from the ",
                 paste0(range(table_raw$year), collapse="-"), " ", 
                 ifelse(length(table_raw$`Bering Sea\nSurvey`) == 1, paste0(tolower(table_raw$`Bering Sea\nSurvey`), " Bering Sea"), SURVEY), 
                 " shelf bottom trawl survey", 
                 ifelse(length(table_raw$`Bering Sea\nSurvey`) == 1, "", plural_surveys),". ")
  ...
  #   figure <-  
  #     cowplot::ggdraw(figure + 
  #      ggplot2::theme(plot.margin = margin(1.5, 0, 0, 0, "cm") ) ) +
  #     cowplot::draw_image(image = readPNG(paste0(dir_img, spp_file, ".png")),
  #                         x = .77, y = .35, # top right
  #                         # x = 0.09, y = -.35, # lower left
  #                         hjust = 0, vjust = 0,
...
```{r, fig-[spp]-population-timeseries, eval = (report_title != "nbspres")}
...
table_raw <- table_raw0 %>% 
  dplyr::rename(ci_up = population_up, 
                ci_dw = population_dw) %>% 
  dplyr::group_by(year, survey_definition_id, SRVY, SRVY_long) %>% 
...
## fig-[spp]-biomass-population-timeseries-nbspres

```{r, fig-[spp]-population-timeseries-nbspres, eval = (report_title == "nbspres")}

nickname <- paste0("fig-", jj, "-", spp_file, "-2-timeseries-nbspres")
height <- 4
legend_font_size <- 10
y_long = "Biomass"
unit = "metric tons"
...
legend0 <- temp00(table_raw0 = table_raw, 
                  var = "biomass_mt", 
                  maxyr = maxyr, 
                  compareyr = compareyr, 
                  unit = unit, 
...
if ("EBS" %in% unique(table_raw$SRVY)) {
  figure_ebs <- temp00(table_raw0 = table_raw %>% 
                         dplyr::filter(SRVY == "EBS"), 
                       var = "biomass_mt", 
                       maxyr = maxyr, 
                       compareyr = compareyr, 
...
if ("NBS" %in% unique(table_raw$SRVY)) {
  figure_nbs <- figure_bio <- temp00(table_raw0 = table_raw %>% 
                                       dplyr::filter(SRVY == "NBS"), 
                                     var = "biomass_mt", 
                                     maxyr = maxyr, 
                                     compareyr = compareyr, 
...
figure <- cowplot::plot_grid(figure_ebs, figure_nbs, legend0, 
                             labels = c(stringr::str_to_title(SRVY11), ""), 
                             hjust = -.75, vjust = 3,  
...
header <- paste0("Timeseries of ", spp_print, 
                 ...
                 ifelse(length(table_raw0$`Bering Sea\nSurvey`) == 1, paste0(tolower(table_raw0$`Bering Sea\nSurvey`), " Bering Sea"), SURVEY), 
                 " shelf bottom trawl survey", 
                 ifelse(length(table_raw0$`Bering Sea\nSurvey`) == 1, "", plural_surveys),". ")
