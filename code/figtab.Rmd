...
for (i in 1:nrow(comb)) {
  ...
  header <- paste0("Total taxon biomass estimates (t), ± 95% confidence limits (CL), and biomass estimates by stratum for ", 
                   ...
                   maxyr, " ", 
                   comb$srvy_long[i],
                   " survey. The 'proportion' column represents the total estimated biomass divided by the total estimated ", 
                   # ifelse(comb$taxon[i] =="invert", "invertebrate", "fish") ,
                   "total estimated biomass (", text_list(formatC(x = total$total, digits = 0, format = "f", big.mark = ",")),
                   " t).")
...
for (i in 1:c(nrow(haul_cruises_maxyr))) {
  ...
  header <- paste0(
    ...
    ifelse("-" %in% table_print0$change, paste0("Percent changes denoted with '-' indicate that less than 100 individuals were caught on the survey",plural_surveys," in ", compareyr," or ",maxyr, ". "), "")) 
