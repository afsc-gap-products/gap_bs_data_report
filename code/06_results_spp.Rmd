...
![](`r here::here("img", paste0(spp_file, ".png"))) `)
...
```{r tab-stats-{{jj}}, eval = (report_title == "community")}
nickname <- paste0("tab-stats-", spp_file)
no_cap <- TRUE
```

```{r tab-stats-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), eval = FALSE}
```

```{r tab-bio-line-{{jj}}, eval = (report_title == "community")}
nickname <- paste0("fig-", jj, "-", spp_file, "-biomass-ci")
```

```{r tab-bio-line-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), eval = (report_title == "community")}
```
