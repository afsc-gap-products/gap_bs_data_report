...
str0_coverage <- ifelse(SRVY == "NEBS", 
                   # NBS + EBS
                   paste0("The survey covered the eastern Bering Sea continental shelf (bottom depths between 20 and 200 meters) from the Alaska mainland coast to the U.S.-Russia Maritime Boundary between the Alaska Peninsula and the Bering Strait. "), 
                   # EBS
                   paste0("The survey covers the Bering Sea continental shelf (bottom depths between 20 and 200 meters) from the Alaska coastline to the U.S.-Russia Maritime Boundary between the Alaska Peninsula and roughly 62°N. "))
...
In `r maxyr`, the Resource Assessment and Conservation Engineering (RACE) Division of the National Marine Fisheries Service’s (NMFS) Alaska Fisheries Science Center (AFSC) conducted the `r str0_srvys`. `r paste0(str0_coverage, str0_stations, str0_temperature, str0_biomass)`This report compares the distribution and relative abundance of `r xunits(sum(!is.na(report_spp$order[report_spp$taxon == "fish"])))` fish species and `r xunits(sum(!is.na(report_spp$order[report_spp$taxon == "invert"])))` invertebrate taxa with side-by-side maps from the past surveys and `r maxyr` `r SURVEY` shelf bottom trawl survey. `r str0_conclusion`
