...
age_plot <- function(srvy, yrs, agecomp, height, width, nickname, legend_font_size, report_title0) {
  ...
  if (!(nrow(table_raw) == 0)) {
      ...
      if (report_title0 == "community") {
        ...
        header <- paste0("Total population estimates at age for ",
                         spp_print, ifelse(is.na(spp_sci), "", 
                                           paste0(" (",spp_sci,")")), 
                         " calculated from ",
                         range_text(yrs)," ",
        ...
        header <- paste0("Total abundance-at-age estimates of ",
                         ...
                         "Age distributions are scaled up to the total estimated population size. Note that otoliths are not collected for each species each year and require, occassionally, years to process after collection. ")
      }
