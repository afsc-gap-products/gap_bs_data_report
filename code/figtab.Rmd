...
temp <- readr::read_csv(paste0(dir_out_rawdata, "/special_projects.csv"))
...
header <- paste0("Time series of mean survey surface and near-bottom temperatures in the eastern Bering Sea (EBS) weighted by stratum based on expendable bathythermograph casts or digital dataloggers attached to the headrope during the eastern Bering Sea bottom trawl surveys from 1982 to ", maxyr, ". The 1982-1987 means (triangles) are based on Strata 10-62 (see Figure ", crossref(list_obj = list_figures, nickname = 'fig_sampled_survey_stations', sublist = 'number'),") and the 1988-", maxyr, " means also include Strata 80 and 92. The dashed lines represent the grand stratum-area weighted mean water temperatures for 1982-", maxyr, ".")

table_raw <- temps_avg_yr

st_col <- nmfspalette::nmfs_cols("dkgray") # nmfs_cols("medgreen")
bt_col <- nmfspalette::nmfs_cols("supdkgray") # nmfs_cols("processblue")

figure <- ggplot(data = table_raw) +
  # surface temperature
  geom_line(aes(x = year, y = st), 
            color = st_col, size = 1) +
  geom_point(aes(x = year, y = st), 
             color = st_col, fill = st_col, 
             pch = 19, size = 2) +
  geom_hline(yintercept = table_raw$st_mean[1], 
             linetype="dashed", color = st_col) + 
  annotate("text", x = 2010, y = 8, label = "Surface", col = st_col) + 
  # bottom temperature
  geom_line(aes(x = year, y = bt), 
            color = bt_col, size = 1) +
  geom_point(aes(x = year, y = bt), 
             color = bt_col, fill = bt_col, 
             pch = 24, size = 3) +
  geom_hline(yintercept = table_raw$bt_mean[1], 
             linetype="dashed", color = bt_col) +
  annotate("text", x = 2010, y = 3, label = "Bottom", color = bt_col) + 
  ...
  scale_x_discrete(
    name = "Year", 
    breaks = seq(from = min(table_raw$year), to = max(table_raw$year), by = 5)) +
  #set legend position and vertical arrangement
  theme(#plot.background = element_rect(fill = "white"), 
...
for (i in 1:2) { # two cases, above and below
  ...
  figure <- plot_temps_facet(
    rasterbrick, key.title = "Bottom Temperature (°C)", reg_dat)
...
width = 9
  ...
  rasterbrick <- coldpool:::ebs_bottom_temperature
  ...
  colnames(temp_df) <- c(temp1, "latitude", "longitude")
  table_raw <- temp_df %>% 
