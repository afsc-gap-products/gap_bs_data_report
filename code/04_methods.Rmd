...
load(file = paste0(dir_out_figtab, "tab-oto-collection-scheme.Rdata")) %>% # creates object called 'obj'
...
Estimates of biomass, population, and size structure of fishes and invertebrate species were calculated from `r text_list(SRVY1)` survey data.  Standard sampling procedures used are described in detail by @RN939 and @RN933. A brief description of the procedures used in the analysis of Bering Sea survey data is described below. Some species were grouped by family for catch data analysis because of their limited commercial value or an inability to identify to lower taxonomic level while in the field. 
...
temp <- obj$raw %>% 
  ...
  dplyr::mutate(SRVY = dplyr::case_when(
    ...
    SRVY == "EBS" ~ "uniquely the EBS",
    SRVY == "NBS" ~ "specifically the NBS",
    TRUE ~ SRVY
  )) %>% 
  dplyr::arrange(-n)
