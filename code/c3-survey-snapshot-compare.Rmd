...
str0 <- paste0(str0, 
  ...
  " stations were completed (out of ",
  haul_cruises_maxyr1$stations_avail,
  ...
  "Bottom depths ranged from ",
  round(min(haul_maxyr$depth_m[haul_maxyr$srvy == srvy1[i]])/divmforft, digits = 1)," ft (", 
  round(min(haul_maxyr$depth_m[haul_maxyr$srvy == srvy1[i]]), digits = 1)," m) to ",
  round(max(haul_maxyr$depth_m[haul_maxyr$srvy == srvy1[i]])/divmforft, digits = 1)," ft (",
  round(max(haul_maxyr$depth_m[haul_maxyr$srvy == srvy1[i]]), digits = 1) , " m). "
  
  # "In comparison, the total ", srvy11[i], 
...
names(temp) <- gsub(x = names(temp), pattern = "ovary samples", replacement = "observations")
...
temp1 <- dplyr::bind_rows(temp_ebs, temp_nbs) |>
  ...
  # I defined "no marked change" in < 10 and > -10% change to get me through the code. What number should it be?
  dplyr::mutate(
    change_case = dplyr::case_when(
      change0 < -10 ~ "decreased",
      change0 > 10 ~ "increased",
      TRUE ~ "" ),
    ...
    change_case1 = dplyr::case_when(
      change0 < -10 ~ "decreases",
      change0 > 10 ~ "increases",
      TRUE ~ NA
    )) |>
...
for (i in 1:length(srvy1)) {
  ...
  str0 <- paste0(str0, 
                 paste0(
                   ...
                   ifelse(i == 1, " (± 10% change from the previous survey year). ", ". "),
...
`r str0` Invertebrates (i.e., shrimps, sea squirts, sea stars, jellyfish, crabs, and urchins) made up `r text_list(paste0(temp3$invert_perc, "% of the biomass in ", temp3$year))`. 
