...
header <- paste0("Map of ",maxyr," ", SURVEY," survey sampling stations. The areas enclosed withinthe dark and light gray lines contain the eastern and northern Bering Sea shelf stations, respectively. The dots within each area indicate station locations. Filled dots are completed stations; unfilled dots represent planned stations that were not sampled in ", maxyr, ".") 
...
for (case in c("bottom", "surface")) {
  for (case_yr in case_yr0) {
    ...
    nickname <- paste0("fig-temperature-", 
                       ifelse(case=="bottom", "bt", "st"), 
                       paste0("-", case_yr))
...
for (case in c("bottom", "surface")) {
  for (case_yr in case_yr0) {
    ...
    nickname <- paste0("fig-temperature-", 
                       ifelse(case=="bottom", "bt", "st"), 
                       paste0("-", case_yr), "-comm")
    a <- temp(yrs, row0, case, temperature_zscore = FALSE)
    figure <- a$figure
    header <- a$header
...
table_raw <- readr::read_csv(file = paste0(dir_out_rawdata, "glossary-of-terms.csv")) %>% 
