...
list_figures1 <- data.frame(nickname = 
                              gsub(pattern = "_", replacement = " ", 
                                   gsub(pattern = "fig_pres_", replacement = "", x = names(list_figures), 
                                        fixed = TRUE), fixed = TRUE), 
                            header = as.character(lapply(list_figures, `[[`, 'header')), 
                            filename = paste0(dir_out_figures, lapply(list_figures, `[[`, 'filename'), '.png'))
# cap <- c("CPUE")
for (i in 1:length(list_figures1$nickname)) {
  list_figures1$nickname[i] <- gsub(pattern = "cpue", replacement = "CPUE", x = list_figures1$nickname[i])
}
...
for (i in 1:nrow(list_figures1)){
  ...
  #             sep = "", collapse = ""))
    ...
    insert <- paste0(insert, 
                     paste(
"
## ", list_figures1$title[i], "

![", 
# ifelse(grepl(x = list_figures1$nickname[i], pattern = "biomass ") | 
#          grepl(x = list_figures1$nickname[i], pattern = "abundance "), 
#        paste0("*", list_figures1$header[i], "*"), ""), 
"](",
                     list_figures1$filename[i], ")\n\n", collpase = ""),
"
...
if (report_title == "community") {
  aa<-report_spp1$file_name[report_spp1$plot_idw == TRUE ]
}
