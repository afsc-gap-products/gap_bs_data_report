...
```{r tab-special-projects-EBS-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), tab.lp = abcd}
```

```{r setup-b, include=FALSE, eval = evalTF}
counter_letters <- counter_letters + 1; abcd <- LETTERS[counter_letters]
# knitr::opts_chunk$set(tab.cap.pre = abcd)
```

```{r tab-app-taxa-encountered-NBS, eval = evalTF, tab.lp = abcd}
...
```{r tab-special-projects-NBS-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), tab.lp = abcd, eval=evalTF}
```

```{r setup-c, include=FALSE, eval = evalTF}
counter_letters <- counter_letters + 1; abcd <- LETTERS[counter_letters]
# knitr::opts_chunk$set(tab.cap.pre = abcd)
```
...
```{r tab-app-principal-fish-EBS-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), tab.lp = abcd}
```

```{r setup-d, include=FALSE, eval = evalTF}
counter_letters <- counter_letters + 1; abcd <- LETTERS[counter_letters]
knitr::opts_chunk$set(tab.cap.pre = abcd)
```
...
```{r tab-app-principal-fish-NBS-child, child = paste0(dir_code, "_child_report_figtab.Rmd"), tab.lp = abcd, eval=evalTF}
```
