...
```{r fig-surveyphoto, eval = FALSE}
# for community highlights

nickname <- "fig-surveyphoto"

header <- readLines(con = paste0(dir_out_rawdata, "/0-surveyphoto.Rmd"))

height <- 5 # ifelse(srvy == "NEBS", full_page_portrait_height, 6)
width <- full_page_portrait_width

figure_print <- figure_print <- 
  cowplot::ggdraw() +
  cowplot::draw_image(readPNG(paste0(dir_out_rawdata, "0-surveyphoto.png")) )
...
header <- paste0("Station map of the ",maxyr," ",SURVEY," survey", plural_surveys, ". Dark and light gray outlines denote the eastern and northern areas, respectively. Dots show station locations: filled dots indicate completed stations, and unfilled dots indicate planned stations not sampled in ", maxyr, ".")
...
header <- paste0("Station map of ",maxyr," ",SURVEY," survey", plural_surveys," sampling stations. The areas enclosed within the dark and light gray lines contain the eastern and northern Bering Sea stations, respectively. The dots within each area indicate station locations. Filled dots are completed stations; unfilled dots represent planned stations that were not sampled in ", maxyr, ".")
...
```{r fig-trawl-gear-blueprint}
header <- paste0("Schematic blueprint of the 83-112 eastern otter trawl gear used during the ", 
                 maxyr ," ", SURVEY ," survey", 
                 ifelse(length(srvy1)>1, "s", ""), ".")
footnote<- NA
nickname <- "fig-trawl-gear-blueprint"
...
figure_print <- 
  cowplot::ggdraw() +
  cowplot::draw_image(readPNG(paste0(dir_img, "E83-112_blueprint.png")) )

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-trawl-gear-diagram-label}
# for community highlights
header <- paste0("Diagram of the 83-112 Eastern otter trawl net with labeled parts.")
nickname <- "fig-trawl-gear-diagram-label"
height <- 1.9
width <- full_page_portrait_width
bg <- 'transparent'
...
figure_print <- 
  cowplot::ggdraw() +
  cowplot::draw_image(readPNG(paste0(dir_img, "E83-113_solid_label.png")), scale = 0.9 )

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-trawl-gear-diagram-nolabel}
# for community highlights
header <- paste0("Diagram of the 83-112 Eastern otter trawl net.")
...
figure_print <- 
  cowplot::ggdraw() +
  cowplot::draw_image(readPNG(paste0(dir_img, "E83-113_solid_nolabel.png")), scale = 0.9 )
...
temp <- stratum |> 
  dplyr::mutate(depth_min_m = ifelse(depth_min_m  == 1, 0, depth_min_m ), 
                area_name = paste0(gsub(replacement = "", x = area_name, pattern = "Domain") , 
...
header <- paste0("Otolith collection types and target counts during the ", 
                 maxyr, " ", 
                 text_list(haul_cruises_maxyr$srvy_long),
                 " survey", ifelse(length(srvy1)>1, "s", ""),".")
...
```{r tab-special-projects-data}
nickname <- "tab-special-projects-data" 
...
header <- paste0("Special projects and collections undertaken during the ",
                 maxyr, " ", 
                 text_list(haul_cruises_maxyr$srvy_long), 
                 " survey, sorted by principal investigator and agency.")
...
header <- "Average surface and bottom temperatures (°C) and time series average (dashed lines) on the eastern Bering Sea shelf (left) and northern Bering Sea (right), based on data collected during standardized summer surveys."
...
plot_mean_temperatures <- function(maxyr, srvy){
  ...
  header <- paste0("Average summer surface and bottom (points and solid lines), and time series average (dashed lines) surface and bottom temperatures (°C) for the eastern Bering Sea shelf",
                   ifelse(srvy == "NEBS", 
                          paste0(" and northern Bering Sea, based on data collected during standardized summer surveys (right)"), ""), 
                   ". ")
...
nickname <- "fig-mean-temperature-ebs"
...
nickname <- "fig-mean-temperature-nebs"
...
header <- paste0("Annual extent of the summer cold pool, based on observations from the eastern Bering Sea survey. The extent of the cold pool is shown as a percentage of the total survey area.",
                 ifelse(srvy == "NEBS", 
                        paste0(" Note that the northern Bering Sea is not included here.") , ""),
...
m <- specimen_maxyr |> 
  dplyr::filter(!is.na(species_code) & !is.na(maturity)) |>
  dplyr::select("srvy", species_code) |> 
  dplyr::group_by("srvy" = srvy, species_code) |> 
  dplyr::summarise(maturity_samples = as.numeric(n())) |> 
  dplyr::ungroup()
...
temp <- readxl::read_excel(path = paste0(dir_out_rawdata, "/other-field-collections.xlsx"), 
                           ...
                           skip = 1) |>
  dplyr::filter(year == maxyr) |> 
  dplyr::select(-count_fecundity_and_maturity_ovary_samples) |> 
...
table_raw0 <- 
  ...
  dplyr::mutate(dplyr::across(where(is.numeric), \(x) replace(x, is.na(x), 0)), 
...
table_raw0$common_name0[table_raw0$common_name0 == "ZZZTotal"] <- "Total"
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0("Biological data collected during the ", maxyr, " ", 
                   ...
                   " survey. ") # See \\@SAPcrab", maxyr, " for crab collection summaries. 
  ...
  names(table_raw)[!(names(table_raw) %in% c("srvy", "common_name", "common_name0"))] <- 
    (gsub(pattern = "_",
          ...
          fixed = TRUE))
  names(table_raw)[names(table_raw) == "Length\nmeasurements"] <- 
    "Lengths"
  names(table_raw)[names(table_raw) == "Otolith\nage\nstructure\nsamples"] <- 
    "Otolith age structures"
  names(table_raw)[names(table_raw) == "Pathobiology\nblood\nsamples"] <- 
    "Pathobiology blood samples"
  names(table_raw)[names(table_raw) == "Genetic\nfin\nclip\nsamples"] <- 
    "Genetic fin clips"
  names(table_raw)[names(table_raw) == "Fecundity\nand\nmaturity\novary\nsamples"] <- 
    "Maturity samples"
  names(table_raw) <- gsub(x = names(table_raw), pattern = "\n", replacement = " ")
  ...
  table_raw00[is.na(table_raw00) | table_raw00 == "0"] <- "-"
  
  table_print <- table_raw00 |> 
    flextable::flextable() |>
    flextable::set_header_labels(
      common_name = stringr::str_to_title(haul_cruises_maxyr$srvy_long[i])) |> 
    flextable::bold(i = which(table_raw00$common_name == "Total")) |>
    flextable::valign(valign = "top") |>
    theme_flextable_nmfstm(pgwidth = full_page_portrait_width) |>
    flextable::align(j = 2:ncol(table_raw00), 
                     align = "right", part = "all")  |>
    flextable::width(width = ((full_page_portrait_width-1.5)/(ncol(table_raw00)-1)), unit = "in") |>
    ...
    y = biomass |> 
      ...
      dplyr::filter(stratum == 999 & 
                      ...
                      dplyr::mutate(year = dplyr::case_when(
                        year == maxyr ~ "maxyr",
                        year == (compareyr["EBS"]) ~ "EBS_compareyr",
                        year == (compareyr["NBS"]) ~ "NBS_compareyr"
                      )) |>
...
if ("NBS_compareyr" %in% names(table_raw0)) {
table_raw0 <- table_raw0 |>   
  dplyr::mutate(
    change_NBS = pchange(start = NBS_compareyr, end = maxyr, value_only = TRUE) )
}

table_raw0 <- table_raw0 |>   
  dplyr::mutate(
    change_EBS = pchange(start = EBS_compareyr, end = maxyr, value_only = TRUE),
    # change = pchange(start = compareyr, end = maxyr, value_only = TRUE),
    change0 = change_EBS, # change,
    change = paste0(formatC(x = change0, big.mark=",", digits = 1, format = "f"), "%"),
...
table_print0 <- table_raw0 |> 
  ...
  dplyr::mutate(
    dplyr::across(dplyr::starts_with("change"), formatC, big.mark=",", digits = 0, format = "f"), 
    dplyr::across(dplyr::starts_with("change"), gsub, pattern = "NA%", replacement = ""), 
  ...
  dplyr::relocate(print_name, maxyr, dplyr::starts_with("compareyr")) |> 
...
table_print0 <- dplyr::bind_rows(
  ...
  names(table_print0)[ names(table_print0) == "maxyr"] <- maxyr
  names(table_print0)[ names(table_print0) == "EBS_compareyr"] <- compareyr["EBS"]
  if ("NBS_compareyr" %in% names(table_print0)) {
    names(table_print0)[ names(table_print0) == "NBS_compareyr"] <- compareyr["NBS"]
  }

for (i in 1:c(nrow(haul_cruises_maxyr))) {
  
  if (haul_cruises_maxyr$srvy[i] == "NBS") {
    yrs0 <- c(maxyr, compareyr[names(compareyr) == "NBS"])
  } else {
    yrs0 <- c(maxyr, compareyr[names(compareyr) == "EBS"])
  }
  ...
  whoismissing <- setdiff(as.numeric(names(table_print0)[grepl(pattern = "2", x = names(table_print0))]), yrs)
  
  nickname <- paste0(nickname0, haul_cruises_maxyr$srvy[i]) 
  header <- paste0(
    "Total estimated biomasses (t) in ", range_text(yrs0)," and the percent change between the ",
    compareyr[names(compareyr) == haul_cruises_maxyr$srvy[i]]," and ",maxyr," ", 
    haul_cruises_maxyr$srvy_long[i], 
    ' surveys for predominant fish and invertebrate taxa', 
    ifelse(length(whoismissing) == 0, "", paste0(" (note that the ", haul_cruises_maxyr$srvy_long[i]," was not surveyed in ", whoismissing, ")")),
    '. Taxa are listed in descending order of percent change from ', compareyr[names(compareyr) == haul_cruises_maxyr$srvy[i]],' to ',maxyr,'. ', 
    ifelse("-" %in% table_print0$change, paste0("Percent changes denoted with '-' indicate that fewer than 100 individuals were caught. ", "")))
  ...
  # 
  # names(table_raw)[ names(table_raw) == "maxyr"] <- maxyr
  # names(table_raw)[ names(table_raw) == "compareyr_EBS"] <- compareyr["EBS"]
  # if ("compareyr_NBS" %in% names(table_raw)) {
  #   names(table_raw)[ names(table_raw) == "compareyr_NBS"] <- compareyr["NBS"]
  # }
...
table_raw <- dplyr::bind_rows(biomass_maxyr, biomass_compareyr) |>
  ...
  dplyr::mutate(
    ...
    compareyr = as.character(compareyr),
  ...
  dplyr::filter( # remove all rows where there are cells with less than 5 characters (e.g., for  organisms with small catches that really shouldnt be in this table like snailfish uid)
    if_all(
      everything(),
      ~ is.na(.) | nchar(as.character(.)) >= 5
    )
  ) 
...
h_r2[grepl(pattern = "Year", x = h_r2)] <- toupper(gsub(pattern = "Year ", x = h_r2[grepl(pattern = "Year", x = h_r2)], replacement = ""))

flextable_color_range <- function(ft, dat0, var0, rnge = -9:9) {
  
  x <- as.character(dat0[[var0]])
  
  # extract numeric percent (keeps sign)
  num <- suppressWarnings(
    as.numeric(sub(".*\\(([-0-9.]+)%\\).*", "\\1", x))
  )
  
  # initialize colors
  color0 <- rep("white", length(x))
  
  # apply rules (order matters)
  color0[grepl("\\([0-9.]+%\\)", x, ignore.case = TRUE)] <- positive
  color0[grepl("\\(-", x, ignore.case = TRUE)] <- negative
  color0[num %in% rnge] <- neutral
    ...
    values = eval(parse(
      text = paste0("list(", 
                    text_list(paste0(names(table_raw), " = '", h_r2, "'"), 
                              sep_last = ", ", sep_last2 = ", "), ") " ) ) ) ) |>
  flextable_color_range(dat0 = table_raw, var0 = "biomass_EBS")  |>
  flextable_color_range(dat0 = table_raw, var0 = "population_EBS") |>
...
if (srvy == "NEBS") {
  table_print <- table_print  |> 
    flextable_color_range(dat0 = table_raw, var0 = "biomass_NBS")  |>
    flextable_color_range(dat0 = table_raw, var0 = "population_NBS") 
}

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r tab-biomass-pchange-alt-pres-[srvy]}

nickname0 <- "tab-biomass-pchange-alt-pres-"
header <- paste0("Biomass and abundance change for species in ", 
                 maxyr, " and ", compareyr," ", SURVEY, 
                 " survey.")
...
table_raw <- dplyr::bind_rows(biomass_maxyr, biomass_compareyr) |>
  ...
  # calculate pchange
  dplyr::mutate(year0 = ifelse(year == maxyr, "maxyr", "compareyr")) |> 
  tidyr::pivot_longer(cols = c("biomass", "population"), names_to = "x1000", values_to = "val") |> 
  dplyr::select(-year) |> 
  tidyr::pivot_wider(id_cols = c("common_name", "x1000", "srvy"), names_from = year0, values_from = val) |> 
  dplyr::mutate(x1000 = stringr::str_to_sentence(x1000), 
                `% Change` = pchange(start = compareyr, end = maxyr, value_only = TRUE), 
                maxyr = maxyr/1000, 
                compareyr = compareyr/1000, 
    dplyr::across(dplyr::where(is.numeric), formatC, format = "f", big.mark = ",", digits = 0) 
    ) |>
  dplyr::ungroup()
  # dplyr::filter( # remove all rows where there are cells with less than 5 characters (e.g., for  organisms with small catches that really shouldnt be in this table like snailfish uid)
...
names(table_raw)[names(table_raw) == "maxyr"] <- maxyr
names(table_raw)[names(table_raw) == "compareyr"] <- compareyr

flextable_color_range <- function(ft, dat0, var0, rnge = -9:9) {
  
  # x <- as.character(dat0[[var0]])
  x <- as.numeric(dat0[[var0]])
  ...
  # apply rules (order matters)
  color0[x>max(rnge)] <- positive
  color0[x<min(rnge)] <- negative
  color0[x %in% rnge] <- neutral
 ...
 for (ii in 1:length(unique(table_raw$srvy))) {
   
   nickname <- paste0(nickname0, unique(table_raw$srvy)[ii])
   
table_print <- table_raw |> 
  dplyr::filter(srvy == unique(table_raw$srvy)[ii]) |> 
  dplyr::select(-srvy) |> 
  flextable::flextable()  |>  
  theme_flextable_nmfstm(row_lines = TRUE, 
                         pgwidth = full_page_landscape_width) |> 
 ...
 table_print <- table_print |> 
  flextable_color_range(dat0 = table_raw, var0 = "% Change")
...
# ESR/Plan Team Figures

```{r tab-length-samples-pres}

nickname0 <- paste0("tab-length-samples-pres-") 
...
for (i in 1:length(srvy)) {
  ...
  nickname <- paste0(nickname0, srvy[i]) 

# length data - from oracle
table_raw <- lengths |> 
  dplyr::filter(year %in% c(maxyr, compareyr[srvy[i]])) |>
  dplyr::filter(!(species_code %in% c(68580, 68590, 68560))) |>
  dplyr::mutate(year = ifelse(year == maxyr, "maxyr", "compareyr")) |> 
  dplyr::select(srvy, species_code, frequency, year) |> 
      dplyr::distinct() |> 
  ...
  dplyr::left_join(
    spp_info |> 
      ...
      dplyr::distinct(), 
    by = "species_code") |>
  dplyr::filter(!(species_code %in% c(68560, 68580, 68590, 69322, 
                                        69323, 69400, 69310, 68577, 68781) ))  |>  # no crab!
  dplyr::group_by(srvy, common_name, year) |>  
  dplyr::summarise(frequency = sum(length, na.rm = TRUE)) |>
  tidyr::pivot_wider(
...
table_raw <- table_raw |> 
  dplyr::mutate(keep = ifelse(maxyr>=1000, T, F))

table_raw0 <- table_raw |>
  dplyr::filter(keep == F) |>
  dplyr::select(-keep) 

  table_raw_other <- table_raw |>
    dplyr::filter(keep == FALSE) |>
    dplyr::select(-keep) |>
    dplyr::summarise(maxyr_count = sum(maxyr, na.rm = TRUE), 
                     compareyr_n = sum(compareyr>0, na.rm = TRUE),
                     maxyr_n = sum(maxyr>0, na.rm = TRUE),
                     compareyr_count = sum(compareyr, na.rm = TRUE)) |>
    dplyr::ungroup() |> 
    dplyr::mutate(common_name = paste0("Other taxa (",maxyr,": ",maxyr_n,"; ",compareyr,": ", compareyr_n, ")")) |> 
    dplyr::rename(maxyr = maxyr_count, compareyr = compareyr_count) |> 
    dplyr::select(-maxyr_n, -compareyr_n)
...
names(table_print0)[names(table_print0) == "maxyr"] <- maxyr
names(table_print0)[names(table_print0) == "compareyr"] <- compareyr
...
table_print <- table_print0 |> 
  ...
  flextable::align(j = 2:3, align = "right", part = "all") |> 
...
```{r fig-biomass-topspecies-pres-prep}

temp <- function(biomass, srvy0, maxyr){
  
  nickname <- paste0("fig-biomass-topspecies-",tolower(srvy0),"-pres")
  header <- paste0("NOAA bottom trawl survey-estimated biomass\n(thousand mt) top ",srvy0," species through ", maxyr,".")
  
  pal <- c(
    "Arrowtooth flounder" = "#54ADDB", 
    "Pacific ocean perch" = "#A2D269", 
    "Walleye pollock" = "#3D348A", 
    "Pacific halibut" = "#D0D0D0", 
    "Pacific cod" = "#FF8400", 
    "Sablefish" = "#B3EDEF", 
    "Flathead sole" = "#001743", 
    "Yellowfin sole" = "#A2D269", 
    "Alaska skate" = "#D0D0D0", 
    "Northern rock sole" = "#B3EDEF")
  
    if (srvy0 == "EBS") {
    table_raw <- biomass |> 
      dplyr::filter(species_code %in% c(21740, 21720, 10210, 10110, 10130, 471, 10261)) 
    
    # } else if (srvy0 == "AI") {
    # species not decided yet
    
  } else if (srvy0 == "GOA") {
    table_raw <- biomass |> 
      dplyr::filter(species_code %in% c(21740, 21720, 30060, 10110, 10130, 10120, 20510)) 
  }
  
  table_raw <- table_raw |> 
    dplyr::filter(srvy == srvy0) |>
    dplyr::filter(area_id > 1000) |>
    # dplyr::filter(common_name %in% names(pal)) |> 
    dplyr::mutate(
      common_name = paste0(toupper(substr(common_name, 1, 1)), 
                           substr(common_name, 2, nchar(common_name))), 
      biomass_up = biomass_up/1000, 
      biomass_dw = biomass_dw/1000, 
      biomass_mt = biomass_mt/1000)
  
  # order of legend matches biomass of orgs in maxyr
  table_raw0 <- table_raw |> 
    dplyr::filter(year == maxyr) |>
    dplyr::group_by(common_name) |> 
    dplyr::summarise(biom = max(biomass_mt, na.rm = TRUE)) |> 
    dplyr::ungroup() |> 
    dplyr::arrange(-biom) |> 
  ...
  table_raw <- table_raw  |> 
    dplyr::left_join(table_raw0) |> 
    dplyr::arrange(order) |> 
    dplyr::arrange(year) |> 
    dplyr::mutate(common_name = factor(x = common_name, 
                                       levels = common_name, 
                                       labels = common_name, 
                                       ordered = TRUE))
  ...
  figure_print <-
    ggplot2::ggplot(data = table_raw, 
                    mapping = aes(
                      ...
                      fill = common_name, 
                      color = common_name)) +
    ggplot2::geom_ribbon(alpha = 0.35, color = "transparent") +
    ggplot2::geom_path(linewidth = rel(1.01)) +
    ggplot2::ggtitle(label = paste0(maxyr, " ", srvy0, " NOAA Bottom Trawl Survey"), 
                     subtitle = paste0("Survey-estimated biomass of top species")) +
    ggplot2::scale_x_continuous(name = "Year",
                                labels = scales::label_number(accuracy = 1, big.mark = ""))  +
    ggplot2::scale_y_continuous(name = "Biomass (thousand mt)", 
                                expand = c(0, 0),
                                breaks = breaks_pretty(), 
                                labels = scales::label_number(big.mark = ","), # accuracy = 1, 
                                limits = c(0, NA)) +
    ggplot2::scale_fill_manual(name = "Species", values = pal) +
    ggplot2::scale_color_manual(name = "Species", values = pal) +
    ggplot2::theme_bw() +
    ggplot2::theme(
...
```{r fig-biomass-topspecies-goa-pres, eval = FALSE}
# eval = (maxyr %% 2) == 1
a <- temp(biomass = biomass_allareas, srvy0 = "GOA", maxyr)
for (jjj in 1:length(a)) { assign(names(a)[jjj], a[[jjj]]) }

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-biomass-topspecies-ai-pres, eval = FALSE}
# , eval = (maxyr %% 2) == 0
a <- temp(biomass = biomass_allareas, srvy0 = "AI", maxyr)
for (jjj in 1:length(a)) { assign(names(a)[jjj], a[[jjj]]) }

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-biomass-topspecies-ebs-pres}
a <- temp(biomass = biomass_allareas, srvy0 = "EBS", maxyr)
for (jjj in 1:length(a)) { assign(names(a)[jjj], a[[jjj]]) }
topten_fig <- figure_print
topten_header <- header

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```


```{r fig-biomass-topspecies-bs-pres-alt}
legend_font_size <- 10
nickname <- paste0("fig-biomass-topspecies-bs-pres-alt")
height <- 6 # full_page_portrait_height-2.5
width <- full_page_landscape_width
header <- topten_header

figure_print <- topten_fig   +
...
```{r  fig-biomass-stackedbar-pres, eval = FALSE}

table_raw <- biomass |> 
  dplyr::select(species_code, year, biomass_mt)


figure_print <- ggplot(table_raw0, aes(x = year, y = biomass_millions, fill = species)) +
  geom_col(width = 0.8) +
  scale_y_continuous(
    name = "Total estimated biomass (millions of tons)",
  ...
  # Custom color palette matching the graphic spectrum
  scale_fill_manual(
    values = c(
      "Pacific ocean perch"   = "#5C82A6",
      "Atka mackerel"         = "#A69279",
      "northern rockfish"     = "#E6A83C",
      "walleye pollock"       = "#FF9E3B",
      "Pacific cod"           = "#F0714E",
      "arrowtooth flounder"   = "#ED6A5A",
      "northern rock sole"    = "#CD8C8C",
      "Pacific halibut"       = "#9BB2C0",
      "Kamchatka flounder"    = "#68BBBA",
      "shortraker rockfish"   = "#58B09C",
      "shortspine thornyhead" = "#65B88A",
      "yellow Irish lord"     = "#A1CA6A",
      "rex sole"              = "#C0C93B",
      "blackspotted rockfish" = "#CBB261",
      "flathead sole"         = "#B88673",
      "sablefish"             = "#A37A8E",
      "Greenland turbot"      = "#BA9BB2",
      "Alaska skate"          = "#BCA08D",
      "giant octopus"         = "#A8A39D",
      "dusky rockfish"        = "#A5B5AF",
      "Dover sole"            = "#A0B3A6",
      "rougheye rockfish"     = "#88998C",
      "Other species"         = "#666666"
    )
  ) +
  theme_classic(base_size = 12) +
  theme(
    # Move legend to bottom and organize into 5 columns
    legend.position = "bottom",
    legend.title = element_blank(),
    ...
    # Axis typography & styling
    axis.title = element_text(size = 12, color = "black"),
    axis.text = element_text(size = 11, color = "black"),
    axis.line = element_line(color = "black", linewidth = 0.6),
    axis.ticks = element_line(color = "black", linewidth = 0.6)
  ) +
  # Controls legend column layout to match the original image
...
tab_taxa_found_ <- function(temp, 
  ...
  for (i in 1:nrow(temp)) {
    ...
    header <- paste0(stringr::str_to_sentence(temp$taxon[i]), 
                     ...
                     " survey listed alphabetically by ", 
  ...
  for (i in 1:nrow(temp)) {
    ...
    if (nrow(table_raw) != 0) { 
      ...
      table_print <- table_raw |>
        ...
        flextable::width(j = "length_mm", width = .55, unit = "in") |> 
...
```{r tab-participants-sci}
nickname <- "tab-participants-sci" 
header <- paste0("Scientific crew of the ",maxyr," ", SURVEY, " bottom trawl surveys. ")
places <- readxl::read_excel(path = paste0(dir_out_rawdata, "/special-projects.xlsx"), 
                             sheet = "affiliations", 
                             skip = 1)
...
table_raw0 <- readxl::read_excel(path = paste0(dir_out_rawdata, "/special-projects.xlsx"), 
                                 sheet = "participants", 
                                 skip = 1) |> 
  dplyr::filter(!(type %in% "vessel")) |> 
  dplyr::filter(year == maxyr) |> 
  dplyr::filter(role != "Survey Coordinator") |> 
  ...
  dplyr::mutate(
    ...
    srvy = factor(x = srvy, 
                  levels = c("EBS", "NBS", "Modernization"), 
                  labels = c("EBS", "NBS", "Modernization"), 
                  ordered = TRUE), 
...
# --- 1. ATTACH THE FOOTNOTES RIGHT TO THE NAMES BEFORE PIVOTING ---
table_prepared <- table_raw0 |>
  dplyr::mutate(
    # Format individual names with standard markdown superscripts (e.g., "John Doe^1^")
    name_with_foot = ifelse(affiliation != "-", 
                            paste0(name, "^", footorder, "^"), 
                            name)
  )

table_collated <- table_prepared |>
  dplyr::group_by(role, key) |>
  dplyr::summarise(
    combined_names = paste(unique(name_with_foot), collapse = ", "),
    .groups = "drop"
  )

role_order <- table_raw0 |>
  dplyr::select(role, order) |>
  dplyr::distinct()

# Pivot wide cleanly into plain text representations containing markdown markers
table_raw <- table_collated |>
  tidyr::pivot_wider(id_cols = role, 
                     names_from = key, 
                     values_from = combined_names,
                     values_fill = "-") |>
  dplyr::left_join(role_order, by = "role") |>
  dplyr::arrange(order) |>
  dplyr::select(-order)

# --- 2. INITIALIZE THE BASE FLEXTABLE LAYOUT ---
aa <- table_raw0 |> 
...
# --- 3. PARSE THE MARKDOWN TO RENDER TRUE SUPERSCRIPTS ---
# ftExtra::colformat_md automatically catches strings like "John^1^" and maps them perfectly
table_print <- table_print |> 
  ftExtra::colformat_md(j = 2:ncol(table_raw), part = "body")

# --- 4. RENDER FOOTER LEGENDS CLEANLY ---
people_footnotes <- table_raw0 |> 
  dplyr::filter(affiliation != "-") |> 
  dplyr::select(affiliation, footorder) |> 
  dplyr::distinct() |>
  dplyr::arrange(footorder)

for (idx in seq_len(nrow(people_footnotes))) {
  ref_num    <- as.character(people_footnotes$footorder[idx])
  text_value <- people_footnotes$affiliation[idx]
  
  table_print <- table_print |> 
    flextable::add_footer_lines(
      values = flextable::as_paragraph(
        flextable::as_sup(ref_num), 
        paste0(" ", text_value)
      )
    )
}
...
```{r tab-participants-vessel}
nickname <- "tab-participants-vessel" 
header <- paste0("Vessel crew of the ",maxyr," ", SURVEY, " bottom trawl surveys. ")

table_raw0 <- readxl::read_excel(path = paste0(dir_out_rawdata, "/special-projects.xlsx"), 
                                 sheet = "participants", 
                                 skip = 1) |> 
  # CHANGED: Now explicitly filtering FOR vessels instead of removing them
  dplyr::filter(type %in% "vessel") |> 
  dplyr::filter(year == maxyr) |> 
  dplyr::left_join(places |> dplyr::rename(affiliation = combined)) |>
  dplyr::arrange(Agency) |>
...
# --- 1. COLLAPSE MULTIPLE NAMES PER CELL ---
table_collated <- table_raw0 |>
  dplyr::group_by(role, key) |>
  dplyr::summarise(
    combined_names = paste(unique(name), collapse = ", "),
    .groups = "drop"
  )
...
# Pivot wide cleanly without footnotes
table_raw <- table_collated |>
  tidyr::pivot_wider(id_cols = role, 
                     names_from = key, 
                     values_from = combined_names,
...
# --- 2. BUILD FLEXTABLE WITHOUT FOOTNOTE LOGIC ---
aa <- table_raw0 |> 
  dplyr::select(key, vessel, col_title) |> 
  dplyr::distinct() 
...
table_print <- table_raw |>
  ...
  flextable::bold(i = 2, part = "header", bold = FALSE) |>
  flextable::vline(j = 5, part = "all") |> 
  flextable::vline(part = "body", border = fp_border(width = .25)) |> 
  flextable::width(
    width = rep_len(x = full_page_landscape_width/ncol(table_raw), 
                    length.out = ncol(table_raw))) |> 
  flextable::merge_h()
...
```{r tab-data-changes}
nickname0 <- "tab-data-changes" 

table_raw00 <- read.csv(file = paste0(dir_data, "/str_data_changes.csv")) 
table_raw0 <- table_raw00 |> 
  janitor::clean_names() |> 
  dplyr::arrange(survey_definition_id, table_name_order, operation_type, table_name) |>
  dplyr::mutate(
    Years = ifelse(year_min == year_max, year_min, paste0(year_min, "-", year_max)), 
    survey_definition_id = stringr::str_to_title(survey_definition_id), 
    operation_type = stringr::str_to_title(operation_type), 
    table_name = stringr::str_to_title(table_name), 
    no_recs = formatC(x = no_recs, digits = 0, format = "f", big.mark = ",")
  ) |> 
...
for (i in 1:2) {
  ...
  if (i == 1) {
    header <- paste0("The number of records in ", maxyr," changed in public-facing production tables. ")
    table_raw <- table_raw0 |> 
      dplyr::filter(Years == maxyr) |> 
      dplyr::filter(Action == "Insertions") |> 
      dplyr::select(-Years)
    nickname <- paste0(nickname0, "-maxyr") 
    
  } else if (i == 2) {
    header <- paste0("The number of records in 1982-", 
                     max(table_raw00$year_max[table_raw00$year_max != maxyr]),
                     " changed in public-facing production tables. ")
    table_raw <- table_raw0 |> 
      dplyr::filter(Years != maxyr)
    nickname <- paste0(nickname0, "-notmaxyr") 
  }
