...
A total of `r text_list(paste0(haul_cruises_maxyr$stations_completed, " ", haul_cruises_maxyr$SRVY_long, " stations"))` were successfully sampled in `r maxyr` (Fig. (#fig-survey-sample-stations)). Haul and catch data from successfully trawled stations that were used for analyses in this section are available for download from the Fisheries One Stop Shop [@FOSSAFSCData].

## Ocean Temperatures and the Cold Pool

```{r results-cold-pool-insert, child=paste0(dir_out_rawdata, "/0-cold-pool.qmd")}
```
...
```{r temperature-1-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-2-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-3-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-4-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r fig-mean-temperature-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r fig-cold-pool-area-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```

## Survey Data and Specimen Collections

Specimens collected during the `r maxyr` `r SURVEY` shelf trawl survey`r plural_surveys` are shown in Table`r plural_surveys` (#tab-specimen-samples-ebs) and (#tab-specimen-samples-nbs). Other special collections are listed in Table (#tab-special-projects).

```{r tab-specimen-samples-0}
nickname <- "tab-specimen-samples-"
```

```{r tab-specimen-samples--child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
if (SRVY == "NEBS" & "NBS" %in% temp0$SRVY) {
  ...
  str0 <- paste0( 
    ...
    " did not occur in the northern Bering Sea (Table (#tab-species-composition)). In comparison, ",
    xunits(nrow(temp3[temp3$where == "nbs",])), 
    " of the ",
    xunits(nrow(temp[temp$SRVY == "NBS",])),
    " fish species encountered in the northern Bering Sea were not encountered in eastern Bering Sea (Table (#tab-species-composition)). ")
  ...
  str0 <- paste0(str0, "In ", maxyr, ", ", 
                   ...
                   text_list(
                     paste0(xunits(temp5$n), 
                            ...
                            temp5$including, ")"), 
                     sep = "; "),
                   " (Table (#tab-species-composition)).")
}

str0 <- gsub(pattern = ") (", replacement = "; ", x = str0, fixed = TRUE)
```

A total of `r xunits(temp1$species_code)` different fish species representing `r xunits(temp1$family)` families and `r xunits(temp1$genus)` genera were identified during the `r maxyr` `r SURVEY` survey`r plural_surveys` (Appendix Table`r plural_surveys` A (#tab-app-taxa-found-EBS-fish)`r ifelse(plural_surveys == "s", " and B (#tab-app-taxa-found-NBS-fish)", "")`). In `r maxyr`, `r text_list(paste0("the ", temp0$SRVY_long, " survey recorded ", xunits(temp0$species_code), " total taxa, of which ", xunits(temp0$species), " were identified to the species level"))`. The remaining fish taxa in each survey area were identified to the genus level or higher. `r str0`
...
`r stringr::str_to_sentence(numbers2words(temp1$species_code))` different invertebrate taxa representing `r xunits(temp1$phylum)` phyla were identified during the `r maxyr` `r SURVEY` survey`r plural_surveys` (Appendix Table`r plural_surveys` A (#tab-app-taxa-found-EBS-invertebrate)`r ifelse(plural_surveys == "s", paste0(" and B (#tab-app-taxa-found-NBS-invertebrate)"), "")`). In `r maxyr`, `r text_list(paste0(xunits(temp0$species_code), " invertebrate taxa ", "were recorded in the ", temp0$SRVY_long, " survey",  ", of which ", xunits(temp0$species), " were identified to the species level"))`. The remaining invertebrate taxa `r ifelse(plural_surveys == "s", "in each survey area ", "")`were identified to the genus level or higher. The lack of species level identifications among invertebrates was due to a variety of factors that are outlined in @RN934 and @Stevensonetal2016.
...
```{r tab-species-composition-child, child = paste0(dir_code, "_child_report_figtab.qmd"), eval = (SRVY == "NEBS" & "NBS" %in% haul_cruises_maxyr$SRVY)}
```
...
for (ii in 1:length(temp2$SRVY)) {
  ...
  str00 <- paste0(str00, 
                  ...
                  paste0(temp2$SRVY_long[ii], 
                         ...
                         temp2$perc_fish[ii], 
                         "; Table (#tab-biomass-est-",  temp2$SRVY[ii],
                         "-fish)) was ", str000,  
                         temp2$case[ii],
                         " invertebrates (", 
...
if (SRVY == "NEBS" & "NBS" %in% haul_cruises_maxyr$SRVY) {
  ...
  str0 <- paste0(
    ...
    " of the total biomass, respectively), and the northern Bering Sea (",
    text_list(
      paste0(round(x = temp1$prop_NBS[1:2]*100, digits = 1), "%")), 
    " of the total biomass, respectively; Tables (#tab-biomass-est-EBS-fish) and (#tab-biomass-est-NBS-fish)). ", 
    temp3)
}
```
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  str0 <- paste0(str0, 
                 ...
                 # taxa increased
                 "Taxa that significantly increased in biomass in the ", SRVY11[i], " included ", 
                 text_list(paste0(temp$print_name[temp$case == "increased"],
                                  " (", temp$change[temp$case == "increased"], ")")), 
                 " (Table (#tab-majortaxa-pchange-", SRVY1[i], ")). ",
                 # taxa decreased
...
str0 <- text_list(paste0(
  "The 10 most abundant fish taxa in the ",temp$SRVY_long,
  ...
  " kg/m^2^ per station; Tables (#tab-est-spp-wt) and (#tab-est-spp-num))."), sep = "", sep_last = "", sep_last2 = "")
```
...
```{r tab-biomass-est-EBS-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r tab-biomass-est-NBS-child, child = paste0(dir_code, "_child_report_figtab.qmd"), eval = ("NBS" %in% haul_cruises_maxyr$SRVY)}
```
...
```{r tab-majortaxa-pchange-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r tab-est-spp-wt-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r tab-est-spp-num-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
Plots of the spatial distribution, abundance-at-length estimates, and tables of CPUE (kg/hectare and no./hectare) for `r length(unique(report_spp1$file_name[report_spp1$taxon == "fish"]))` major fish species caught during the `r SURVEY` continental shelf survey are presented in Figs. `r paste0("(#fig-dist-",temp$taxon[1],"-",temp$file_name[1],"-data)")` to `r paste0("(#fig-dist-",temp$taxon[nrow(temp)],"-",temp$file_name[nrow(temp)],"-data)")` and Tables `r paste0("(#tab-est-",temp2$taxon[1],"-",temp2$file_name[1],"-wt)")` to `r paste0("(#tab-est-",temp2$taxon[nrow(temp2)],"-",temp2$file_name[nrow(temp2)],"-num)")`. Differences in estimates and totals may be due to rounding.
