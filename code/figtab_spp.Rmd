    ...
    for (i in 1:2) {
      ...
      if (i == 1){ # CPUE (WT/HA) and BIOMASS
        ...
        if (nrow(table_raw) != 0) { # if none were caught, don't do any of this
          ...
          varbio0<-find_units(unit = "", unt = "", table_raw$varbio[table_raw$varbio != 0])
          ...
          header <- paste0("Mean CPUE by weight",meanwgtcpue0$unit_word,
           ...
           table_print <- table_raw %>% 
            ...
            flextable::width(x = ., width = 1.5, unit = "in", j = "stratum") %>%
        ...
        #                  NMFSReports::text_list(paste0(haul_cruises_maxyr0$SRVY_long, 
        #                                                " shelf (", 
        #                                                haul_cruises_maxyr0$SRVY, "; ",
        #                                                haul_cruises_maxyr0$stations_completed,
        #                                                " stations completed)")),
        ...
        if (nrow(table_raw) != 0) { # if none were caught, don't do any of this
          ...
          varpop0 <- find_units(unit = "", unt = "", table_raw$varpop[table_raw$varpop != 0])
          ...
          table_print <- table_raw %>% 
            ...
            flextable::valign(valign = "top") %>%
            flextable::width(x = ., width = 1.5, unit = "in", j = "stratum") %>%
            NMFSReports::theme_flextable_nmfstm(row_lines = FALSE, font = font0, 
                                                pgwidth = full_page_landscape_width) %>% 
            flextable::padding(x = .,
                               j = 1, i = ~ is.na(SRVY),
                               padding.left = 5) %>%
