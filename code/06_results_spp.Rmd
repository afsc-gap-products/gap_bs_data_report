...
# a <- report_spp1[which(report_spp1$file_name == jj), ]
# a <- report_spp1[which(report_spp1$file_name == "alaska-plaice"), ]
# a <- report_spp1[which(report_spp1$file_name == "wattled-eelpout"), ]

spp_code <- a$species_code
spp_file <- a$file_name[1]
spp_print <- a$print_name[1]
spp_sci <- text_list(unique(a$species_name))
spp_sci <- ifelse(spp_file == "tunicates", "Tunicata", spp_sci) # TOLEDO
spp_sci <- ifelse(spp_file == "neptune-whelks", "*Neptune* spp.", spp_sci) # TOLEDO
# spp_taxon <- a$taxon[1]
...
fig_bt <- "\\@ref(fig:fig-temperature-bt-data)"
fig_st <- "\\@ref(fig:fig-temperature-st-data)"

fig_comb <- paste0("\\@ref(fig:", "fig-combined-", spp_file, ifelse(report_title == "data", "-data", "-comm"), ")")
...
`r ifelse((report_title == "community"), "##", "###")` `r stringr::str_to_title(spp_print)` `r ifelse(is.na(spp_sci), "", paste0("(",spp_sci,")")) `{#`r spp_file`}
...
```{r spp-fig0-{{jj}}, eval = (report_title %in% "community"), fig.width = fig_width, fig.height = fig_height}
cowplot::ggdraw() +
...
if (ncol(temp)>0) {
  str0_lang <- paste0("**", names(temp), "**: ", 
                      ...
                      collapse = ifelse(report_title == "data", "\n\n", " | "))
...
str0 <- paste0("The estimated biomass of ", spp_print)

if ("EBS" %in% temp$srvy) {
  temp0 <- temp[temp$srvy == "EBS", ]
  str0 <- paste0(str0, " in the eastern Bering Sea ")
  
  case <- dplyr::case_when(
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "Not caught") ~ 
      paste0("was ", temp0$`Biomass`), # , " t"
    temp0$catch_min < 100 ~ 
      paste0(tolower(strsplit(x = temp0$`Observed Catch Totals`, split = " ", fixed = TRUE)[[1]][1]), 
             " to ", temp0$`Biomass`), 
             # ", extrapolated from ", 
             # gsub(pattern = paste0(" in ", maxyr), 
             #      replacement = " caught", 
             #      x = strsplit(x = temp0$`Observed Catch Totals`, split = " to ", TRUE)[[1]][2]) 
             # ), 
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "decrease") ~ 
      paste0("decreased by ", strsplit(split = " ", x = temp0$`% Change\nin Biomass`)[[1]][1]), 
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "increase") ~ 
      paste0("increased by ", strsplit(split = " ", x = temp0$`% Change\nin Biomass`)[[1]][1]),
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "no change") ~ 
      "did not notably change"
  )
  
    case_below100 <- ifelse(temp0$catch_min < 100,  
      paste0("These estimates were extrapolated from ", 
             (gsub(pattern = paste0(" in ", maxyr), 
                  replacement = " caught", 
                  x = strsplit(x = temp0$`Observed Catch Totals`, split = " to ", TRUE)[[1]][2])), 
             " during the ", tolower(temp0$`Bering Sea\nSurvey`)," Bering Sea survey. "),
      "")
    
  case_transition_ebs <- dplyr::case_when(
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "Not caught") ~ "", 
    grepl(x = temp0$`Observed Catch Totals`, pattern = "Decreased") & temp0$catch_min < 100 ~ "decrease", 
    grepl(x = temp0$`Observed Catch Totals`, pattern = "Increased") & temp0$catch_min < 100 ~ "increase",
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "decrease") ~ "decrease", 
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "increase") ~ "increase",
    grepl(x = temp0$`% Change\nin Biomass`, pattern = "no change") ~ "no change"
  )
  
  str0 <- paste0(str0, 
                 case, 
                 ifelse(temp0$catch_min < 100,
                 paste0(" in ", maxyr), 
                 paste0(" from ", compareyr[names(compareyr) == "EBS"], " to ", maxyr)), 
                 ifelse(report_title == "data", paste0(cite_bio),  ""), 
                 ifelse(spp_print == "tunicates", "", 
                        paste0(", and the population was estimated at ", 
                               temp0$Population," individuals")), 
                 cite_pop, ". ", 
                 case_below100)
}
...
if ("NBS" %in% temp$srvy) {
  ...
  str0 <- paste0(str0, str0_transition, 
                 ifelse(("EBS" %in% temp$srvy), "biomass estimate", ""),
                 " in the northern Bering Sea ", case,  
                                  ifelse(temp0$catch_min < 100,
                 paste0(" in ", maxyr), 
                 paste0(" from ", compareyr[names(compareyr) == "NBS"], " to ", maxyr)), 
                 # " from ", compareyr[names(compareyr) == "NBS"], " to ", maxyr, 
                 ifelse(report_title == "data", cite_bio,  ""), 
                 ifelse(spp_print == "tunicates", "", 
                        paste0(", and the population was estimated at ", 
                               temp0$Population," individuals")), 
                 cite_pop,". ", 
...
if (nrow(temp) == 1 & length(srvy11)==2) { # temp$srvy_long %in% stringr::str_to_title(srvy11)
  str0 <- paste0(str0, 
                 ...
                 " in ", maxyr," (Fig. ", 
                 fig_comb, "). "
                 # ifelse(report_title == "data", fig_dist, fig_comb),"). ")
                 )
}
...
str0 <- paste0(str0, 
                 ifelse(
                   (spp_file %in% c("eulachon", "pacific-capelin", "pacific-herring", "rainbow_smelt")), 
                   paste0(stringr::str_to_sentence(spp_print), " lengths are not collected during the eastern and northern Bering Sea surveys. "), 
                   "") )  
  
str0 <- paste0(str0, 
                 ifelse((spp_print == "tunicates"), 
                        "No population estimate is calculated because this organism is colonial and it is not possible to count individuals. ", 
                        "") )

str0 <- paste0(ifelse((spp_print == "snailfishes"), 
                        "Snailfishes are composed of several species of *Liparis* and *Careproctus* that are difficult to distinguish in the field. ", 
                        ""), 
               str0)
...
```{r spp-text-{{jj}}, child = paste0(dir_out_rawdata, spp_file,".Rmd"), eval=FALSE, results='asis'}
# eval=(spp_text)
```
...
```{r fig-combined-{{jj}}}
nickname <- paste0("fig-combined-", spp_file, 
                   ifelse(report_title %in% "community", "-comm", "-data"))
```

```{r fig-combined-{{jj}}-child, child = paste0(dir_code, "_child_report_figtab.Rmd") }
```
