...
if (report_title == "data") {
    yrs <- sort(nbsyr) # c(compareyr, maxyr)
...
if (report_title == "pres") {
  ...
  if (sum(table_raw$SRVY %in% c("NBS", "EBS"))>0) {
...
table_print <- table_print1 %>% 
  ...
  flextable::width(j = 1:ncol(table_print1), 
                   width = c(ifelse(length(stat$SRVY)==2, 2, 3), 
                             1.75, rep_len(x = 1.25, length.out = (ncol(table_raw)-1)) ))
...
for (i in 1:nrow(temp_bio)) {
  ...
  compareyr0 <- ifelse(temp_bio$year[i] == min(yrs), min(yrs), min(yrs[yrs < temp_bio$year[i]]))
  srvy0 <- temp_bio$SRVY[i]
  temp_bio0 <- ifelse(temp_bio$year[i] == min(yrs), NA, temp_bio$biomass_mt[temp_bio$SRVY == srvy0 & temp_bio$year == compareyr0])
      ...
      "Biomass\n% Change" = dplyr::case_when(
        ...
        TRUE ~ sub(replacement = "", pattern = "an ", 
                   x = gsub(pattern = "a ", replacement = "", 
                            x = pchange(start = temp_bio$biomass_mt[i], 
                                        end = temp_bio0))) ) )
  )
}
...
```{r fig-[spp]-[bio/pop]-pres, eval = (report_title == "pres")}
table_raw0 <- biomass %>%
  dplyr::filter(stratum == 999 & 
                  species_code %in% spp_code) %>% 
  dplyr::select(SRVY, year, species_code, #population_var, biomass_var, 
  ...
  dplyr::group_by(SRVY, year, species_code, SRVY_long, print_name) %>% 
...
temp00 <- function(table_raw0, 
  ...
  b <- table_raw00 %>% 
    ...
    dplyr::mutate(pchange0 = pchange(start = compareyr, end = maxyr, value_only = TRUE), 
                  dplyr::across(dplyr::ends_with("yr"), formatC, format = "f", big.mark = ",", digits = 0), 
                  col = dplyr::case_when(
                    pchange0 > 0 ~ positive, 
                    pchange0 < 0 ~ negative, 
                    TRUE ~ "black"), 
                  # dig = ifelse(pchange0>5, 0, 2), 
                  # pchange = round(x = pchange0, digits = dig), 
                  pchange1 = formatC(x = pchange0, format = "f", big.mark = ",", digits = 2), 
                  pchange2 = formatC(x = pchange0, format = "f", big.mark = ",", digits = 0), 
                  pchange = ifelse(pchange0>5, pchange2, pchange1))
  ...
  for (i in 1:nrow(b)){
    ...
    str00 <- c(paste0(b0$SRVY, ifelse(i == 1, paste0(" ", y_long), "")), 
                # maxyr total
...
```{r, fig-[spp]-biomass-pres, eval = (report_title == "pres")}
height <- 4
nickname <- paste0("fig-", jj, "-", spp_file, "-1-biomass-pres")
table_raw <- table_raw0
figure <- temp00(table_raw0 = table_raw0, 
            var = "biomass_mt", 
            maxyr = maxyr, 
            compareyr = compareyr, 
            unit = "metric tons", 
...
```{r, fig-[spp]-population-pres, eval = (report_title == "pres")}
y_long <- "Population"
unit = "individuals"
unt = ""
height <- 4
