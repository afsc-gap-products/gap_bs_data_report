  ...
  dplyr::left_join(x = reg_dat$survey.grid, 
                   y = dplyr::left_join(x = haul_maxyr, 
                     ...
                     dplyr::mutate(vess_col = dplyr::case_when(
                       (vess_shape == vessel_info$vess_shape[1]) ~ nmfspalette::nmfs_cols("darkgreen"), 
                       (vess_shape == vessel_info$vess_shape[2]) ~ nmfspalette::nmfs_cols("darkblue"))) %>% 
...
figure <- figure + 
  ...
  scale_color_manual(
    name = " ", #"Survey Region",
    values = c(alpha(colour = c(survey_reg_col, 
                                unique(vess$vess_col)), 0.7)),
...
```{r}
# for (i in 1:length(report_species$fish1)){
#   
# }
