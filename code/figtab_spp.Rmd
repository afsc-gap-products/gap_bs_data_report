...
# for (SRVY000 in SRVY1) {

header <- paste0("Summary of environmental variables that ", spp_print, " (", spp_sci, ") have been found in across the ", SRVY000)
...
table_raw <- haul0 %>% 
  ...
  dplyr::select(SRVY, 
                ...
                "Bottom Temperature" = gear_temperature, 
...
table_print <- table_raw %>%
  ...
  flextable::add_header_row(
    values = eval(parse(
      text = paste0(
        "list(", paste0(names(table_raw), " = '",
                           c('Survey region:\nStations sampled:',
                             sort(rep_len(x = paste0(stat$SRVY, "\n", stat$str), 
                                          length.out = length(SRVY1)*length(stat$SRVY)))
                             # paste0(sapply(strsplit(x = names(table_raw_env)[-1], split = "_"), "[[", 2),
                             # "\n",
                             # sort(c(stat$str, stat$str))
                             #        )
                             ),
                           "'", collapse = ", "), ")"))),
    top = TRUE
  ...
  flextable::set_header_labels(
    values = eval(parse(text = 
                          paste0("list(", paste0(names(table_raw_env), " = '", 
                                                 c('', sapply(strsplit(x = names(table_raw_env)[-1], split = "_"), "[[", 1)), "'", collapse = ", "), ")")))
  ) %>% 
...
for (i in 1:nrow(temp_bio)) {
  ...
  # if (temp_bio$year[i] != maxyr) {
  #   show <- (temp_bio$biomass[temp_bio$year == maxyr & temp_bio$SRVY == temp_bio$SRVY[i]]/
  #   temp_bio$total[temp_bio$year == maxyr & temp_bio$SRVY == temp_bio$SRVY[i]])*100
  # }
  ...
  table_raw  <- dplyr::bind_rows(
    ...
    c(
      ...
      "Population\n" = xunits(sum(temp_bio$population[i], na.rm = TRUE), 
...
if (spp_table_cpue) {
  ...
  temp <- biomass_strat %>% 
    ...
    dplyr::select(SRVY, stratum, meanwgtcpue, sdcpuewt, 
                  ...
                  population, sdpop, lowerp, upperp, catcount) %>%
