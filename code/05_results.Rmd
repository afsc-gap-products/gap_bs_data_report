...
### INSERT COLD POOL TEXT HERE WHEN SEAN IS READY

```{r results_cold_pool_insert, child=paste0(dir_out_rawdata, "/0_cold_pool_description.Rmd"), eval = FALSE}
```
...
for (i in 1:nrow(temp0)) {
  a <- list.files(path = dir_out_figtab, 
                  pattern = paste0("fig-dist-fish-",temp0$file_name[i]), 
                  ignore.case = TRUE)
  ...
  temp <- c(temp, 
            paste0(
              ...
              ifelse(length(a)>1, 
                     paste0(" \\@ref(fig:fig-dist-fish-",temp0$file_name[i], "-above) and",
                            " \\@ref(fig:fig-dist-fish-",temp0$file_name[i],"-below)"), 
                     paste0(" \\@ref(fig:fig-dist-fish-",temp0$file_name[i],")") ),
...
for (i in 1:nrow(temp0)) {
  a <- list.files(path = dir_out_figtab, 
                  pattern = paste0("fig-dist-invert-",temp0$file_name[i]), 
                  ignore.case = TRUE)
  a <- a[grepl(pattern = ".rdata", x = a, ignore.case = TRUE)]
  ...
  temp <- c(temp, 
            paste0(
              ...
              " (Fig", ifelse(length(a)>1, "s.", "."),
              ifelse(length(a)>1, 
                     paste0(" \\@ref(fig:fig-dist-invert-",temp0$file_name[i], "-above) and",
                            " \\@ref(fig:fig-dist-invert-",temp0$file_name[i],"-below)"), 
                     paste0(" \\@ref(fig:fig-dist-invert-",temp0$file_name[i],")") ),
              " and Tables \\@ref(tab:tab-estimates-maxyr-invert-",temp0$file_name[i],"-wt) and",
              " \\@ref(tab:tab-estimates-maxyr-invert-",temp0$file_name[i],"-num))"))
}
str0 <- text_list(temp)
```
