...
```{r tab-specimen-samples-, eval = FALSE}
...
s <- foodlab_predprey0 |>
  ...
  dplyr::select(year, cruise, haul, type, hauljoin) |> 
...
table_raw0 <- 
  ...
  dplyr::mutate(
    ...
    print_name = paste0(toupper(substr(print_name, start = 1, stop = 1)), 
                        substr(print_name, start = 2, stop = nchar(print_name))))

table_raw0$maxyr <- unlist(table_raw0[names(table_raw0) %in% maxyr])
table_raw0$compareyr <- unlist(table_raw0[names(table_raw0) %in% max(compareyr)])
table_raw0$compareyr1 <- unlist(table_raw0[names(table_raw0) %in% min(compareyr)])
...
table_raw0 <- table_raw0 |> 
  ...
  dplyr::ungroup()  |>
      dplyr::mutate(
    change2 = pchange(start = compareyr1, end = maxyr, value_only = TRUE), 
    # change = pchange(start = maxyr, end = compareyr, value_only = TRUE), 
    dplyr::across(where(is.character), gsub, pattern = "NA", replacement = ""), 
    change11 = change1,
    change = paste0(formatC(x = change1, big.mark=",", digits = 1, format = "f"), "%") ) |> 
  dplyr::ungroup()  |>
  dplyr::arrange(desc(change1))
}
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  table_print <- table_print |> 
    flextable::set_header_labels(.,
                                 ...
                                 change = paste0("Change (", maxyr, ", ", max(compareyr), ")" ))
  } else {
      table_print <- table_print0  |> 
    dplyr::filter(srvy == haul_cruises_maxyr$srvy[i]) |> 
    dplyr::select(-srvy, -change0) |>
    ...
    flextable::color(color = "red", # https://stackoverflow.com/questions/57474647/italic-and-color-in-an-r-flextable
        ...
        flextable::color(color = "red", # https://stackoverflow.com/questions/57474647/italic-and-color-in-an-r-flextable
                     i = ~ grepl(pattern = "-[0-9]+", x = change), 
                     j = "change2") |> 
    flextable::width(j = "change1", width = 1)  |> 
    flextable::width(j = "change2", width = 1) 
      
  table_print <- table_print |> 
    flextable::set_header_labels(.,
                                 print_name = "Common name", 
                                 dummy = "Scientific name", 
                                 change1 = paste0("Change (", maxyr, ", ", min(compareyr), ")" ), 
                                 change2 = paste0("Change (", maxyr, ", ", max(compareyr), ")" ))
  }
