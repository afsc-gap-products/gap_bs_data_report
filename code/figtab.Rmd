...
m <- specimen_maxyr |> 
  ...
  dplyr::summarise(maturity = as.numeric(n())) |> 
  dplyr::ungroup()
    ...
    y = biomass |> 
      dplyr::filter(stratum == 999 & 
                      ...
                      dplyr::mutate(year = dplyr::case_when(
                        year == maxyr ~ "maxyr",
                        year == (compareyr["EBS"]) ~ "compareyr_EBS",
                        year == (compareyr["NBS"]) ~ "compareyr_NBS"
                      )) |>
...
if ("compareyr_NBS" %in% names(table_raw0)) {
table_raw0 <- table_raw0 |>   
  dplyr::mutate(
    change_NBS = pchange(start = compareyr_NBS, end = maxyr, value_only = TRUE) )
}

table_raw0 <- table_raw0 |>   
  dplyr::mutate(
    change_EBS = pchange(start = compareyr_EBS, end = maxyr, value_only = TRUE),
...
table_print0 <- dplyr::bind_rows(
  ...
  names(table_print0)[ names(table_print0) == "maxyr"] <- maxyr
  names(table_print0)[ names(table_print0) == "compareyr_EBS"] <- compareyr["EBS"]
  if ("compareyr_NBS" %in% names(table_print0)) {
    names(table_print0)[ names(table_print0) == "compareyr_NBS"] <- compareyr["NBS"]
  }
