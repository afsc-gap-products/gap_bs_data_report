...
for (i in 1:length(SRVY00)) {
  ...
  temp3 <- temp33 %>% dplyr::filter(SRVY = SRVY00[i])
  temp4 <- temp44 %>% dplyr::filter(SRVY = SRVY00[i])
  
str0 <- paste0(str0, 
paste0(
  "In the ",SRVY00[i]," between ", compareyr, " and ", maxyr , ", ",
               text_list(paste0(temp4$str0, " experianced ", temp4$change_case, " biomass")),
...
" in biomass included ", temp3$spp_list, collapse = ". "), 
ifelse(i == 1, " (Table \\@ref(tab:tab-majortaxa-pchange-NBS). ", ". ")
) )
