...
if (report_title == "Data Report") {
  ...
  print_n <- FALSE
  SRVY11 <- SRVY1
} else if (report_title == "Community Report") {
  yrs <- nbsyr
  ...
  if (report_title != "Community Report" & SRVY == "NEBS" & length(unique(table_raw$SRVY))>1) {
  ...
  } else if (report_title != "Community Report" & SRVY == "NEBS" & length(unique(table_raw$SRVY))==1) { 
    height <- 3.25 
  }
  ...
  header <- paste0("Total abundance-at-size estimates of ",
                   ...
                   ifelse(print_n, 
                          "Total number of individuals measured during the survey indicated in the upper right corner of each plot.", ""))
  ...
  if (!(pres_img) & report_title == "Community Report") {
...
if (report_title == "Data Report") {
  ...
  if (length(unique(table_raw$SRVY))==1 & SRVY == "NEBS") { # is this species in only 1 of the 2 survey areas (when SRVY == NEBS)?
    ...
    height <- ifelse(reg_dat0$survey.area$SURVEY == "EBS_SHELF" & 
                       report_title == "Data Report", 4.25, 3.5)
  }
  
  if (spp_plot_idw) {
    
    header <- paste0(stringr::str_to_sentence(spp_print), ifelse(is.na(spp_sci), "", paste0(" (",spp_sci,")")), 
                     " distribution and weight CPUE (kg/ha) (kg/ha) from the ",
                     NMFSReports::text_list(yrs), " ", 
