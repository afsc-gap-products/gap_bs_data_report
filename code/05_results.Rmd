...
A total of `r NMFSReports::text_list(paste0(haul_cruises_maxyr$stations_completed, " ", haul_cruises_maxyr$SRVY, " stations"))` were successfully sampled in `r maxyr` (Fig. `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number")`). Haul and catch sample data for successfully trawled stations used in the analyses can be found and downloaded from the Fisheries One Stop Shop (<https://www.fisheries.noaa.gov/foss/f?p=215:200:1099772399154:Mail:NO:::>).
...
str0 <- paste0(
  ...
  " than the time series mean (", round(x = temp$st_mean, digits = 1),"°C; Figs. ", 
  NMFSReports::crossref(list_obj = list_figures, 
                        nickname = c('fig_mean_temperature', 'fig_cold_pool_area', 'fig_bt_temperature_ebs_below','fig_bt_temperature_ebs_above'), 
                        sublist = 'number'),"). ", 
  ifelse(SRVY == "NEBS", 
         paste0(
                ifelse(a$coefficients[2]<0 & a$coefficients[8] <= 0.05, 
                       paste0("In the EBS south of 60°N, surface temperatures increased from east to west across the shelf (Fig. ",NMFSReports::crossref(list_obj = list_figures, 
                                      nickname = 'fig_st_temperature_nebs_surveys', sublist = 'number'),"). "), ""),
                "An average of ", 
                round(x = mean(temp2$surface_temperature), digits = 1),
                "°C was observed in Norton Sound (Fig. ", 
                NMFSReports::crossref(list_obj = list_figures, 
                                      nickname = 'fig_st_temperature_nebs_surveys', sublist = 'number'), 
                ")."), 
         ""))
...
During the last `r length(unlist(temps_avg_yr_abovebelow))` years, `r NMFSReports::range_text(x = temps_avg_yr_abovebelow$below)` were colder than average ("cold stanza"; Fig. `r crossref(list_obj = list_figures, nickname = 'fig_bt_temperature_ebs_below', sublist = 'number')`), while `r NMFSReports::range_text(x = temps_avg_yr_abovebelow$above)` were warmer than average ("warm stanza"; Fig. `r NMFSReports::crossref(list_obj = list_figures, nickname = 'fig_bt_temperature_ebs_above', sublist = 'number')`). `r ifelse((maxyr>2020 & maxyr<(2021+length(unlist(temps_avg_yr_abovebelow)))), "There was no survey in 2020 due to the COVID-19 pandemic.", "")` The highly variable survey bottom temperatures in the EBS shelf are related to the area occupied by the summer cold pool (Fig. `r crossref(list_obj = list_figures, nickname = 'fig_cold_pool_area', sublist = 'number')`). Over the period of the `r haul_cruises_maxyr$yrofsurvey[haul_cruises_maxyr$SRVY == "EBS"]`-year time series, the areal coverage of the summer survey cold pool in the EBS has varied in size from `r NMFSReports::xunits(min(temp$area_km2, na.rm = TRUE))` km^2^ in `r temp$year[temp$area_km2 == min(temp$area_km2, na.rm = TRUE)]` to `r NMFSReports::xunits(max(temp$area_km2, na.rm = TRUE))` km^2^ in `r temp$year[temp$area_km2 == max(temp$area_km2, na.rm = TRUE)]`, respectively comprising `r round(temp$perc[temp$area_km2 == min(temp$area_km2, na.rm = TRUE)], digits = 1)`% to `r round(temp$perc[temp$area_km2 == max(temp$area_km2, na.rm = TRUE)], digits = 1)`% of EBS shelf area (Fig. `r crossref(list_obj = list_figures, nickname = 'fig_cold_pool_area', sublist = 'number')`). In `r maxyr`, the cold pool covered `r formatC(temp$area_km2[temp$year == maxyr], digits = 0, big.mark = ",", format = "f")` km^2^ (`r round(temp$perc[temp$year == maxyr], digits = 1)`%) of the EBS shelf survey area`r temp4`
...
str0 <- paste0("A total of ", NMFSReports::text_list(sep="; ", paste0(temp$nspec, " ", temp$col, " were collected from ", temp$ntaxa, " ", temp$ctaxa, " ", ifelse(temp$ntaxa0==1, "taxon", "taxa"))), ". ")
...
A total of `r xunits(temp1$species_code)` different fish species representing `r xunits(temp1$family_taxon)` families and `r xunits(temp1$genus_taxon)` genera were identified during the `r maxyr` `r NMFSReports::text_list(SRVY1)` survey`r ifelse(length(SRVY1)>1, "s", "")` (`r ifelse(SRVY == "NEBS", "Appendices", "Appendix")` `r NMFSReports::crossref(list_tables, "tab_app_taxa_encountered_*_fish", "number", exact = FALSE)`). In `r maxyr`, `r NMFSReports::text_list(paste0("the ", temp0$SRVY, " survey recorded ", xunits(temp0$species_code), " total taxa, of which ", xunits(temp0$species_taxon), " were identified to the species level"))`. The remaining fish taxa in each survey area were identified to the genus level or higher. `r insert`
...
`r stringr::str_to_title(NMFSReports::numbers2words(temp1$species_code))` different invertebrate taxa representing `r xunits(temp1$phylum_taxon)` phyla were identified during the `r maxyr` `r NMFSReports::text_list(SRVY1)` survey (`r ifelse(SRVY == "NEBS", "appendices", "appendix")` `r NMFSReports::crossref(list_tables, "tab_app_taxa_encountered_*_invertebrate", "number", exact = FALSE)`). In `r maxyr`, `r NMFSReports::text_list(paste0("the ", temp0$SRVY, " survey recorded a total of ", xunits(temp0$species_code), " taxa of which ", xunits(temp0$species_taxon), " were identified to the species level"))`. The remaining invertebrate taxa `r ifelse(SRVY == "NEBS", "in each survey area ", "")`were identified to the genus level or higher. The lack of species level identifications among invertebrates was due to a variety of factors that are outlined in @RN934 and @Stevensonetal2016. Additionally, trawl catchability of small invertebrates is not known.
...
`r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_species_composition", sublist = "res") `
...
if (SRVY == "NEBS") {
  ...
  for (i in 1:length(SRVY1)){
    temp4 <- NMFSReports::text_list(paste0(
                             ...
                             temp0$str0[temp0$SRVY == SRVY1[i]]), sep_last = "")
    temp3 <- paste0(temp3, 
...
str0 <- paste0("Noticeable changes were observed in",ifelse(SRVY == "NEBS", " both", ""), " the ", NMFSReports::text_list(SRVY1)," the benthic communities between ",
               compareyr," and ", maxyr,". ")
...
Plots of the spatial distribution and tables of CPUE (kg/hectare and no./hectare) for `r length(NMFSReports::crossref(list_obj = list_figures, nickname = "fig_idw_cpue_fish_", sublist = "number", exact = FALSE, text = FALSE))` major fish species caught during the `r NMFSReports::text_list(SRVY1)` continental shelf survey are presented in the subsections below (Figs. `r NMFSReports::crossref(list_obj = list_figures, nickname = "fig_idw_cpue_fish_", sublist = "number", exact = FALSE)` and Tables `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_estimates_maxyr_fish_", sublist = "number", exact = FALSE)`). Differences in sums of estimates and totals are due to rounding. Total abundance-at-length and mean length estimates for major fish species are also presented in the subsections below (Figs. `r NMFSReports::crossref(list_obj = list_figures, nickname = "fig_sizecomp_fish_", sublist = "number", exact = FALSE)`). Appendices `r NMFSReports::crossref(list_tables, "tab_app_principal_fish_", "number", exact = FALSE)` contain population estimates by sex and size class for `r nrow(temp0)` fish species.
