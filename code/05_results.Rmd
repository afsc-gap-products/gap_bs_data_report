...
A total of `r xunits(temp1$species_code)` different fish species representing `r xunits(temp1$family)` families and `r xunits(temp1$genus)` genera were identified during the `r maxyr` `r SURVEY` survey`r plural_surveys` (Appendix Table`r plural_surveys` A \@ref(tab:tab-app-taxa-found-EBS-fish)`r ifelse(plural_surveys == "s", " and B \\@ref(tab:tab-app-taxa-found-NBS-fish)", "")`). In `r maxyr`, `r text_list(paste0("the ", temp0$SRVY_long, " survey recorded ", xunits(temp0$species_code), " total taxa, of which ", xunits(temp0$species), " were identified to the species level"))`. The remaining fish taxa in each survey area were identified to the genus level or higher. `r str0`
...
str0 <- text_list(paste0(
  ...
  " kg/m^2^ per station; Tables \\@ref(tab:tab-est-spp-wt) and \\@ref(tab:tab-est-spp-num)). "), sep = "", sep_last = "", sep_last2 = "")
```
