---
output:
  word_document:
    pandoc_args: ["--metadata-file=header.yaml"]
    reference_docx: styles_reference.docx
csl: "../cite/citestyle.csl"
bibliography: "../cite/bibliography_RPack.bib"
nocite: '@*'
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, error = FALSE, message = FALSE)
```

</br>

## Data Sources

```{r}
insert <- readtext2(file = paste0(dir_out_rawdata, "/0_data_sources.docx"), 
                    refcontent = refcontent)
```

`r insert `

## Acknowledgements

```{r}
insert <- readtext2(file = paste0(dir_out_rawdata, "/0_acknowledgements.docx"), 
                    refcontent = refcontent)
```

`r insert `

</br>

## ORCID

```{r}
insert <- readxl::read_xlsx(path = paste0(dir_out_rawdata, "/0_orcid.xlsx")) %>% 
  flextable::flextable() %>%
  NMFSReports::theme_flextable_nmfstm()
```

`r insert `

\newpage


## Works Cited

</br>

### Recent Technical Memorandums

Copies of this and other NOAA Technical Memorandums are available from the National Technical Information Service, 5285 Port Royal Road, Springfield, VA 22167 (web site: [www.ntis.gov](www.ntis.gov)). Paper and electronic (.pdf) copies vary in price. The recent Technical Memoranda produced by the Alaska Fisheries Science Center can be found at  [https://www.fisheries.noaa.gov/resource/publication-database/alaska-fisheries-science-center-technical-memorandums](https://www.fisheries.noaa.gov/resource/publication-database/alaska-fisheries-science-center-technical-memorandums). 

```{r}
insert <- readtext2(file = paste0(dir_out_rawdata, "/0_recent_pubs.docx"), 
                    refcontent = refcontent)
```

`r insert `

</br>


### Works Cited

<!-- Note that when the report is compiled, all of the citations from the rest of the report will naturally come to the end of this page.  -->
