...
```{r fig-dist-[taxon]-[spp], eval = (spp_plot_idw | spp_plot_pa)}
  ...
  nickname <- paste0("fig-", spp_file, "-4dist-pres")
    yrs <- nbsyr
    height <- 5
    row0 <- 1
  } 
...
nickname0 <- paste0("tab-summary-", spp_file)
...
table_raw0 <- biomass %>%
  ...
  dplyr::select(SRVY, year, species_code, taxon, 
                ...
                # upperp, lowerp, 
                population_var, population_count) %>% 
  dplyr::left_join(x = . , 
                   y = haul_cruises_maxyr %>% 
                     dplyr::select(SRVY, SRVY_long),
                   by = "SRVY")  %>% 
...
temp00 <- function(table_raw, 
  ...
  figure0 <- plot_timeseries(dat = table_raw,
                            ...
                            unt = unt,
                            y_long = y_long,
                            error_bar = FALSE, # ifelse(length(spp_code)>1, FALSE, TRUE),
                            spp_print = spp_print, 
...
```{r, fig-population-[spp]-pres, eval = (report_title == "pres")}
y_long <- "Population"
unit = "individuals"
unt = ""
height <- 4
...
table_raw <- table_raw0 %>%
  dplyr::rename(y = population_count, 
                # lower = lowerp, 
                # upper = upperp, 
                var = population_var)
