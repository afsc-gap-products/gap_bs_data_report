...
for (i in 1:nrow(temp0)) {
  load(file = paste0(dir_out_figtab, "tab-biomass-est-", temp0$type[i],"-", temp0$SRVY[i], ".rdata"))
...
if (SRVY == "NEBS") {
  ...
  for (i in 1:length(SRVY1)) {
    load(file = paste0(dir_out_figtab, "tab-biomass-est-fish-", temp0$SRVY[i], ".rdata"))
    temp1 <- obj$raw %>%
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  load(file = paste0(dir_out_figtab, "tab-estimates-maxyr-spp-wt.rdata"))
...
if (length(unique(temp$print_name))==1) {
  ...
  load(file = paste0(dir_out_figtab, "tab-estimates-maxyr-invert-",report_spp1$file_name[report_spp1$print_name == temp$print_name[1]],"-wt.Rdata"))
