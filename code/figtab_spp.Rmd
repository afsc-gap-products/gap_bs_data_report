...
size_plot <- function(srvy, yrs, print_n, height, width, nickname, legend_font_size, report_title0) {
...
temp <- size_plot(srvy, yrs, print_n, height, width, nickname, legend_font_size, report_title0) 
for (jjj in 1:length(temp)) { assign(names(temp)[jjj], temp[[jjj]]) }
res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```
...
## fig-sizecomp-alt-[spp]-prep

```{r fig-sizecomp-alt-[spp]-prep, eval = FALSE}
width <- full_page_portrait_width # 6.5  
header <- ""
legend_font_size <- 12
...
library(ggridges)
  ...
  ggplot2::scale_x_continuous(name = paste0(type, " Length (cm)")) +
...
## fig-sizecomp-alt-[spp]-data

```{r fig-sizecomp-alt-[spp]-data, eval = FALSE}
yrs <- nbsyr
height <- full_page_portrait_height 
print_n <- FALSE
srvy <- SRVY1
nickname <- paste0("fig-sizecomp-alt-", spp_file, "-data")
legend_font_size <- 10
report_title0 <- "data"
...
## fig-sizecomp-alt-[spp]-comm

```{r fig-sizecomp-alt-[spp]-comm, eval = FALSE}
yrs <- nbsyr
height <- full_page_portrait_height 
print_n <- FALSE
srvy <- SRVY1
nickname <- paste0("fig-sizecomp-alt-", spp_file, "-comm")
legend_font_size <- 10
report_title0 <- "community"
srvy <-"NBS"
...
## fig-sizecomp-alt-[spp]-pres

```{r fig-sizecomp-alt-[spp]-pres, eval = FALSE}
srvy <- SRVY1
nickname <- paste0("fig-sizecomp-alt-", spp_file, "-pres")
yrs <- c(maxyr, compareyr)
print_n <- FALSE
height <- 4.5
width <- 5
