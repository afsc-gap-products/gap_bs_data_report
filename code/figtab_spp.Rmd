...
if (spp_plot_sizecomp) {
  ...
  header <- paste0("Total abundance-at-length estimates of ",
                   ...
                   ". Length distributions are expanded to the total estimated population size. ", 
                   ifelse(print_n, 
  ...
  for (case_yr in case_yr0) {
    ...
    if (spp_plot_idw) {
      ...
      figure <- plot_pa_facet(
        ...
        year = "year",
        reg_dat = reg_dat0, 
        key.title = paste0(stringr::str_to_sentence(spp_print), " encounters"),
        row0 = row0, 
    ...
    for (i in 1:2) {
      ...
      header <- paste0(header,
                       ...
                       ifelse(is.na(spp_sci), "", paste0(" (", spp_sci, ")")), 
                              # ifelse(grepl(pattern = "*", x = spp_sci, fixed = TRUE), 
                              # paste0(" (<em>",gsub(pattern = "*", replacement = "", x = spp_sci, fixed = TRUE),"</em>)"), 
                              # spp_sci)
                              # ),
                       " by stratum observed during the ",
                       maxyr," ", text_list(SRVY1), 
                       # text_list(paste0(haul_cruises_maxyr0$SRVY, " (",
