...
temp5 <- 
  ...
  dplyr::left_join(
    x = ., 
    y = list_tables$tab-majortaxa-pchange-comm$raw %>% 
...
temp1 <- list_tables$tab-majortaxa-pchange-comm$raw
...
for (i in 1:length(nbsyr)) {
  ...
  yr <- as.numeric(sort(nbsyr, decreasing = TRUE)[i])
  temp <- temp2$biomass_mt[temp2$year == yr]
