...
dist_plot <- function(yrs, height, nickname, report_title0, col, key_title) {
...
```{r fig-diversity-H-gif}
yrs <- sort(unique(diversity$year))

width <- full_page_portrait_width # 6.5  
height <- full_page_portrait_height # + ifelse(srvy == "EBS", 0, - 1)
nickname <- paste0("fig-diversity-H-gif")
report_title0 <- "gif"
col <- "diversity_H"
key_title <- 'Shannon Diversity Index'

temp <- dist_plot(yrs, height, nickname, report_title0, col, key_title)
for (jjj in 1:length(temp)) { assign(names(temp)[jjj], temp[[jjj]]) }
res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-diversity-D-gif}
yrs <- sort(unique(diversity$year))

width <- full_page_portrait_width # 6.5  
height <- full_page_portrait_height # + ifelse(srvy == "EBS", 0, - 1)
nickname <- paste0("fig-diversity-D-gif")
report_title0 <- "gif"
col <- "diversity_D"
