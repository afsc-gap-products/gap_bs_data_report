...
# temp <- sizecomp |> 
#   dplyr::right_join(data.frame(year = c(compareyr, maxyr, maxyr), 
#                            srvy = c(names(compareyr), names(compareyr)))) |> 
#   dplyr::distinct(year, srvy)

if (srvy == "EBS") {
  yrs <- c(maxyr, compareyr)
  temp <- size_plot(srvy, yrs, lengths, sizecomp, print_n, height, width, nickname, legend_font_size, report_title0) 
  pres_size <- figure_print <- temp$figure_print
  ...
  # EBS 
  if (sizecomp |> dplyr::filter(srvy == "EBS" & year == maxyr & species_code %in% spp_code) |> nrow() > 0) {
    temp <- size_plot(srvy = "EBS", yrs, lengths, sizecomp, print_n, height, width, nickname, legend_font_size, report_title0) 
    pres_size_ebs <- temp$figure_print
  } else {
    pres_size_ebs <- NULL
  }
  
  # NBS 
  if (sizecomp |> dplyr::filter(srvy == "NBS" & year == maxyr & species_code %in% spp_code) |> nrow() > 0) {
    temp <- size_plot(srvy = "NBS", yrs, lengths, sizecomp, print_n, height, width, nickname, legend_font_size, report_title0) 
    pres_size_nbs <- temp$figure_print
  } else {
    pres_size_nbs <- NULL
  }
  
  if (is.null(pres_size_nbs) | is.null(pres_size_ebs)) {
    
    if (is.null(pres_size_nbs)) {
      pres_size <- pres_size_ebs
    } else {
      pres_size <- pres_size_nbs
    }
    
    figure_print <- pres_size <- pres_size + 
      # ggplot2::ggtitle(label = NULL) +
      # ggplot2::annotate("text", label = "NBS", hjust = 0, vjust = 1, size = 4) +  
    ...
    # combine
    figure_print <- pres_size <- 
      cowplot::plot_grid(
        pres_size_nbs + 
          ggplot2::ggtitle(paste0(maxyr, " northern Bering Sea")) +
          # ggplot2::ggtitle(label = NULL) +
          # ggplot2::annotate("text", label = "NBS", hjust = 0, vjust = 1, size = 4) +
          ggplot2::theme(
            ...
            plot.margin=grid::unit(c(.1,.1,.1,.1), "mm") ), 
        pres_size_ebs +
          ggplot2::ggtitle(paste0(maxyr, " eastern Bering Sea")) +
          # ggplot2::ggtitle(label = NULL) +
          # ggplot2::annotate("text", label = "NBS", hjust = 0, vjust = 1, size = 4) +   
          ggplot2::theme(#axis.title.y = element_blank(),
            plot.title = element_text(size = 10, face = "bold"), 
            ...
            legend.position = "none", 
            plot.margin=grid::unit(c(.1,.1,.1,.1), "mm") ) , 
        cowplot::get_legend(pres_size_ebs + 
                              ggplot2::labs(fill = NULL)) , 
        nrow = 3, 
        ncol = 1, 
        rel_heights = c(.3, .3, .05))
    # cowplot::plot_grid(cowplot::get_y(pres_size_ebs), 
    #                    pres_size, 
    #                    nrow = 1, ncol = 2, rel_widths = .1, .9)
  }
  header = ""
}
...
temp <- timeseries_plot(table_raw = table_raw0, table_raw_mean, nickname, height, width, legend_font_size = legend_font_size, haul_cruises)
for (jjj in 1:length(temp)) { assign(names(temp)[jjj], temp[[jjj]]) }
res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```
...
temp <- dplyr::left_join(pres_timeseries_mean, pres_stats)
...
header <- header0 <- paste0(
  ...
  "Top left: ", 
  "Total abundance-at-length estimates by sex encountered during the ", maxyr, " eastern and northern Bering Sea shelf surveys. Length distributions are scaled to the total estimated population size. ", # pres_size_txt, 
  "Bottom right: ", pres_timeseries_txt)

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-combined-[spp]-comm-old, eval = FALSE}

# may be radically updated with next chunk Jan 13, 2026
...
if (exists("comm_size")) {
  comm_size1 <- comm_size + 
    ggplot2::theme(
      ...
      # legend.background = element_blank(), # Make inner legend background transparent if needed
      # legend.key.size = unit(0.5, "cm"),  # Adjust overall key size
      # legend.key.width = unit(0.3, "cm"), # Make them even narrower
      legend.key.height = unit(0.15, "cm"),
...
temp1 <- cowplot::plot_grid(
  ...
  rel_widths = c(1.2,.05,.75)) # 1, .1., .9 worked for presentations

temp2 <- cowplot::plot_grid(
  pres_timeseries + 
    # ggplot2::ggtitle(NULL) + 
    ggplot2::guides(fill = "none") +
    ggplot2::labs(color = "") + 
...
figure_print <- cowplot::ggdraw(figure_print) +
  cowplot::draw_label("A", x = 0.00, y = 0.94, hjust = 0, vjust = 0, size = 16) +
  cowplot::draw_label(ifelse((exists("comm_size")), "C", "B"), x = 0.00, y = 0.32, hjust = 0, vjust = 0, size = 16) +
...
header <- paste0(
  ...
  "from the ", range_text(c(compareyr, maxyr)), " eastern (EBS) and northern (NBS) Bering Sea shelf survey",plural_surveys,
  ...
  "survey", plural_surveys," (points and solid lines, with estimated 95% confidence intervals error bars) and time series mean (dashed lines). ",
  "(",ifelse((exists("comm_size")), "D", "C"),") Photo of ", ifelse(substr(x = spp_print, nchar(spp_print), nchar(spp_print)) == "s", "", ifelse(tolower(substr(spp_print, 1, 1)) %in% c("a", "e", "i", "o", "u"), "an ", "a ")), spp_print, ". ")

res <- knitr::knit_child(text = knitr::knit_expand( file = paste0(dir_code, "_child_save_figtab.Rmd")), quiet = TRUE)
```

```{r fig-combined-[spp]-comm}
...
header <- paste0(
  ...
  ifelse(spp_file %in% c("snow-crab", "blue-king-crab", "red-king-crab"), "", ", with estimated 95% confidence intervals error bars"), ") and time series mean (dashed lines). ")
# ,
