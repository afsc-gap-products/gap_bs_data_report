...
header <- paste0("Stomach collection target size bins (cm) used to collect each study fish during the ", 
                 maxyr, " ", 
                 text_list(paste0( 
...
header <- paste0("Annual extent of the summer cold pool on the eastern Bering Sea shelf, based on observations from the eastern Bering Sea bottom trawl survey. The extent of the cold pool is shown as a percentage of the total eastern Bering Sea shelf survey area (northern Bering Sea area not included). Shading denotes near-bottom temperatures \u2264 2°C, \u2264 1°C, \u2264 0°C, and \u2264 -1°C.")
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0("Biological data collected during the ", maxyr, " ", 
                   ...
                   " shelf bottom trawl survey. For crab data, see ",paste0("@SAPcrab", maxyr),". ")
...
table_print <- flextable::flextable(
  ...
  #     # common name and first ( if there are sci names
  #     as_chunk(paste0(ifelse(is.na(common_name_), "", common_name_), 
  #                     ifelse((species_name_ital_ != "" | species_name_noital_ != ""), " (", "")),
  #              props = fp_text_default(bold = FALSE, font.size = font_size0, font.family = font0)), 
  #     # italic sci name
  #     as_chunk(ifelse(species_name_ital_ != "", species_name_ital_, ""),
  #              props = fp_text_default(italic = TRUE, font.size = font_size0, font.family = font0)), 
  #     # non-italic sci name and/or )
  #     as_chunk(paste0(ifelse((species_name_ital_ != "" & species_name_noital_ != ""), " ", ""),
  #                     ifelse((species_name_noital_ != ""), species_name_noital_, ""),
  #                     ifelse((species_name_ital_ != "" | species_name_noital_ != ""), ")", "")),
  #              props = fp_text_default(bold = FALSE, font.size = font_size0, font.family = font0)) 
  #     )) %>%  
  # flextable::compose(
  ...
  #     # common name and first ( if there are sci names
  #     as_chunk(paste0(ifelse(is.na(common_name), "", common_name), 
  #                     ifelse((species_name_ital != "" | species_name_noital != ""), " (", "")),
  #              props = fp_text_default(bold = FALSE, font.size = font_size0, font.family = font0)), 
  #     # italic sci name
  #     as_chunk(ifelse(species_name_ital != "", species_name_ital, ""),
  #              props = fp_text_default(italic = TRUE, font.size = font_size0, font.family = font0)), 
  #     # non-italic sci name and/or )
  #     as_chunk(paste0(ifelse((species_name_ital != "" & species_name_noital != ""), " ", ""),
  #                     ifelse((species_name_noital != ""), species_name_noital, ""),
  #                     ifelse((species_name_ital != "" | species_name_noital != ""), ")", "")),
  #              props = fp_text_default(bold = FALSE, font.size = font_size0, font.family = font0)) )) %>% 
...
table_raw0 <- table_raw0  %>% 
  ...
  dplyr::mutate(
    ...
    prop0 = prop) %>% 
  dplyr::relocate(SRVY, group, taxon, biomass_mt_999, a, CI95, prop) %>% 
...
for (i in 1:nrow(comb)) {
  header <- paste0("Total taxon biomass estimates (t) ± 95% confidence interval (CI) and biomass estimates by stratum for major ", 
                   ifelse(comb$taxon[i] =="invert", "invertebrate", "fish") ,
                   " taxa collected during the ", 
                   maxyr, " ", 
                   comb$SRVY_long[i],
                   " shelf bottom trawl survey. Proportion of taxon biomass is divided by the total ",ifelse(comb$taxon[i] =="invert", "invertebrate", "fish") ," biomass caught on the survey. ")
  ...
  pgwidth <- ifelse(comb$SRVY[i] == "EBS", 
                    full_page_landscape_width, full_page_portrait_width)
  
  footnote0 <- paste0("The ", total$SRVY_long, 
                      " bottom trawl survey total estimated ", 
                      text_list(total$taxon_long)," biomass is ", 
                      text_list(formatC(x = total$total, digits = 0, format = "f", big.mark = ",")), 
                      " t." )
  
  table_print <- table_raw1 %>% 
    dplyr::mutate(spacer = "") %>% 
    dplyr::relocate("group", "taxon", "999", "a", "CI95", "prop", "spacer") %>% 
    flextable::flextable()  %>%
    ...
    flextable::add_header_row(
      top = TRUE,
      values = c("Taxon",
                 paste0(ifelse(comb$taxon[i] =="invert", "Invertebrate", "Fish"), " biomass"),
                 "", # spacer
                 "Estimated biomass by stratum"),
                              colwidths = c(1,5,1,(length(strat)))) %>%
    flextable::width(width = (pgwidth-1-.25-1-1-.05)/length(strat), 
                     unit = "in") %>%
    flextable::width(j = "prop", width = 1, unit = "in") %>%
    flextable::width(j = c("a"), width = 0.25, unit = "in") %>%   
    flextable::width(j = c("spacer"), width = 0.1, unit = "in") %>%   
    flextable::width(j = c("group"), width = 1, unit = "in") %>%    
    flextable::width(j = c("taxon"), width = 1, unit = "in") %>% 
    flextable::footnote(part = "header", i = 1, j = 5, ref_symbols = "1",
                        value = as_paragraph(footnote0)) %>%
    flextable::bold(i = ~ (grepl(pattern = "Total", x = group) )) %>%
    flextable::set_header_labels(
      "group" = "Taxon", 
      "taxon" = "Estimated biomass ± 95% CI",
      "999" = "",
      "a" = "",
      "CI95" = "",
    ...
    flextable::merge_at(j = c("group"), part = "header") %>%
    flextable::merge_at(j = c("taxon", "999", "a", "CI95"), i = 2, part = "header") 
  row_lines <- TRUE
  pad <- 0
  body_size = 10
  header_size = 10
  ...
  table_print <- table_print %>% 
    ...
    flextable::align(j = c("999", "prop", strat), # , "CI95", strat
                     align = "right", part = "all") %>%
...
yrs <- nbsyr[(length(nbsyr)-2):length(nbsyr)]
...
#   #                 !is.na(year)) %>% 
#       # dplyr::mutate(species_code = ifelse(is.na(species_code), group, species_code)) %>% 
#       dplyr::select(print_name = group, year, taxon, SRVY, biomass_mt), 
#     x = report_spp %>% 
#       dplyr::filter(table_bio_spp) %>% 
#       dplyr::select(print_name, table_bio_spp)#, # species_name = species_name1, species_code, taxon, 
#     # relationship = "many-to-many",
#     # by = c("species_code")
#     ) %>%
...
for (i in 1:c(nrow(haul_cruises_maxyr))) {
  ...
  header <- paste0(
    ...
    compareyr," and ",maxyr," ", 
    haul_cruises_maxyr$SRVY_long[i], 
    ' shelf bottom trawl surveys for predominant fish and invertebrate taxa. Taxa are listeed in descending order of percent (%) change from ',compareyr,' to ',maxyr,'. ') 
...
for (i in 1:2) {
  ...
  if (i == 1){ # CPUE (WT/km²) and BIOMASS
    ...
    header <- paste0("Mean CPUE by weight (kg/km²) with standard deviation, and estimated biomass (t) with standard deviation and 95% lower (LCL) and upper (UCL) confidence limits for common groundfish species for the ",
    ...
    table_raw <- temp %>% 
      dplyr::select(print_name, SRVY, 
                    cpue_kgkm2_mean, cpue_kgkm2_sd, 
                    biomass_mt, biomass_sd, n_weight) 
    ...
    if (SRVY == "NEBS"){
      table_print <- table_print %>% 
        ...
        flextable::set_header_labels(
          print_name = "Species", 
          SRVY = "Shelf area", 
          cpue_kgkm2_mean = paste0("Mean CPUE (kg/km²)"), 
    ...
    header <- paste0("Mean CPUE by number (no/km²) with standard deviation, and estimated population with standard deviation ", 
    ...
    table_raw <- temp %>% 
      dplyr::select(print_name, SRVY, 
                    cpue_nokm2_mean, cpue_nokm2_sd, 
                    population_count, population_sd, n_weight)
    ...
    if (SRVY == "EBS") {
      table_print <- table_print %>% 
        ...
        flextable::set_header_labels(
          print_name = "Species", 
          cpue_nokm2_mean = paste0("Mean CPUE (no/km²)"), 
  ...
  table_print <- table_print %>% 
    flextable::width(j = "print_name", width = 3) %>%
    ...
    table_print <- table_print %>%
      ...
      flextable::add_header_row(top = TRUE,
                                values = c(ifelse(temp$taxon[i] == "fish", "Family", "Phylum"),
                                           "Scientific name", "Common name", "# Stns Present",
      ...
      flextable::width(width = .1, j = c("spacer"), unit = "in") %>%
      flextable::width(width = 1, j = c("taxon", "common_name"), unit = "in") %>%
...
# Prepare final table ----------------------------------------------------------
table_print <- table_raw %>% 
  flextable::flextable(col_keys = c("dummy", "common_name", "year_started")) %>%
  flextable::compose(
    j = "dummy", 
    value = flextable::as_paragraph(
