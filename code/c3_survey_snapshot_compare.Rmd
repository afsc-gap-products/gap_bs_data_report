...
temp3 <- temp1 %>% 
  ...
  dplyr::select(change0) %>% 
  dplyr::mutate(case = dplyr::case_when(
    change0 > 0 ~ "increased", 
    change0 %in% c(0, NA) ~ "did not change",
    change0 < 0 ~ "decreased")) %>%  
  dplyr::group_by(case) %>%
  dplyr::summarise(change = length(change0)) %>% 
  dplyr::ungroup()
...
In `r maxyr`, the total bottom-dwelling organismal biomass of the EBS shelf was estimated at `r temp$sum[temp$year == maxyr & temp$SRVY == "EBS"] ` million metric tons (t) and the NBS shelf was estimated at `r temp$sum[temp$year == maxyr & temp$SRVY == "NBS"] ` mmt. Previously, the total bottom-dwelling animal biomass of the `r text_list(paste0(temp$year[temp$year != maxyr], " ", temp$SRVY[temp$year != maxyr], " shelf was estimated at ", temp$sum[temp$year != maxyr], " million t"))`. The percent change in biomass varied by fish and invertebrate taxon (Table \@ref(tab:tab-majortaxa-pchange-comm)). 

Calculated EBS biomass `r text_list(paste0(temp3$case, " for ", xunits(temp3$change), ifelse(temp3$change == 1, " taxon", " taxa")))` from `r compareyr ` (`r ifelse(temps_avg_yr$bt_above_mean[temps_avg_yr$year == compareyr], "warm", "cold")`-stanza year) to `r maxyr `. Some of the largest increases in biomass from `r compareyr ` to `r maxyr ` were observed in the `r text_list(paste0(head(temp2$print_name, 5) ," (",head(temp2$change, 5),")"))` groups. Decreases in biomass were observed in the `r text_list(paste0(tail(temp2$print_name, 5) ," (",tail(temp2$change, 5),")"))` groups (Table \@ref(tab:tab-majortaxa-pchange-comm)).
...
for (i in 1:length(unique(temp1$case))) {
  insert <- paste0(insert, 
                   paste0("Species groups that previously exhibited ", 
                          ...
                          "\\@ref(tab:tab-majortaxa-pchange-comm)). "))
}
...
Detailed summary profiles outlining several of the species showing ecologically significant trends are discussed in Table \@ref(tab:tab-majortaxa-pchange-comm).


```{r tab-majortaxa-pchange-comm}
# <!---BLOCK_LANDSCAPE_START--->
nickname <- "tab-majortaxa-pchange-comm"
# <!-- -BLOCK_LANDSCAPE_STOP- -->
```

```{r tab-majortaxa-pchange-comm-child, child = paste0(dir_code, "_child_report_figtab.Rmd")}
```
