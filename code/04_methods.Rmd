...
if (sum(!is.na(temp$vessel)) == nrow(temp)) {
  ...
  str0 <- paste0(str0, "To estimate missing net width values, the mgcv package in R [@RN997] was used to relate mean net width with the inverse scope (m) and mean net height (m) from valid tows following the relationship investigated by @RN921, where $w$ is the net width (m), $h$ is the net height (m), and $s$ is the scope, inversed, below.
...
At the beginning of the survey, scientists boarded the vessels (`r text_list(vessel_info$vessel_ital)`) in Dutch Harbor, Alaska, then headed to eastern Bristol Bay to begin surveying.  From Bristol Bay, the survey then proceeded westward to the shelf edge (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `). The east to west survey progression is intended to ensure the survey moves in the opposite direction of the seasonal on-shelf (eastward) migration of yellowfin sole and other species [@RN928]. In the EBS, `r str0$str[str0$SRVY == "EBS"]` (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `).
...
`r NMFSReports::crossref(list_obj = list_tables, nickname = c("tab_specimen_samples_", "tab_oto_collection_scheme_"), sublist = "number", exact = FALSE)`) and brought to the AFSC for age determination. Otolith samples were collected following `r str2 ` (Table `r NMFSReports::crossref(list_obj = list_tables, nickname = "tab_oto_collection_scheme_", sublist = "number", exact = FALSE)`). Otoliths were preserved in 50% glycerol-thymol solution. Individual fish weights and lengths were collected for each fish from which age structures were taken. For walleye pollock, the survey area was also divided into low- and high-density strata based on historical population densities and an isobath of approximately 70 m.  
