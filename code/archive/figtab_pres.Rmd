...
## fig_pres_sample_grid1

```{r fig_pres_sample_grid1}
...
nickname <- "fig_pres_sample_grid1"
...
nickname <- "fig-pres-temperature-[bt/st]"
height <- 5
...
rasterbrick <- raster::brick(
              raster::subset(x = coldpool:::nbs_ebs_surface_temperature,
                              subset = paste0("ebs_ste_", maxyr, "_surface_temperature")), 
              raster::subset(x = coldpool:::nbs_ebs_bottom_temperature,
                              subset = paste0("ebs_ste_", maxyr, "_gear_temperature")) )
figure <- plot_temps_facet(
  rasterbrick = rasterbrick, 
  key.title = expression(bold("Temperature (°C)")), 
  reg_dat = report_types$NEBS$reg_dat, 
  colorbar_breaks = c(-Inf, seq(from = 0, to = 14, by = 2), Inf),
  viridis_palette_option = "H", 
  row = 1, 
  use_col_name = c(paste0(maxyr, " Surface Temperature"), 
                      paste0(maxyr, " Bottom Temperature")))
...
header <- paste0("Contour map of surface temperatures from the ",maxyr," ",
                 text_list(haul_cruises_maxyr$survey_name)," shelf bottom trawl surveys.")
