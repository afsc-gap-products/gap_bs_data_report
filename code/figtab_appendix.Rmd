...
for (i in 1:nrow(temp)) {
  ...
  for (ii in 1:ceiling(ncol(table_print1)/ncol_per_page)) {
    cnt_pre <- "A"
    subobj <- TRUE
  ...
  nickname <- paste0(nickname0, 
                     ...
                     gsub(pattern = " ", replacement = "", perl = TRUE, 
                          x = gsub(pattern = "F/V ", replacement = "", 
                                   x = temp$vessel_name[i], fixed = TRUE)), "_", i,
                     "_", ii)
    
    c_start <- c_start + 1
    if (ii == ceiling(ncol(table_print1)/ncol_per_page)) {
      c_end <- ncol(table_print1)
    } else {
    ...
    table_print <- table_print1[,c(1, c_start:c_end)] %>% 
      ...
      header <- paste0("Haul and catch data for successfully completed tows by ", 
                       temp$vessel_ital[i]," during the ", maxyr, " ", 
                       temp$SRVY_long[i], 
