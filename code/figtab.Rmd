...
table_raw0 <- 
  dplyr::left_join(
    ...
    x = report_spp1 %>% 
      dplyr::select(print_name, species_name = species_name1, species_code, group0, taxon), 
    relationship = "many-to-many",
    by = c("species_code")) %>%
  dplyr::filter(!is.na(group0)) %>%
  dplyr::group_by(print_name, species_name, SRVY, year, taxon) %>% 
  dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
  dplyr::ungroup() %>%
  ...
  tidyr::pivot_wider(id_cols = c(SRVY, print_name, species_name, taxon), 
                     names_from = year, 
...
for (i in 1:c(nrow(haul_cruises_maxyr))) {
  ...
  table_print <- table_raw  %>% 
    ...
    # flextable::flextable() %>%
    flextable::flextable(data = ., col_keys = c(#"group", 
      "print_name",
      "dummy", 
      as.character(nbsyr[!(nbsyr %in% c(maxyr, compareyr))]), 
      "compareyr", "maxyr", 
      "change")) %>%
