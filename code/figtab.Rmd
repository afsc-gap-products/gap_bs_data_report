...
```{r fig_sample_grid}
header <- paste0("Sampling grid and station identifiers for the ",maxyr," ",NMFSReports::text_list(haul_cruises_maxyr$SRVY)," continental shelf bottom trawl surveys.")
...
# header <- paste0("Sampled survey stations by vessel and the stratification scheme used for data analysis of ",maxyr," ",NMFSReports::text_list(haul_cruises_maxyr$SRVY)," continental shelf bottom trawl surveys.")
nickname <- "fig_sampled_survey_stations"
header <- paste0("Sampled survey stations by vessel and the stratification scheme used for data analysis of ",maxyr," ",NMFSReports::text_list(haul_cruises_maxyr$SRVY)," continental shelf bottom trawl surveys. The map also depicts the stations sampled by each survey vessel and where, if any, Norton Sound crab resampling was done.")
# header <- paste0("Map of the Bering Sea survey stations sampled in ",maxyr," during the EBS and NBS survey. The area enclosed within the light gray line contains the EBS shelf stations that have been sampled annually since 1982, whereas the area outlined in the dark gray line are the NBS stations that were sampled in ",maxyr,". The dots within each area indicate each station location.") # TOLEDO
width <- 6.5
height <- 6
...
```{r fig_trawl_gear}
header <- paste0("Schematic diagram of the 83-112 eastern otter trawl gear used during the ", maxyr ," ", SRVY ," bottom trawl surveys.")
footnote<- NA
...
table_print <- flextable::flextable(table_raw) %>%
  ...
  flextable::width(x = ., j = "Region", width = 3) %>% 
  flextable::width(x = ., j = "density", width = 4) %>% 
  ...
  theme_flextable_nmfstm(font = font0)
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  flextable::flextable(data = .) %>%
    ...
    flextable::valign(valign = "top") %>%
    theme_flextable_nmfstm(font = font0, x = .)
...
table_print <- table_raw %>% 
  ...
  NMFSReports::theme_flextable_nmfstm(font = font0, x = ., row_lines = TRUE) 
...
table_print <- table_raw %>%
  ...
  #                    j = 1, i = ~ is.na(SRVY), 
  #                    padding.left = 20) %>%
  theme_flextable_nmfstm(font = font0, row_lines = TRUE) %>% 
  flextable::width(x = ., j = "Agency", width = 1.5) %>%
...
color_sst <- nmfs_palette("oceans")(2)[1]#"darkgreen"
color_bt <- nmfs_palette("oceans")(2)[2]#"darkblue"

ebs_mean_temp_df <- data.frame(region = c(rep("Eastern Bering Sea", 2),
...
# height <- dim(readPNG(paste0(dir_img, "stacked_temperature_simple_label_",maxyr,".png")))[1]/300/2 # 4
# width <- dim(readPNG(paste0(dir_img, "stacked_temperature_simple_label_",maxyr,".png")))[2]/300/2 # 6
height = 4
width <- 6.5
# get the most updated file, just in case
...
# figure <- 
#   cowplot::ggdraw() +
#   cowplot::draw_image(readPNG(paste0(dir_img, "stacked_temperature_simple_label_",maxyr,".png")) )
# }
...
header <- paste0("Contour map of surface temperatures from the ",maxyr," ",
                 NMFSReports::text_list(haul_cruises_maxyr$SRVY)," Shelf Bottom Trawl Surveys.")
...
table_raw <- catch_haul_cruises_maxyr %>% 
  ...
  table_print <- temp %>%
    ...
    theme_flextable_nmfstm(font = font0, row_lines = FALSE) %>%
...
temp <- function(dat, total_biomass){
  ...
  biomass_group_totals <- dat %>%
    ...
    dplyr::mutate(taxon = paste0("Total ", group), 
                  order = 3)
  
  biomass_group_totals_999 <- biomass_group_totals %>% 
    dplyr::group_by(SRVY, group, taxon, order) %>% 
    dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
    dplyr::mutate(strata = 999)
  
  # find estimated total biomass for all stratum for all taxon
  biomass_totals <- dat %>%
    dplyr:: group_by(SRVY, group, taxon) %>%
    dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
    dplyr::mutate(strata = 999, 
                  order = ifelse(grepl(pattern = "other ", x = taxon), 2, 1))
  
  biomass_group_strata <- dat %>%
    dplyr::group_by(SRVY, strata, group, taxon) %>%
    dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
    dplyr::mutate(order = ifelse(grepl(pattern = "other ", x = taxon), 2, 1))
  
  biomass_all_spp_total <- 
    dplyr::bind_rows(biomass_group_strata, 
                     biomass_group_totals, 
                     biomass_totals) %>% 
    dplyr::filter(!grepl(pattern = "Total ", x = taxon)) %>%
    dplyr::group_by(SRVY, strata) %>% 
    dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
    dplyr::mutate(order = 5, 
                  group = gsub(pattern = "Other ", 
                               replacement = "Total ", 
                               x = unique(biomass_group_strata$group[grepl(pattern = "Other ", 
                                                    x = biomass_group_strata$group)])))
  ...
  #                    biomass_totals) %>% 
  #   dplyr::filter(!grepl(pattern = "Total ", x = taxon)) %>%
  #   dplyr::group_by(SRVY, group, taxon, strata) %>% 
  #   dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE)) %>% 
  #   dplyr::mutate(order = 5, 
  #                 taxon = paste0("Total ", taxon))
  
  out <- dplyr::bind_rows(biomass_group_strata, 
                          biomass_group_totals, 
                          biomass_group_totals_999, 
                          biomass_totals, 
                          biomass_all_spp_total) %>% 
  ...
  out$order[grepl(pattern = "Other ", x = out$group)] <- 4
  ...
  out$order[grepl(pattern = "Other ", x = out$group)] <- nrow(out)+1
  out$order[grepl(pattern = "Total ", x = out$group)] <- nrow(out)+2
  
  out <- dplyr::left_join(
    x = out, 
    y = total_biomass, 
    by = "SRVY")%>%
    dplyr::mutate(CI95 = NA, 
                  a = "±", 
                  prop = `999`/total) %>% 
    dplyr::arrange(order) %>%
...
temp1 <- function(table_raw, strat, total) {
  ...
  table_raw <- table_raw %>% 
    dplyr::select("group", "taxon", "999", "a", "CI95", "prop", 
      ...
      footnote0 <- paste0("Proportion of total estimated biomass, fish and invertebrates combined, for the total survey area = ", 
                      NMFSReports::xunits(total, words = FALSE), " mt.")
  ...
  table_print <- table_raw %>% 
    dplyr::mutate(prop = ifelse(prop < 0.0001, "<0.0001", 
                                formatC(x = prop, digits = 4, 
                                        format = "f", big.mark = ",")), 
                  CI95 = paste0(round(x = CI95, digits = 0), "%") )  %>% 
    dplyr::mutate(across(where(is.numeric), 
                       formatC, big.mark=",", digits = 0, format = "f"))
  ...
  table_print <- table_print %>%
    ...
    flextable::add_header_row(top = TRUE, 
                              values = c("Taxon", 
                                         "Estimated total biomass (mt) and 95% confidence", 
    ...
    # flextable::bold(x = ., i = ~ (grepl(pattern = "Total ", x = taxon) )) %>%
    ...
    flextable::set_header_labels(.,
                                 ...
                                 "a" = "", 
                                 "prop" = "") %>%
    NMFSReports::theme_flextable_nmfstm(font = font0, row_lines = TRUE) 
...
fish_groups <- biomass_by_stratum %>%
  dplyr::filter(species_code < 35000) %>% # species codes greater than 35000 are invertebrates
  dplyr::mutate(group0 = case_when(
    ...
    species_code == 10115 ~ "Pleuronectidae (flatfishes)_Greenland turbot",
    ...
    species_code == 30060 ~ "Scorpaenidae (rockfish)_Pacific ocean perch",
    species_code >= 30000 & species_code <= 31550 & 
      species_code != 30060 ~ "Scorpaenidae (rockfish)_other rockfish",
    TRUE ~ "Other fish"))

table_raw0 <- temp(dat = fish_groups, total_biomass = total_biomass)
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0("Biomass estimates (mt) for major fish species and groups taken during the ", 
                   maxyr, " ", 
                   haul_cruises_maxyr$SRVY[i],
                   " Shelf Bottom Trawl Survey.")
...
invert_groups <- biomass_by_stratum %>%
  ...
  dplyr::mutate(group0 = dplyr::case_when(
    ...
    species_code >= 45000	& species_code <= 59999 ~ "Other invertebrates",
    species_code >= 92000	& species_code <= 97999 ~ "Other invertebrates",
    species_code >= 99990 & species_code <=	99992 ~ "Other invertebrates",
    species_code >= 77011 & species_code <= 77012 ~ "Other invertebrates",
    species_code == 99987 ~ "Porifera (sponges)",
    TRUE ~ "find my group"))


table_raw0 <- temp(dat = invert_groups, total_biomass = total_biomass)
...
for (i in 1:nrow(haul_cruises_maxyr)) {
  ...
  header <- paste0("Biomass estimates (mt) for major invertebrate species and groups taken during the ", 
                   maxyr, " ", 
                   haul_cruises_maxyr$SRVY[i],
                   " Shelf Bottom Trawl Survey.")
...
#     newobj <- ifelse(i==1, TRUE, FALSE)
#   }
#   header <- paste0("Total estimated biomass in metric tons (mt) and the percent change from the ",compareyr," and ",maxyr," ",haul_cruises_maxyr$SRVY[i]," (",haul_cruises_maxyr$SRVY_long[i],") bottom trawl surveys for invertebrate and fish taxa captured in greater than 50 trawl samples from both years combined.")
#   
#   nickname <- paste0(nickname0, haul_cruises_maxyr$SRVY[i]) 
#   
...
#     NMFSReports::theme_flextable_nmfstm(font = font0, row_lines = FALSE)  %>% 
...
header <- paste0(
  "Total estimated biomass in metric tons (mt) and the percent change from the ",
                 compareyr," and ",maxyr," ", 
                 haul_cruises_maxyr$SRVY[i], 
                 " Shelf Bottom Trawl Surveys for invertebrate and fish taxa captured in greater than 50 trawl samples from both years combined.")
...
table_print <- table_print %>%
  ...
  flextable::color(color = "red", 
                   i = grepl(pattern = "-", x = as.character(table_print[, "change "])), 
                   j = "change ") %>% 
  flextable::width(x = ., j = "Fish taxon", width = 1.5) %>% 
  flextable::width(x = ., j = "Invertebrate taxon", width = 1.5) %>% 
  flextable::width(x = ., j = "change ", width = .5) %>% 
  flextable::width(x = ., j = "change", width = .5) %>% 
  flextable::set_header_labels(.,
                               # group = "Common name",
                               # dummy = "Taxon",
                               ...
                               change = paste0("Change (", maxyr, ", ", compareyr, ")" )) %>% 
  NMFSReports::theme_flextable_nmfstm(font = font0, row_lines = FALSE, x = .)  %>% 
