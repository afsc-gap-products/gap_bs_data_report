...
```{r fig-temperature-[SRVY]-[st/bt]}
  ...
  for (case in c("bottom", "surface")) {
      yrs0 <- (maxyr-17):maxyr # sort(sort(c(unique(cruises$year), 2020), decreasing = TRUE)[1:20])
      row0 <- 3
      ...
      if (case == "surface") {
        ...
        sd_mean <- data.frame(year = coldpool::cold_pool_index$YEAR, 
                                   sd = scale(coldpool::cold_pool_index$MEAN_SURFACE_TEMPERATURE)) # TOLEDO NOT SD
        ...
        sd_mean <- data.frame(year = coldpool::cold_pool_index$YEAR, 
                                   sd = scale(coldpool::cold_pool_index$MEAN_GEAR_TEMPERATURE)) # TOLEDO NOT SD
    ...
    figure <- plot_temps_facet(
      ...
      title0 = paste0("Eastern", ifelse(SRVY == "NEBS", " and northern", ""), 
                      " Bering Sea ", 
                      stringr::str_to_title(case), " Temperature (", range_text(yrs), ")"), 
      sd_mean = sd_mean) 
