...
str0 <- paste0("Between ", compareyr," and ", maxyr, 
               ifelse(grepl(pattern = "*", x = spp_sci, fixed = TRUE), # is this a group of fishes/organisms?
                      paste0(", the biomass of ", spp_print), # true, e.g., 'snailfishes'
                      paste0(", the ", spp_print, " biomass")), # false
               " estimate ")
...
cite_pop <- paste0(" (Table",ifelse(spp_table_cpue, "s ", " "), 
  ...
  if (temp0$catch_min < 100) {
    aa <- temp0$`Survey catch totals`
    aa <- gsub(x = aa, pattern = "\n", replacement = "")
    aa <- gsub(x = aa, pattern = "Increased ", replacement = "")
    aa <- gsub(x = aa, pattern = "Decreased ", replacement = "")
    aa <- gsub(x = aa, pattern = paste0("in ", maxyr), replacement = "caught on survey")
    case <- paste0(case, " (", aa, ")")
  }
