...
```{r fig_sample_grid}
header <- paste0("Sampling grid and station identifiers for the ",maxyr," ",NMFSReports::text_list(haul_cruises_maxyr$SRVY)," continental shelf bottom trawl survey", 
                 ifelse(SRVY == "NEBS", "s", ""), ".")
...
header <- paste0("Sampled survey stations by vessel and the stratification scheme used for data analysis of ",maxyr," ",NMFSReports::text_list(haul_cruises_maxyr$SRVY)," continental shelf bottom trawl survey", ifelse(SRVY == "NEBS", "s", ""), ". The map also depicts the stations sampled by each survey vessel and where, if any, Norton Sound crab resampling was done.")
# header <- paste0("Map of the Bering Sea survey stations sampled in ",maxyr," during the EBS and NBS survey. The area enclosed within the light gray line contains the EBS shelf stations that have been sampled annually since 1982, whereas the area outlined in the dark gray line are the NBS stations that were sampled in ",maxyr,". The dots within each area indicate each station location.") # TOLEDO
width <- 6.5
height <- 6
...
table_print <- flextable::flextable(
  data = table_raw, 
  col_keys = c("Region", "stratum", "area", "stations_completed", "density"))  %>%
  ...
  flextable::width(x = ., j = "Region", width = 4) %>% 
  flextable::width(x = ., j = "stratum", width = 2) %>% 
...
for (i in 1:2) { # two cases, above and below
  ...
  figure <- plot_temps_facet(
    ...
    colorbar_breaks = c(-Inf, seq(from = 0, to = 14, by = 2), Inf),
    viridis_palette_option = "H", 
    row = ifelse(length(rasterbrick)>6, 3, 4))
...
temp <- function(dat, total_biomass){
  ...
  dat <- dat  %>% 
    ...
    tidyr::separate(
      ...
      sep = "_", 
      remove = FALSE) %>% 
    dplyr::select(SRVY, strata, group, group0, taxon, biomass_mt, ci)
  ...
  biomass_group_totals <- dat %>%
    ...
    dplyr::mutate(taxon = paste0("total ", tolower(group)), 
                  order = 3)
  ...
  biomass_group_strata <- dat %>%
    dplyr::group_by(SRVY, strata, group, taxon) %>% #, area, year) %>%
    dplyr::summarise(biomass_mt = sum(biomass_mt, na.rm = TRUE), 
                     ci = sum(ci, na.rm = TRUE)) %>% 
...
temp1 <- function(table_raw, strat, total) {
  ...
  table_print <- table_raw %>% 
    dplyr::mutate(prop = ifelse(prop < 0.0001, "<0.0001", 
                                formatC(x = prop, digits = 4, 
                                        format = "f", big.mark = ",")) ) %>% # , 
                  # CI95 = formatC(x = CI95, digits = 0, format = "f", ) ) %>% 
    dplyr::mutate(across(where(is.numeric), 
                       formatC, big.mark=",", digits = 0, format = "f"))
  ...
  table_print <- table_print %>%
    ...
    flextable::add_header_row(top = TRUE, 
                              values = c("Taxon", 
                                         "Estimated total biomass (mt) and 95% confidence interval", 
...
table_print <- table_print %>%
  ...
  flextable::width(x = ., j = "change", width = 1) %>% 
  # flextable::width(x = ., j = maxyr, width = 2) %>% 
  # flextable::width(x = ., j = compareyr, width = 2) %>% 
  flextable::set_header_labels(.,
                               # group = "Common name",
                               # dummy = "Taxon",
