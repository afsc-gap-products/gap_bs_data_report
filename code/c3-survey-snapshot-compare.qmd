...
```{r results-cold-pool_insert, child=paste0(dir_out_rawdata, "/0-cold-pool.qmd")}
```
...
```{r temperature-10-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-20-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-30-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r temperature-40-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r fig-mean-temperature0-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
```{r fig-cold-pool-area0-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```
...
From the `r SURVEY` shelf trawl survey`r plural_surveys`, length measurements were collected from `r xunits(temp000$n_samples)` individual fish representing `r xunits(temp000$n_taxa)` fish taxa. Additionally, `r str0`. 
...
paste0(
  ...
  ifelse(i == 1, " (± 25% change from the previous survey year). ", ". "),
               ...
               #        collapse = ifelse(i == 1, " (Table (#tab-majortaxa-pchange-NBS)). ", ". ")),  
  collapse = ". " )#,  ifelse(i == 1, ".\n\n", ". ") 
)
}
...
`r str0`Prominent organism's biomasses are listed in Table (#tab-majortaxa-pchange-NBS) in order of largest increase to largest decrease. 
...
Northern Bering Sea survey catches were generally smaller than those from the eastern Bering Sea. Distributions of many predominant species extended throughout much of both survey regions. Several key species were found in the northern Bering Sea in greater numbers than the eastern Bering Sea, including `r text_list(temp5$print_name)`.
...
```{r tab-majortaxa-pchange-NBS0-child, child = paste0(dir_code, "_child_report_figtab.qmd")}
```

# Summary Results for Select Major Taxa

Survey results for select taxa are presented with a photograph of the taxon, a short summary of results, maps of geographic distribution of CPUE (kg/km^2^), and plots showing total population estimates by size. Maps of species distributions include both the eastern and northern Bering Sea survey regions to better illustrate patterns and trends in fish distribution and movement. For comparison, distribution maps and abundance-at-size plots show survey data for the `r text_list(nbsyr) ` surveys. Detailed summary profiles outlining several of the species showing ecologically significant trends are included below. 

**You can help us with this document by providing names in local language(s) and cultural or traditional uses for each fish and invertebrate species reviewed in this report by reaching out to [afsc.gap.metadata@noaa.gov](afsc.gap.metadata@noaa.gov).**
