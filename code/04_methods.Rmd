...
The `r text_list(vessel_info$vessel_ital)` conducted the standard EBS shelf survey charters. At the beginning of the survey, scientists boarded the vessels in Dutch Harbor, Alaska, then headed to eastern Bristol Bay to begin surveying.  From Bristol Bay, the survey then proceeded westward to the shelf edge (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `). The progression from east to west was established in response to movements of yellowfin sole and perhaps other species, which may be migrating eastward during the course of the survey [@RN928]. In the EBS, `r str$str[str$SRVY == "EBS"]` (Figure `r crossref(list_obj = list_figures, nickname = "fig_sampled_survey_stations", sublist = "number") `).
...
if (SRVY == "NEBS") {
  ...
  nbs_insert <- paste0(
    "After the completion of the EBS shelf survey, both vessels transitioned into sampling survey stations in the southwest corner of the NBS survey region. In the NBS survey, ", 
...
`r NMFSReports::crossref(list_obj = list_figures, nickname = "tab_specimen_samples_", sublist = "number", exact = FALSE) `
...
`r NMFSReports::crossref(list_obj = list_figures, nickname = "tab_oto_collection_scheme_", sublist = "number", exact = FALSE) `

```{r}
temp <- NMFSReports::crossref(list_obj = list_tables, 
                              nickname = "tab_stomach_collection_scheme", 
                              sublist = "raw")  
```
