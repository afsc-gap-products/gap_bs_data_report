...
The standardized `r SURVEY` bottom trawl survey`r plural_surveys` use`r ifelse(plural_surveys == "s", "", "s")` a systematic design with `r ifelse(SRVY == "NEBS", text_list(paste0(haul_cruises_maxyr$stations_avail, " fixed sampling stations in the ", haul_cruises_maxyr$SRVY_long)), paste0( haul_cruises_maxyr$stations_avail, " fixed sampling stations"))` arranged on a regularly spaced 37.04 × 37.04 km grid (20 × 20 nautical mile; Fig. \@ref(fig:fig-survey-grid)). Additional stations, called "corner stations", were added to the survey in 1990 to better sample regions of historically high blue king crab abundances. There are 26 corner stations located at the intersections of the grid lines in the waters surrounding St. Matthew and the Pribilof Islands (Fig. \@ref(fig:fig-survey-grid)). These corner stations are sampled in addition to the grid cells. `r ifelse(SRVY == "NEBS", paste0("In addition to the eastern Bering Sea shelf bottom trawl survey, the ",maxyr," northern Bering Sea shelf bottom trawl survey was conducted using the same systematic sampling design for stations bounded by the U.S.-Russian Maritime Boundary and the Bering Strait, including Norton Sound.") , "")` 
...
temp <- obj$raw %>% 
  ...
  SRVY_long = dplyr::case_when(
    SRVY_long == "Eastern and northern Bering Sea Combined" ~ "eastern and northern Bering Sea combined", 
    SRVY_long == "Northern Bering Sea" ~ "northern Bering Sea",
    SRVY_long == "Eastern Bering Sea" ~ "eastern Bering Sea" )) %>% 
  dplyr::arrange(-n)
