...
table_print <- table_raw %>%
  dplyr::select(-SRVY) %>% 
  flextable::as_grouped_data(x = ., groups = c("SRVY_long"), columns = NULL)  %>%
  ...
  flextable::bold(x = ., j = 1, i = ~ !is.na(SRVY_long) ) %>%
  flextable::padding(x = .,
                     j = 1, i = ~ is.na(SRVY_long),
                     padding.left = 5)   %>%
...
table_print <- table_raw %>% 
  ...
  theme_flextable_nmfstm(x = ., 
                         ...
                         row_lines = TRUE, 
                         pgwidth = full_page_portrait_width)  %>%
  flextable::width(width = c((full_page_portrait_width - (4.5/length(SRVY1))), 
...
table_print <- table_raw %>%
  ...
  flextable::padding(x = .,
                     j = 1, i = ~ is.na(SRVY_long),
                     padding.left = 5) %>% 
  flextable::hline(x = ., i = ~ !is.na(SRVY_long), 
                   border = fp_border(color = "white"), 
                   part = "body")
...
if (!("NBS" %in% catch_haul_cruises_maxyr$SRVY)) {
  ...
  table_print <- flextable::flextable(
    ...
    flextable::compose(x = ., i = 1, j = "dummy_ebs", part = "header",
                       as_paragraph("Encountered in eastern Bering Sea but not in northern Bering Sea\nCommon Name ", 
...
temp1 <- function(table_raw, strat, total) {
  ...
  footnote0 <- paste0("Total estimated biomass is ", 
                      ...
                      unique(table_raw$SRVY_long), " bottom trawl survey.")
