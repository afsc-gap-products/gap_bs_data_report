  ...
  for (i in 1:length(unique(temp$SRVY))) {
    ...
    if (length(str2)>1 & # if there is more than one region
      ...
      str2 <- paste0(str2[1], " sampling method", plural_surveys, 
                     " in both the ", text_list(unique(temp$SRVY)))
    } else {
      temp0 <- temp %>% 
        dplyr::group_by(SRVY) %>% 
