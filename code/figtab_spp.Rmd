...
for (case_yr in case_yr0) {
  ...
  #                   # 1,#2,3,
  #                   7, 
  #                   set.breaks[length(set.breaks)]) # (length(set.breaks)-3):
  # }
...
header <- paste0("Summary of catch location environmenal variables, as well as biomass and population estimates, for ", 
...
if (sum(temp$biomass_mt[temp$stratum == "Total"] != 0) > 0) {
  ...
  flextable::as_flextable(hide_grouplabel = TRUE) %>%
    ...
    table_print <- table_print %>% 
      ...
      flextable::width(width = .75, unit = "in") %>%
      flextable::width(width = .5, unit = "in", j = c("stratum", "n_haul")) %>%
