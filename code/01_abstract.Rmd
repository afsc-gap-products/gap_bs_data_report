...
for (i in 1:length(SRVY1)){
  ...
  # if(srvy == "EBS") {
  #   total_biomass <- total_biomass %>% filter(year >= maxyr - 5)
  # }
  temp_mean <- total_biomass %>% 
    dplyr::group_by(SRVY, year) %>% 
    dplyr::mutate(biomass = sum(total)) %>%
    dplyr::group_by(SRVY) %>%
