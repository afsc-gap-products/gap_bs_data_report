...
The mean bottom temperature on the eastern Bering Sea was `3.0`°C in `r maxyr`, which was `0.5`°C warmer than the time series average from 1982 to `r maxyr-1` (`both 2.5`°C; Fig. \@ref(fig:fig-mean-temperature)). Although temperatures increased from `r maxyr-1` to `r maxyr`, the `r maxyr` mean bottom temperature was still classified as near-average (within one standard deviation of the time series mean).Over the history of the eastern Bering Sea survey (1982–`r maxyr-1`), annual mean summer bottom temperatures have ranged from `0.7`°C to `4.4`°C, and surface temperatures have ranged from `3.9`°C to `9.5`°C. The `r maxyr` mean eastern Bering Sea surface temperature (`6.1`°C) was colder than the times-series average surface temperature (`6.7`°C), and warmer than the mean surface temperature in `r maxyr-1` (`5.8`°C). 
...
compareyr_maxyr_area_perc_diff <- pchange(start = compareyr_area0, end = maxyr_area0)
compareyr_maxyr_area_diff <- (compareyr_area0 - maxyr_area0)
compareyr_maxyr_area_diff_desc <- paste0(ifelse(compareyr_maxyr_area_diff>0, "larger", "smaller"))
compareyr_maxyr_area_diff <- formatC(x = abs(compareyr_maxyr_area_diff), digits = 1, format = "f")

max_yr <- formatC(x = temp$year[temp$perc == max(temp$perc)], digits = 1, big.mark = ",", format = "f")
max_area <- formatC(x = temp$area_km2[temp$perc == max(temp$perc)], digits = 0, big.mark = ",", format = "f")

min_yr <- formatC(x = temp$year[temp$perc == min(temp$perc)], digits = 1, big.mark = ",", format = "f")
min_area <- formatC(x = temp$area_km2[temp$perc == min(temp$perc)], digits = 0, big.mark = ",", format = "f")

mean_perc <- formatC(x = mean(temp$perc), digits = 1, big.mark = ",", format = "f")
mean_area <- formatC(x = mean(temp$area_km2), digits = 0, big.mark = ",", format = "f")
...
The cold pool is primarily found in the `middle domain (50–100 m)` and covered `r maxyr_perc`% (`maxyr_area` 
km^2^) of the eastern Bering Sea survey area in `r maxyr` (Figs. \@ref(fig:fig-temperature-bt-data) and \@ref(fig:fig-cold-pool-area)). The cold pool was `27.7`% (`43,425` km^2^) `smaller` than in `r maxyr-1` and was `36.6`% `smaller` than the time series mean (`mean_area` km^2^). The extent of isotherms for bottom temperatures in `r maxyr` of ≤ 0°C (`30,275` km^2^) and ≤ -1°C (`8,350` km^2^) were `6.0`% and `19.8`% larger than in `r maxyr-1` During the `43`-year time series, the cold pool area has ranged from a minimum of `6,150` km^2^ in `2018` to a maximum of `385,975` km^2^ in `1999`, comprising `1.2`% to `78.2`% of the total eastern Bering Sea survey area.

The cold pool is primarily found in the `middle domain (50–100 m)` and covered `r maxyr_perc`% (`maxyr_area` km^2^) of the eastern Bering Sea survey area in `r maxyr` (Figs. \@ref(fig:fig-temperature-bt-data) and \@ref(fig:fig-cold-pool-area)). The cold pool was `27.7`% (`43,425` km^2^) `smaller` than in `r maxyr-1` and was `36.6`% `smaller` than the time series mean (`mean_area` km^2^). The extent of isotherms for bottom temperatures in `r maxyr` of ≤ 0°C (`30,275` km^2^) and ≤ -1°C (`8,350` km^2^) were `6.0`% and `19.8`% larger than in `r maxyr-1` During the `43`-year time series, the cold pool area has ranged from a minimum of `6,150` km^2^ in `2018` to a maximum of `385,975` km^2^ in `1999`, comprising `1.2`% to `78.2`% of the total eastern Bering Sea survey area.
